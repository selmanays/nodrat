...
**Sürüm:** v0.5
**Son güncelleme:** 2026-05-18 (v0.8.1 — #927 §4.5: Faz-C #997 Wikidata-alias entity genişletme ❌ NEGATİF spike (flag-OFF×2==baseline kanıtlanabilir no-op, flag-ON×2 Δ=0.000, CLOSED not-planned merge-edilmedi; Faz-B-bis DROPPED; Faz-D ATLANDI kanıt-yönlü pivot; **#927 EPIC CLOSED/COMPLETED**). v0.8 (2026-05-17) — #927 zinciri §4.5: Türkçe entity match C-locale collation (#939 — DB `datcollate=C` → `LOWER(x COLLATE "tr-TR-x-icu")` RESCUE/FILTER) + planner entity kök-form backstop & `planner_cache` `prompt_version` invalidation (#942/#945/#947). v0.7 — #893 §3.1: yeni `embedding_fast_queue` + `worker_embedding_fast` (taze haber adanmış lane; clean→aranabilir ~2-9dk → ~30sn; bulk `embedding_queue` DEĞİŞMEDİ); embedding_queue açıklaması güncel (concurrency 4, bulk). v0.6 — denetim §4.5 chat-only/agentic yönlendirme. Önceki: 2026-05-11 (v0.5 — #714 cards NER; v0.4 — #685 worker concurrency; #696 admin retrieval settings)
...
> 🔧 **Türkçe entity match — C-locale collation (#939, 2026-05-17):** Production DB `datcollate=C datctype=C` ile kurulu. PostgreSQL C-locale `LOWER()` **yalnız ASCII a-z** küçültür; Türkçe büyük harfleri (Ö Ü Ç Ş Ğ İ) **olduğu gibi bırakır** (`LOWER('Özgür Özel')` = `'Özgür Özel'`). `critical_entities` MUST_MATCH (RESCUE + FILTER) `LOWER(a.title||subtitle||clean_text) LIKE/~* :ent` ile eşleştirir; `:ent` Python `.lower()` ile tam küçük → SQL tarafı C-locale'de büyük kalır → **Türkçe-karakterli her entity (Özgür Özel, Çin, İzmir, 15 Temmuz…) ASLA eşleşmiyordu** (dense embedding'den gelen prototipik haber baskın çıkıyor, niş/taze haber kaçıyordu — "entity-synonym broken" diye yıllarca yanlış teşhis edilen niche_007/009'un Türkçe-tarafı). **Fix:** RESCUE/FILTER 4 noktada `LOWER(x)` → `LOWER(x COLLATE "tr-TR-x-icu")` (ICU collation prod'da mevcut; operatör/RRF/parent-doc DEĞİŞMEZ). Benchmark recall@5 0.636→0.727, recall@10 0.818→0.909. Detay: [turkish-collation-entity-match](../../wiki/decisions/turkish-collation-entity-match.md).

> 🔧 **Planner entity kök-form + cache invalidation (#942/#945/#947, 2026-05-17):** `query_planner` LLM `critical_entities`'i Türkçe çekim eki + noktalama karşısında ya kelime-ortasından kesiyor ("özel"→"öz") ya da çekim-ekli üretiyordu ("özelle" — kök değil); ekli/kesik entity #939-fixli RESCUE'da bile eşleşmez. **Fix (iki katman):** (1) `SYSTEM_PROMPT` §CRITICAL_ENTITIES kök-form ZORUNLU kuralı + few-shot (`PROMPT_VERSION` 1.4.0→1.6.0); (2) `parse_response` kod-backstop `_canonical_token`/`_entity_canonical` — entity token ham sorgudaki bir kelimenin TR-ek-soyulmuş kökü değilse düş, ekliyse köke indir; DAR `_STEM_SUFFIXES` (tek-harf ünlü hariç → "rusya"/"boğazı" over-stem'den korunur). Ayrıca **`planner_cache._cache_key` bileşenine `prompt_version`** eklendi (önceden `sha1(request|locale|tier|date)` — PROMPT_VERSION'suz + 24h TTL → bir prompt/planner fix'i deploy edilse de o gün içinde deploy-öncesi BOZUK cached plan 24h servis ediliyordu; #939/#940/#942 fix'lerinin "canlıda gecikmeli" görünmesinin gizli sistemik nedeni). Artık prompt/planner sürümü değişince gün-içi cache otomatik MISS → deploy etkisi anında. Detay: [planner-critical-entity-tr-guard](../../wiki/decisions/planner-critical-entity-tr-guard.md) · [planner-cache-key-v2](../../wiki/decisions/planner-cache-key-v2.md).

> 🔧 **Wikipedia knowledge yolu — SEÇ→GETİR→İÇERİK 3 kök (#967/#970/#973, 2026-05-18):** `search_wikipedia` evergreen factual yolu (Layer 2) üç ardışık kök neden ile aynı senaryoda (TR "Yıldız Geçidi SG-1" sorguları) kırılıyordu. **(1) SEÇİM (#967, PR #968):** Wikipedia full-text (`list=search` #824) relevance-ranked döner ama kanonik maddeyi her zaman #1 vermez; `execute_search_wikipedia` `articles[0]`'ı hem #863 sitelink-QID hem ilk `[n]` bloğu temsilci alıyordu → yan sayfa (karakter listesi / "200 (…)" / film) cevaba temel oluyordu. `_prioritize_canonical`: norm-tam-başlık eşleşen kanonik sayfa 3-katmanlı stable sırada `[1]` olur (TR-duyarlı normalize #939; tam-eşleşme yoksa geri uyum). **(2) RETRİEVAL GARANTİSİ (#970, PR #971):** LLM follow-up'ta niteleyicili query üretince ("Yıldız Geçidi SG-1 ilk bölüm kanal") full-text canonical'ı top-3'e HİÇ koymuyor → seçilecek aday yok; `_resolve_canonical` query'yi sağdan kademeli kısaltıp (bounded ≤3, Redis 24h cache) hedefli arama ile canonical'ı kümeye katar. Ayrıca SYSTEM_PROMPT_NODRAT_AGENT rule 4 "Takip sorusu tuzağı (C1)": context'te varlık net olsa bile yeni olgusal boyut (yıl/sezon/kanal) = yeni olgu → tool zorunlu, "meşhur/akıcı" bellek-gerekçesi değil (prompt-katmanı, #955/#964 deseni). **(3) İÇERİK DERİNLİĞİ (#973, PR #974):** `wikipedia.py._fetch_summary` REST `/api/rest_v1/page/summary` = yalnız lead/giriş (~333 char) çekiyordu → doğru kanonik sayfa SEÇİLİP GETİRİLSE bile gövde-içi olgu ("Türkiye'de ilk bölümü TRT 1 / 14 Nisan 2007") lead'de olmadığı için görünmüyordu (C1 gereği dürüstçe "kaynakta yok" — veri yapay kırpık). Fix: `action=query&prop=extracts&explaintext=1` tam makale düz-metin, `_WIKI_EXTRACT_CAP=8000` char cap (paragraf sınırında kes), `CACHE_KEY_VERSION` v1→v2 (eski lead-only Redis girdileri 24h stale kalmasın — #947 versiyon-in-key dersi). Lead→full süperset (bilgi kaybı yok). Üçü de **LLM tool spec & query DEĞİŞTİRMEZ** (deterministik kod / prompt-housekeeping; #842/#863 ailesi). Genel ders: bir retrieval'da "hangi belge" (seçim) + "belge geldi mi" (retrieval) + "belgenin ne kadarı" (içerik derinliği) ayrı kök nedenlerdir; doğrulama smoke'u hangi katmanı ölçtüğünü bilmeli (#970 smoke yalnız seçimi doğrulayıp içerik kökünü kaçırmıştı). Detay: [llm-tool-use-wikipedia](../../wiki/decisions/llm-tool-use-wikipedia.md) · [wikipedia-provider](../../wiki/entities/wikipedia-provider.md) · [chat-knowledge-evolution](../../wiki/topics/chat-knowledge-evolution.md) (ders #35-#37).

> 🔧 **#927 niche-entity recall — 4-faz epic (2026-05-18):** Türkçe entity yüzey-form varyasyonu (çekim eki "Özel'in", eş-ad "ABD↔Amerika", büyük-harf tuzağı "İzmir") substring/dense recall'da kaçıyor → doğru/taze haber elenip eski prototipik haber öne çıkıyor (niche_007/009 kökü). #939/#942/#945/#947 KISMEN çözdü; kod-doğrulamayla 3 kalan kök → 4 bağımsız faz, her biri ayrı PR + revert + kill-switch, risk-artan. **Faz-A (PR #985, ✅ deployed):** agenda-card sparse path (`retrieval.py:878-880` `title/summary/canon_norm_sql`) `LOWER(<quote-strip> COLLATE "tr-TR-x-icu")` (#939 RESCUE pattern birebir; Python param/RRF/parent-doc DEĞİŞMEZ). V2 benchmark agenda-card'ı ölçmez (`niche_chunks_benchmark_v2` yalnız `hybrid_search_chunks`) → prod-trace mechanism smoke: canlı prod DB `LOWER(title) LIKE '%özel%'`=39 → `COLLATE "tr-TR-x-icu"`=132 (+93); `%çin%` 746→905. **Faz-B (PR #989 → REVERT #992, ❌ GATE FAIL):** `meta_norm` + keyword path aynı collation; 2-koşum BEFORE/AFTER V2 prod-parity: recall@5 0.727→**0.636**, niche_003 stabil top-3→stabil 9-10 (tutarlı, yapısal), **sıfır upside** (niche_007/009 yine NF, recall@10 0.818 sabit). Kök: collation RRF-scoring sparse(K=60)+keyword(K=15-30) stream'ini rakip-Türkçe-chunk'la doldurdu → niche_003 hedefi geri itildi (R-B materyalize). Revert+baseline-restore doğrulandı (niche_003 rank=3). **Ders: `COLLATE` RESCUE/FILTER+agenda'da güvenli ama RRF-scoring stream'inde net-negatif** (güvenlik stream-bağlamına özgü). Baseline ölçüldü (V2 prod-parity recall@10=0.818, wiki'deki #939→0.909 iddiası uyuşmuyor — D1). Detay: [failed-experiments-rag-quality](../../wiki/topics/failed-experiments-rag-quality.md) tablo #5. Faz-B-bis / Kademe-1(b)(c) DÜŞÜRÜLDÜ (#988, FILTER zaten #939-collation'lı). **Faz-C (#997 → ❌ NEGATİF spike, CLOSED not-planned, MERGE EDİLMEDİ):** entity eş-ad genişletme (elle sözlük DEĞİL, Wikidata `wbsearchentities→wbgetentities labels\|aliases`, #863 zinciri reuse; planner backstop-sonrası `entity_synonyms` → RESCUE/FILTER OR-term; `retrieval.entity_alias_expansion_enabled` default-OFF flag-spike). **GATE:** flag-OFF ×2 == baseline (recall@5=0.727 recall@10=0.818 NF={007,009}) → kanıtlanabilir no-op (14 unit + 175 komşu regression pass); flag-ON ×2 **İDENTİK** (Δ=0.000, latency bütçe-içi ~43-45s). Sıfır upside + sıfır regresyon. Kök (çözülen synonym dökümü): mekanizma çalıştı (alias'lar doğru) ama 2 NF eş-ad-substring DEĞİL — niche_009 çıplak `15 temmuz`→Wikidata yanlış-anlam (takvim Q-ID); niche_007 alias doğru ama single-chunk RRF-ranking/answer-extraction. **Ders: çıplak TR haber-olay keyword→`wbsearchentities` ilk-QID disambiguation güvenilmez; eş-ad ihtiyacı zaten #863/#967/#973 Wikipedia-yolu ile chat'te karşılanıyor.** Branch `fix/997-wikidata-alias-expansion` (`093fe92`) merge edilmedi (Faz-B precedent — main'de ölü kod yok). Detay: failed-experiments tablo #6. **Faz-D ⏭️ ATLANDI (kanıt-yönlü pivot, 2026-05-18):** Snowball stemmer aynı entity-match katmanı; #947 DAR `_STEM_SUFFIXES` yaygın çekimleri zaten karşılıyor + zaten test-kilitli (`test_query_planner_prompt.py:496-518`). Faz-B+C ampirik: aynı-sınıf müdahale bu 2 NF'yi oynatmıyor → koşmaya değmez. **#927 EPIC CLOSED/COMPLETED** — entity-match-katmanı tükendi; #939+Faz-A gerçek collation kazanımı kalıcı, baseline kilit recall@5=0.727/recall@10=0.818 (D1). niche_007/009 → ayrı chunking/answer-extraction epic'i (kanıtlanmış farklı kök, recall-retrieval değil). Kanonik: [turkish-collation-entity-match](../../wiki/decisions/turkish-collation-entity-match.md) · [planner-critical-entity-tr-guard](../../wiki/decisions/planner-critical-entity-tr-guard.md) · plan dosyası `nerdi-in-ekilde-faz-2-unified-nebula.md`.
