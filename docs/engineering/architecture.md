...
> 🔧 **#927 niche-entity recall — 4-faz epic (2026-05-18):** Türkçe entity yüzey-form varyasyonu (çekim eki "Özel'in", eş-ad "ABD↔Amerika", büyük-harf tuzağı "İzmir") substring/dense recall'da kaçıyor → doğru/taze haber elenip eski prototipik haber öne çıkıyor (niche_007/009 kökü). #939/#942/#945/#947 KISMEN çözdü; kod-doğrulamayla 3 kalan kök → 4 bağımsız faz, her biri ayrı PR + revert + kill-switch, risk-artan. **Faz-A (PR #985, ✅ deployed):** agenda-card sparse path (`retrieval.py:878-880` `title/summary/canon_norm_sql`) `LOWER(<quote-strip> COLLATE "tr-TR-x-icu")` (#939 RESCUE pattern birebir; Python param/RRF/parent-doc DEĞİŞMEZ). V2 benchmark agenda-card'ı ölçmez (`niche_chunks_benchmark_v2` yalnız `hybrid_search_chunks`) → prod-trace mechanism smoke: canlı prod DB `LOWER(title) LIKE '%özel%'`=39 → `COLLATE "tr-TR-x-icu"`=132 (+93); `%çin%` 746→905. **Faz-B (planlı):** `meta_norm` (1225) + keyword path (1543-1575) aynı collation; V2 before/after regresyon kapısı (epic-root V2 prod-parity baseline: recall@5 **0.636** / recall@10 **0.818** / NF=niche_007,009 — ölçüldü, memory'nin 0.909 iddiası yanlış). **Faz-C (planlı):** entity eş-ad genişletme — elle sözlük DEĞİL, Wikidata alias (topluluk-kürate KB; #863 `wbgetentities` altyapısı reuse; backstop-sonrası yeni `entity_synonyms` alanı → RESCUE/FILTER OR-term; flag-OFF spike + latency-gate + graceful-fallback). **Faz-D (planlı, spike):** gerçek TR stemmer (`snowballstemmer`) flag-OFF + #947 over-stem invariant pytest gate (merge-bloklayıcı); negatifse kod-OFF dökümante. Kanonik: [turkish-collation-entity-match](../../wiki/decisions/turkish-collation-entity-match.md) · [planner-critical-entity-tr-guard](../../wiki/decisions/planner-critical-entity-tr-guard.md) · plan dosyası `nerdi-in-ekilde-faz-2-unified-nebula.md`.
