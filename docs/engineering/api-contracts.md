...
| Parametre | Değerler | Varsayılan | Not |
|---|---|---|---|
| `search` | string | — | `canonical_normalized` **VEYA** bağlı bir `entity_aliases.alias_normalized` `ILIKE %q%` (#1558, EXISTS) → "chp" araması "Cumhuriyet Halk Partisi"yi de getirir. Yönetim UI'ı hem ana liste hem detay-diyalog merge-aday seçicisi için bu (sunucu-taraflı, alias-aware) aramayı kullanır (#1562). |
...
| `artifact` | `{artifact_id, cluster_id, cluster_name[, secondary_clusters]}` | (#1678+) Küme-bağlı artefakt oluştu (flag `artifacts.enabled`; cevap kalıcı kart). Küme CEVAP-tarafından çözülür (#1751: cevabın atıf yaptığı kaynakların cevapta adı geçen baskın entity'si). Çapa-entity'siz sorgu → event YOK. **0-cited yanıt** (clarification/honest-refusal, `sources_used` boş) → artefakt+küme OLUŞMAZ, event YOK (#1754; kart yalnız kaynak-atıflı yanıtta). **#1762 (Faz 2, flag `artifacts.multi_cluster.enabled` default OFF):** flag ON ise `secondary_clusters: [{cluster_id, cluster_name}]` — artefaktın ait olduğu ikincil kümeler (cevapta adı geçen diğer entity'ler; abonelik YALNIZ birincil). Veri akışı: kart history re-fetch'ten render edilir → `message.secondary_clusters` (data-model §13b.4) |
