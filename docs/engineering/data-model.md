...
### 6.1b.1 `wikidata_entity_resolutions` — Wikidata canonical-etiket zenginleştirme (#1710/#1714/#1720)
...
- **`wikidata_entity_resolutions`** (raw-SQL-only, additive, migration `20260621_0100`): `entity_normalized`
  + `entity_type` PK · `status` (resolved|no_match|type_mismatch|**llm_reject**) · `wikidata_qid VARCHAR(24)` ·
...
- **Faz 4 — canonical-katman merge + LLM precision gate (#1720/#1721/#1722):** entity-df taraması (Pass 1)
  düşük-df varyantları AGGREGATE eden **token_subset/seed canonical'larını** kaçırır (ör. "15-16 Haziran
  Direnişi"+"…Büyük İşçi Direnişi" tek tek df<min_freq, ama agregat Wikipedia'da: "15-16 Haziran olayları").
  Pass 2 `_enrich_canonical_layer` bunları `canonical_name`'den çözer → wikidata canonical (W) upsert; aynı
  normalized=yerinde-yükseltme, farklı=**merge** (alias'lar W'ye re-point + orphan token_subset/seed canonical
  **DELETE** — admin liste status filtrelemez; `research_clusters.canonical_entity_id` FK SET NULL; cluster
  retro-fit gerekmez — Faz 2 resolver yeni sorguda W'ye bağlar). **LLM precision gate** (#1721): full-text arama
  event/prosedür adlarında ~%50 konu-kayması üretiyor (token-örtüşmesi hem akronimi YKS↔Yükseköğretim Kurumları
  Sınavı hem fuzzy'yi bozar → deterministik ayrılamaz) → tip-gate sonrası tek **v4-flash** doğrulama (`_llm_
  confirm_same_entity`: "Wikipedia maddesi varlığın TAM karşılığı mı? EVET/HAYIR"; HAYIR/hata→`llm_reject`,
  merge yok), cost-log op=`wikidata_verify`. Worker registry bootstrap (#1722) gate'i çalışır kılar. Flag
