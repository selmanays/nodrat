...
sources:
  ...
  - "GitHub #1729/#1730 (Faz 4c entity-pass LLM gate + drift alias re-verify temizliği)"
...
**Faz 4c — entity-pass LLM gate + drift alias temizliği (#1729/#1730, ✅ CANLI 2026-06-22):**
- **Sorun (founder admin'de fark etti):** "kemal irmak" (ayrı kişi, df=42) "Mustafa Kemal Atatürk" altına alias düşmüştü. **Kök:** enrich **entity-pass (Pass 1) LLM precision gate'ten YOKSUNDU** — #1721 gate'i bilerek yalnız canonical-katman Pass 2'ye koymuştu (scope disiplini). Pass 1 yüzey formlarını gate'siz çözer → full-text drift ("kemal irmak" araması Atatürk maddesine gider) + zayıf person tip-gate (yalnız "insan mı?") → farklı kişi ünlüye bağlanır. Guard doğruladı: kemal irmak → Q5152 resolved.
- **#1729:** Pass 1 `_resolve_one` çağrısına da `verifier=_llm_confirm_same_entity` (Pass 2 ile aynı). **Invariant: HER Wikipedia-resolution yolu (Pass 1 + Pass 2) LLM gate'ten geçer.** Person tip-gate "herhangi bir insan"ı kabul ettiği için drift'e en açık katman.
- **#1730 — cleanup:** `_reverify_wikidata_aliases` (standalone, beat'e bağlı DEĞİL): yüzey-formundan çözülmüş wikidata alias'larını `_resolve_one`'la **yeniden çözer** (özet'li/bağlamsal gate) — AYNI canonical → koru; FARKLI-geçerli → doğru W'ye **re-point**; çözülemez/red → **sil** + guard llm_reject. İlk context-free tasarım (alias vs ad, özet yok) fazla agresifti — dry-run'da 70 aday, ~8 **yanlış-red** (KESK→Kamu Emekçileri Sendikaları Konfederasyonu, dersim→Tunceli, rams park→Ali Sami Yen Spor Kompleksi: akronim/tarihsel/sponsor-ad bağlam olmadan reddedildi). Yeniden-çözümleme bunları KORUR. **dry-run-önce zorunlu.**
- **Prod cleanup (founder onaylı, dry-run→review→run):** 325 alias → **43 sil (drift) + 20 re-point (yanlış-eşleme)**. kemal irmak→Atatürk SİLİNDİ (Atatürk 20→19, kalan hepsi meşru) ✓; bist 100→Borsa İstanbul / mit→Millî İstihbarat Teşkilatı / amasya·sivas→doğru il re-point ✓; KESK/Dersim korundu ✓. Kalan ~2-3 akronim alias kaybı (ti̇p→TİP, kısa akronim re-resolve'da bulunamıyor) kabul edildi (veri bozmaz).
- **Genel ders:** tek bir resolution-yolunu gate'lemek YETMEZ — drift TÜM Wikipedia-resolution çağrılarında olur; person tip-gate özellikle zayıf. Tarihsel hasarı temizlerken **context-free LLM karşılaştırma yetersiz** (akronim/tarihsel-ad bağlam ister) → **yeniden-çözümleme** (özet'li) gerekir.
