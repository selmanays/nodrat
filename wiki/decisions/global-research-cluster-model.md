...
- **Prod rebuild** (kullanıcı-onaylı; eski küme+membership sil → assigner re-run 110 mesaj → hierarchy): #1590→27, **#1594→17 aktif küme**; number/money + low-evidence gürültü (zaman/elektrik/bugün/dönüşüm/gram/alan/**hürmüz-fragment**) GİTTİ; `Donald Trump`/`Cumhuriyet Halk Partisi`/`Merkez Bankası`/`Özgür Özel` canonical-birleşik; gerçek yerler (türkiye/abd/küba) kaldı. Kalan "var"(org df5/4kaynak) = **tutarlı mis-NER** (gate'i geçecek kadar etiketli; trends de gate-pass eder ama hacimle gömer) → NER-kalite tail'i (ayrı iş).
