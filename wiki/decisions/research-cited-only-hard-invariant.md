...
sources:
  ...
  - "PR #1068 (#1067 RC3 v1 — LLM-verifier; prod'da 4/8 yanlış-pozitif olunca #1076/PR #1077 v2 ile yenilendi)"
  - "PR #1077 (#1076 RC3-B v2 — yapısal marker-detect; LLM-verifier silindi)"
  - "apps/api/app/api/app_research_stream.py (_is_substantive, C1 backstop, hard-refuse, Fix B′, _RECONSTRUCTION_MARKER_RE, _has_reconstruction_marker — v1 LLM-verifier silindi #1077)"
...
- **RC3-B v2 — yapısal marker-detect (#1076 KALICI; v1 LLM-verifier yanlış-pozitif olduğu için yenilendi):** `_RECONSTRUCTION_MARKER_RE` + `_has_reconstruction_marker` (saf, AST-test edilebilir) — cevap "anlaşıldığı kadarıyla / tepkisinden anlaşıl / olduğu anlaşılıyor / tepkisine bakılırsa / anlaşıldığına göre / yansıdığı kadarıyla / olduğu sanılıyor / muhtemelen X demiş" gibi geriye-çıkarsama imleçleri içeriyorsa → reframe + `faithfulness_reframed` step ([[research-retrieval-transparency]]; RC2 telemetri kancası `reconstruction_marker`). #1058 ile karşılıklı dışlayan (`not all_sources` vs `all_sources`). Cheap (LLM call YOK), deterministik, calibration-stable.
- Flag `research.faithfulness_guard_enabled` default **True** (escape-hatch, #1058/#854 deseni); flag-off byte-eş; cevap-çekirdeği DOKUNULMADI.

> 🔧 **v1 → v2 DÜRÜST REVİZE (#1076, kullanıcı prod'da yakaladı):** İlk Hibrit C tasarımı (v1) cheap-tier LLM-verifier (`_verify_primary_grounding`+`_parse_faithfulness_verdict`+`_FAITHFULNESS_VERIFIER_PROMPT`) kullanıyordu — "ana iddia kaynak metinde DOĞRUDAN destekleniyor mu" sorusuyla DIRECT/INDIRECT/UNSUPPORTED dönüyordu. **Prod son 90dk: 4/8 yanlış-pozitif** (kullanıcı yakaladı): agenda ("Bugünkü gündemde ne var?", 10 source), aggregate ("Özel dün neler yaptı", 20 source), topic-partial ("Çocukların bahis çalışması var mı", 8 source — Gürlek doğrudan cevap), single-direct ("Özel bugün neler yaptı", 1 doğrudan source) — hepsi yanlış INDIRECT → kullanıcılar gerçek-grounded cevap yerine reframe gördü. **Kök:** LLM-verifier "tek-ana-iddia" varsayımıyla çalışıyor; multi-claim/aggregate/topic-partial sınıflarında uyumsuz; NLP-faithfulness LLM-only judgment kanıtlı calibration-fragile (literatür; daha fazla prompt-tweaking Goodhart-law). **Kritik bulgu:** 4 yanlış-pozitifin **hiçbirinde** rekonstrüksiyon-marker'ı yoktu; Özel/Çelik orijinalinde "anlaşıldığı kadarıyla" VARDI — marker-detect doğru sinyali kullanıyor. v2 (deterministik regex) ile yer değiştirdi; **prod-kanıt:** 4 yanlış-pozitif sorgu yeniden Playwright = `reframed=false` hepsi (`031ba46a`/`16226b20`/`98098a80`/`8f08dbeb` — hepsi gerçek grounded cevap aldı); 5. test (Özel/Kocaeli reconstruction-risk) LLM bu sefer RC3-A prompt'una sadık kaldı (marker üretmedi, `anma≠tanım` davranışı sergileyerek "Özel'in iddiası oldu; Çelik 'baştan aşağı yanlış' dedi" → doğru); AST proof 13/13 (6 reconstruction varyantı yakalanır). API eval golden-set yeşil; cevap-çekirdeği invariant. **Ders:** observability/grounding sinyalleri deterministik/yapısal olmalı; genel-amaçlı LLM-temelli faithfulness judgment'lar (entailment-style) calibration-fragile (literatür bilinir; NLI-fine-tune veya structural rule gerekir; mimarimizde yapısal regex seçildi — kullanıcı-doğrulamasının kritikliği).
