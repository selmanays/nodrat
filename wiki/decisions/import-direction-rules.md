...
- **Extraction primitives (P4 PR-D1, 2026-05-23)**: HTML/content parsing primitive'leri (`extract_article`, `extract_with_trafilatura`, `extract_with_selectors`, `extract_fallback`, `extract_listing_cards`, `extract_structured_tier`, `extract_body_images`, `ExtractedArticle`/`ListingCard`/`BodyImage` dataclass'ları + `_filters` + `structured_data`) **`app/shared/extraction/` altında yaşar** (Seviye 0; pure functions; I/O yok). Tüm modüller (kernel articles/sources dahil) doğrudan `from app.shared.extraction import ...` ile import eder; **`modules/crawler/extractor/`'a değil** — kernel→crawler "Forbidden imports" kuralı (contract 2/3) korunur. Crawler (Seviye 3) "extraction cascade *orchestration*" sorumluluğunda kalır; gerekirse `shared.extraction` primitive'lerini *kendi* kullanır. Karar: [[modular-monolith-boundary]] §"Karar notu (2026-05-23): Extraction primitives → shared/extraction/". Code move PR-D2'de uygulanacak (PR-D1 yalnız docs).
- **Direction note (tablo okuma)**: "Allowed imports" tablosundaki `crawler | articles, sources, shared/*` satırı *crawler'ın* allowed *outgoing* yönüdür (orta→kernel/shared = izinli, "yukarıdan aşağı"). "Forbidden imports"taki `articles | ... crawler` ve `sources | ... crawler` satırları *kernel'in* outgoing yönüdür (kernel→orta = yasak, "aşağıdan yukarı"). Tablolar tutarlı; aynı çift modülün her iki yönü ayrı satırlardadır.
