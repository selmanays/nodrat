...
> **#927 Faz-A (2026-05-18, PR #985) — agenda-card sparse path:** `retrieval.py:878-880` `title_norm_sql`/`summary_norm_sql`/`canon_norm_sql` → `LOWER(<quote-strip> COLLATE "tr-TR-x-icu")` (#939 RESCUE pattern birebir; `_build_sql_quote_strip` korunur, Python `:q`/`:phrase` zaten `.lower()` → değişmez; RRF/similarity/parent-doc DEĞİŞMEZ). V2 benchmark agenda-card'ı ölçmez (kod-doğrulandı: `niche_chunks_benchmark_v2` yalnız `hybrid_search_chunks` — D2) → **prod-trace mechanism smoke**: canlı prod DB `LOWER(title) LIKE '%özel%'` = **39** vs `LOWER(title COLLATE "tr-TR-x-icu")` = **132** (+93); `%çin%` 746→905 (+159) → Türkçe-uppercase entity'ler artık agenda-card'da görünür. 86 retrieval pytest yeşil. **Faz-A KALICI.**

> **#927 Faz-B (2026-05-18, PR #989 → REVERT #992) — meta_norm + keyword GATE FAIL:** Aynı `COLLATE` pattern'i sparse `meta_norm` + keyword path'e taşındı ama bunlar **RRF-scoring stream'i** (sparse K=60 + keyword K=15-30) besliyor → C-locale'de kaçan Türkçe-uppercase rakip chunk'lar RRF'e doldu, niche_003 stabil top-3 (2,3) → 9-10'a düştü, recall@5 0.727→0.636 (2-koşum AFTER tutarlı, yapısal); **sıfır upside** (niche_007/009 yine NF, recall@10 sabit 0.818). Revert + baseline-restore doğrulandı (niche_003 rank=3). **Ders: `COLLATE` RESCUE/FILTER + agenda'da güvenli (ayrı stream / sadece-daraltma) ama RRF-scoring sparse+keyword stream'inde net-negatif.** Detay: [[failed-experiments-rag-quality]] tablo #5. Kalan #927: Faz-B-bis (FILTER-only, kazanım belirsiz — Faz-C asıl lever), Wikidata-alias (Faz-C), stemmer-spike (Faz-D).
