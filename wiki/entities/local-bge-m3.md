...
> **TL;DR:** Nodrat'ın **production primary embedding provider'ı** (2026-05-06 itibarıyla). `BAAI/bge-m3` modelini sentence-transformers ile **VPS CPU üzerinde** çalıştırır (1024-dim). Tüm `article_chunks` ve `agenda_cards` embedding'leri buradan üretilir. Tek embedding provider — NIM bağımlılığı yok. Maliyet $0/1M token (VPS CPU compute).
...
- [apps/api/app/api/admin_settings.py:257](../../apps/api/app/api/admin_settings.py) — 
...
- [apps/api/app/providers/registry.py:80](../../apps/api/app/providers/registry.py) — `_fallback("local_bge_m3")` routing (tek provider)
