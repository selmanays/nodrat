...
- [[query-class-classification|query_class — 4-sınıf kullanıcı sorgu sınıflandırma]] — Query Planner output yeni field: `news_query | general_knowledge | meta_query | mixed`. Mevcut `intent` (content-generation) ile karıştırılmamalı. 8 few-shot örnek planner prompt'a inject. Confidence Router'a omurga. PR [#810](https://github.com/selmanays/nodrat/pull/810).
- [[retrieval-confidence-score|Retrieval Confidence Score — 5-signal fusion]] — semantic + source_count + recency + entity_match + citation_density ağırlıklı fusion (default `0.40/0.20/0.15/0.15/0.10`). Score 0-1 → routing decision. PR [#810](https://github.com/selmanays/nodrat/pull/810).
...
- [[tiered-knowledge-architecture|Tiered Knowledge Architecture (Faz 2)]] — Chat 3 katmana ayrıldı: Layer 1 (haber, mevcut moat), Layer 2 (Wikipedia + Wikidata, opsiyonel fallback), Layer 3 (conversation memory). Confidence Router (5-signal score) hangi katmanın tetikleneceğini belirler. LLM kendi bilgi haznesinden ASLA cevap yok (C1). PR [#810/#812/#814/#816](https://github.com/selmanays/nodrat/issues/808).
- [[confidence-based-routing|Confidence Router — 5-signal score fusion]] — Retrieval kalitesi 0-1 skoruna; T_high (0.70) → STRICT, T_low (0.40) → Wikipedia CTA, arası → hybrid+banner. Ağırlıklar single JSON setting (`retrieval.confidence_weights`). PR [#810](https://github.com/selmanays/nodrat/pull/810).
- [[wikipedia-fallback-controlled|Wikipedia fallback CONTROLLED]] — Layer 1 yetersizse Wikipedia kaynaklı cevap; ama kullanıcı CTA onayı zorunlu. CC BY-SA 4.0 + 25-kelime cap + `[W1]` citation. PR [#812](https://github.com/selmanays/nodrat/pull/812) + [#814](https://github.com/selmanays/nodrat/pull/814).
- [[news-first-strict-contamination-guard|News-first STRICT — Wikipedia leak engelleme]] — `query_class='news_query'` hard gate, Wikipedia ASLA tetiklenmez. Brand contamination koruma. Telemetry log ile invariant doğrulanır. PR [#816](https://github.com/selmanays/nodrat/pull/816).
...
- Açık locked decision: **22** (#440 sonrası 2 + Epic #448 sonrası 1 + 2026-05-09: shadcn-customization-policy + sse-streaming-default + 2026-05-10: realtime-rss-polling + own-slm-strategy + 2026-05-11: age-gate-18-plus + model-improvement-default-opt-out + pre-launch-security-checklist + margin-70-target + geographic-pricing-policy + 2026-05-14 #800: chat-only-migration + sft-message-source + dpo-rejected-samples)
