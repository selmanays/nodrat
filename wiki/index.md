...
- [[cross-encoder-rerank-disabled|Cross-encoder reranker production'da kalıcı kapalı (3 model eval-confirmed)]] — `retrieval.cross_encoder_enabled=false` (#760 default OFF, 2026-05-13). 3 model fail: off baseline NDCG@10=0.627 / recall@5=0.727 (8/11), local bge-reranker 0.509 / 0.636 (#750), NIM rerank 0.542 / 0.636 (#750), **Jina v2 base multilingual ~0.547 / 0.636 (#760)** + latency 18s → **73s** (+304%). Karar **kalıcı**; bottleneck katman 5 (rerank) değil katman 2 (retrieval-level — chunk granularity + NER coverage). Provider altyapısı (`local_jina_rerank`) tutulur, setting OFF default. Pipeline RRF + NER (#667) + mode-aware phrase boost (#718) + LLM rerank Faz 4 kombinasyonu yeterli.
