...
| Alan | Değer |
|---|---|
| Aktif faz | **Phase 3** ([#1091](https://github.com/selmanays/nodrat/issues/1091)) — in-progress; **PR 1a + 1b + 2a + 2b merged** (sources kernel + articles kernel aktif); transient `ignore_imports` muafiyeti kaldırıldı; next: embedding migration mini plan |
...
| Aktif tracking | T1 [#1080](https://github.com/selmanays/nodrat/issues/1080), T2 [#1082](https://github.com/selmanays/nodrat/issues/1082), T3 [#1081](https://github.com/selmanays/nodrat/issues/1081), T4 [#1083](https://github.com/selmanays/nodrat/issues/1083), T5 [#1084](https://github.com/selmanays/nodrat/issues/1084), **T6 [#1085](https://github.com/selmanays/nodrat/issues/1085) — closable** (transient `ignore_imports` PR 2a'da kaldırıldı), T7 [#1086](https://github.com/selmanays/nodrat/issues/1086), T8 [#1087](https://github.com/selmanays/nodrat/issues/1087) |
| Son güncelleme | 2026-05-20 — **Phase 3 PR 2a + PR 2b merged + smoke PASS** (main `ed669ed`). Sources & articles kernel modules artık ayrı, decoupled (string-bound Celery dispatch). Transient `ignore_imports` muafiyeti tamamen kaldırıldı; import-linter 12/12 KEPT muafiyetsiz. READ-only active smoke (list + detail) + natural fire OBSERVED (Site 2 decoupling task runtime executed). #1114 docs-only deploy skip empirically VERIFIED (PR #1128 deploy run skipped vs PR #1127/#1130/#1131 deploy proceeded). |
| Bir sonraki adım | **Embedding migration mini plan** (Phase 3 next PR) — `workers/tasks/embedding.py` (1007 LoC) → `modules/embedding/tasks/embedding.py`. PR 2b'de articles → embedding edge zaten kırıldı (A1 deseni), taşıma temiz olacak. Kullanıcı talimatı bekleniyor. |
