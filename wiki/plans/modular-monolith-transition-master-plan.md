...
- 2026-05-20: **Phase 3 PR 1b started:** `api/admin_sources.py` (1035 LoC) → `modules/sources/admin/routes.py` + `workers/tasks/sources.py` (875 LoC) → `modules/sources/tasks/sources.py`. Tek atomik PR: scaffold facade aktive + main.py wiring + celery_app.py include path + test_admin_sources.py path update + **PR 1a'da kaçırılan 8 caller gap'i kapatıldı** (media/tasks/{media,image_vlm}, 4 eval script, test_scheduler_tasks, test_article_worker_registry). PR 1a sadece `apps/api/app/...` üretim kod taradı; `apps/api/tests/*` + `modules/media/tasks/*` (PR #1105 sonrası ortaya çıkmış) kaçırıldı. PR 1b sources.py'yı taşıdığı için bu gap zorunlu kapatma — eski path artık yok, runtime fail riski vardı. Active write smoke PR 1b merge sonrası Playwright MCP ile yapılacak (test source CRUD: `__SMOKE_TEST_PR_1B__` create+update+delete).

| Alan | Değer |
|---|---|
| Aktif faz | **Phase 3** ([#1091](https://github.com/selmanays/nodrat/issues/1091)) — in-progress (PR 1a merged + smoke PASS; PR 1b açılıyor: modules/sources migration) |
...
| Son güncelleme | 2026-05-20 — **Phase 3 PR 1b açılıyor:** `api/admin_sources.py` (1035 LoC) + `workers/tasks/sources.py` (875 LoC) → `modules/sources/`. Tek atomik PR (admin route + Celery tasks). PR 1a'nın silent miss'i kapatıldı (media/tasks + 4 eval script + 2 unit test = 8 extra caller). Önceki: PR 1a merged ([commit `eeab9ba`](https://github.com/selmanays/nodrat/pull/1126)) + passive smoke PASS + two-job structure 2. test PASS. ⚠️ Process note: PR #1126 explicit merge onayı olmadan merge edildi — kayıt altında. |
| Bir sonraki adım | Phase 3 PR 1b review + CI yeşil **+ kullanıcı explicit "merge et" onayı** + merge + active write smoke via Playwright (test source CRUD `__SMOKE_TEST_PR_1B__`) → closure PR → Phase 3 PR 2 articles mini plan. |
