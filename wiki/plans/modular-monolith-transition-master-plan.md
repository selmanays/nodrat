...
| Alan | Değer |
|---|---|
| Aktif faz | **Phase 4/5/6 paralel** — P4 T6: char (#1144) + facade scaffold (#1146, boundary açık) + internal split (#1147); P5 T6: char (#1148) + phrase+vector split (#1149) + scoring split (#1152); P6 T6: char (#1150) + pure-helper split (#1153) + async helper char (#1155) + **heavy mock async char (`_generate_followups`, #1157)**; P5+P6 worker mini-cycle done; `modules/generations` + `modules/rag` + `modules/crawler` Phase 1 scaffold → active facade; **workers/tasks/ artık yalnız `__init__.py`**; import-linter **13 contracts, 0 broken** muafiyetsiz |
| Bekleyen | P4 PR-D caller flip (boundary kararına bağlı; PR #1146 açık sorular); P6 PR-A2b heavy mock async helper (`_tracked_chat_generate` — ctx manager + telemetry factory; tek kalan); P6 PR-C+ replay tests + full SSE integration; P7a frontend api.ts ([#1095](https://github.com/selmanays/nodrat/issues/1095)); P7b frontend rag-page ([#1096](https://github.com/selmanays/nodrat/issues/1096)); P8 boundary hardening ([#1097](https://github.com/selmanays/nodrat/issues/1097)); N+1 model relocation ([#1098](https://github.com/selmanays/nodrat/issues/1098)) — blocked by T8 |
| Tamamlanan | **P0/1/2** kapandı (#1088/89/90); **P3** ana cycle (6 PR); **P5 mini-cycle** worker (#1143); **P6 mini-cycle** worker (#1140-#1142); **P4 god-file T6 PR-A/B/C** (#1144 + #1146 + #1147); **P5 god-file T6 PR-A/B/C** (#1148 + #1149 + #1152); **P6 god-file T6 PR-A/B/A1/A2a** (#1150 + #1153 + #1155 + #1157); **4 closure docs** (#1145 + #1151 + #1154 + #1156) |
| Aktif tracking | T1-T5 (#1080-#1084), **T6 [#1085](https://github.com/selmanays/nodrat/issues/1085) — OPEN** (P4: 3 PR ✅ + caller flip ertelendi; P5: 3 PR ✅; P6: 4 PR ✅ (char + split + async char + heavy mock async `_generate_followups`) + 1 heavy mock async (`_tracked_chat_generate` PR-A2b) + replay pending; P7a/b: 0), **T7 [#1086](https://github.com/selmanays/nodrat/issues/1086) — OPEN** (cost_tracker pending), **T8 [#1087](https://github.com/selmanays/nodrat/issues/1087) — OPEN/BLOCKED** (5 ön-koşul) |
| Son güncelleme | 2026-05-21 — **B seçeneği uzun tur 14 PR cumulative** (#1144-#1157). PR #1156 closure docs v4 + PR #1157 P6 PR-A2a heavy mock async helper `_generate_followups` char eklendi. God-file LoC reduction sabit: extractor 1189→1019 (-170; helpers 212), retrieval 2174→1911 (-263; helpers 373), SSE 1440→1406 (-34; helpers 64). **Toplam 84 characterization test** safety-net (extractor 15 + retrieval 25 + SSE pure 18 + SSE async light 17 + SSE async heavy 9). **Veri güvenliği invariant — KORUNDU** (14 PR'da hiç chunk/embedding/index müdahalesi, manual rechunk/reembed/backfill, direct DB/Redis, manual production task trigger, state-changing smoke yok). PR #1157 test-only PR + production behavior değişikliği YOK (image içine unit test girmez). |
...
| Bir sonraki adım | **B seçeneği devamı (kullanıcı 2026-05-21 onayı):** (1) **Bu closure docs v5 PR** (PR #1156 + #1157 cumulative); (2) Sonrasında DUR — PR-A2b (`_tracked_chat_generate` heavy mock), replay tests, Phase 7 frontend, extractor boundary kararına otomatik geçilmez; kullanıcı açık onayı beklenir. |
