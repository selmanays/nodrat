...
| Son güncelleme | 2026-05-20 — Phase 2 PR 8a açılıyor: `app/core/prompts_store.py` → `app/shared/runtime_config/prompts_store.py` (304 LoC, 13 caller / 16 import). 8 guardrail (PR #1112) **ilk gerçek uygulaması**. Önceki: CI/CD ordering #1108 PR #1113 ([commit `3b0013b`](https://github.com/selmanays/nodrat/pull/1113)) merged + empirical PASS (CI→Deploy ordering doğrulandı, head_sha pinning verified, HTTP 200). PR #1112 ([commit `ab08ab1`](https://github.com/selmanays/nodrat/pull/1112)) merged — 8 guardrail + 19/0 audit. |
| Bir sonraki adım | Phase 2 PR 8a review + merge + **passive runtime smoke** (5 worker log scan + singleton identity + Redis NUMSUB + cross-process consistency) → Phase 2 PR 8b: `api/admin_prompts.py` → `modules/prompts_admin/routes.py` (657 LoC, admin route ownership + active write smoke). Sonra Phase 2 closure + #1114 paths-ignore optimization. |
