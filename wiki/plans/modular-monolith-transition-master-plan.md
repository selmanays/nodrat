...
| Alan | Değer |
|---|---|
| Aktif faz | **Phase 4/5/6 paralel** — P4 T6: char (#1144) + facade scaffold (#1146, boundary açık) + internal split (#1147); P5 T6: char (#1148) + phrase+vector split (#1149) + **scoring split (#1152)**; P6 T6: char (#1150) + **pure-helper split (#1153)**; P5+P6 worker mini-cycle done (önceki); `modules/generations` + `modules/rag` + `modules/crawler` Phase 1 scaffold → active facade; **workers/tasks/ artık yalnız `__init__.py`**; import-linter **13 contracts, 0 broken** muafiyetsiz |
| Bekleyen | P4 PR-D caller flip (boundary kararına bağlı; PR #1146 açık sorular); P6 PR-A1 async SSE helpers characterization (mock infra); P6 PR-C+ replay tests; P7a frontend api.ts ([#1095](https://github.com/selmanays/nodrat/issues/1095)); P7b frontend rag-page ([#1096](https://github.com/selmanays/nodrat/issues/1096)); P8 boundary hardening ([#1097](https://github.com/selmanays/nodrat/issues/1097)); N+1 model relocation ([#1098](https://github.com/selmanays/nodrat/issues/1098)) — blocked by T8 |
| Tamamlanan | **P0/1/2** kapandı (#1088/89/90); **P3** ana cycle (sources/articles/embedding/ops; 6 PR); **P5 mini-cycle** worker (raptor #1143); **P6 mini-cycle** worker (agenda #1141, cluster_assigner #1142, A1 decoupling #1140); **P4 god-file T6 PR-A/B/C** (#1144 + #1146 + #1147); **P5 god-file T6 PR-A/B/C** (#1148 + #1149 + #1152); **P6 god-file T6 PR-A/B** (#1150 char + #1153 pure-helper split); **2 closure docs** (#1145, #1151) |
| Aktif tracking | T1 [#1080](https://github.com/selmanays/nodrat/issues/1080), T2 [#1082](https://github.com/selmanays/nodrat/issues/1082), T3 [#1081](https://github.com/selmanays/nodrat/issues/1081), T4 [#1083](https://github.com/selmanays/nodrat/issues/1083), T5 [#1084](https://github.com/selmanays/nodrat/issues/1084), **T6 [#1085](https://github.com/selmanays/nodrat/issues/1085) — OPEN** (P4: 3 PR ✅ + caller flip ertelendi; P5: 3 PR ✅ (char+phrase/vector+scoring); P6: 2 PR ✅ (char+pure-helper split) + async/replay pending; P7a/b: 0), **T7 [#1086](https://github.com/selmanays/nodrat/issues/1086) — OPEN** (settings+prompts P2'de ✅; cost_tracker pending), **T8 [#1087](https://github.com/selmanays/nodrat/issues/1087) — OPEN/BLOCKED** (5 ön-koşul) |
| Son güncelleme | 2026-05-21 — **B seçeneği uzun tur devamı: 3 PR daha merged** (PR #1151 closure docs v1 `2b70bff` + #1152 P5 PR-C `c238f0a` + #1153 P6 PR-B `d72b3fc`). **Toplam 10 PR uzun turda** (#1144-#1153). God-file LoC reduction güncel: extractor 1189 → 1019 (-170; helpers 212), retrieval 2174 → 1911 (-263; helpers 373 = phrase 194 + vector 40 + scoring 139), SSE 1440 → 1406 (-34; helpers 64). 58 characterization test hâlâ safety-net (15 extractor + 25 retrieval + 18 SSE). **Veri güvenliği invariant — KORUNDU** (10 PR'da hiç chunk/embedding/index müdahalesi, manual rechunk/reembed/backfill, direct DB/Redis, state-changing smoke yok). |
...
| Bir sonraki adım | **B seçeneği devamı (kullanıcı 2026-05-21 onayı):** (1) **Bu closure docs v3 PR** (PR #1152 + #1153 cumulative); (2) Phase 6 SSE PR-A1 — async helper characterization (`_resolve_style_block`, `_tracked_chat_generate`, `_generate_followups`, `_recent_conversation_context`); (3) Replay tests / Phase 7 frontend / extractor boundary — DUR, kullanıcı onayı bekle. |
