...
| Alan | Değer |
|---|---|
| Aktif faz | **Phase 4** ([#1092](https://github.com/selmanays/nodrat/issues/1092)) — started (T6 P4 PR-A merged); Phase 5 ([#1093](https://github.com/selmanays/nodrat/issues/1093)) + Phase 6 ([#1094](https://github.com/selmanays/nodrat/issues/1094)) **mini-cycle done** (raptor + agenda + cluster_assigner task migrations); `modules/generations` + `modules/rag` Phase 1 scaffold → **active facade**; **workers/tasks/ artık yalnız `__init__.py` (323 byte)** — 8 task dosyası 5 modüle dağıtıldı; **transient `ignore_imports` muafiyeti yok**; import-linter **13 contracts, 0 broken** muafiyetsiz |
| Bekleyen | P4 god-file split (PR-B/C — `modules/crawler/extractor/` facade + caller flip + internal split), P5 retrieval characterization + facade ([#1093](https://github.com/selmanays/nodrat/issues/1093)), P6 SSE characterization/replay + facade ([#1094](https://github.com/selmanays/nodrat/issues/1094)), P7a frontend api.ts ([#1095](https://github.com/selmanays/nodrat/issues/1095)), P7b frontend rag-page ([#1096](https://github.com/selmanays/nodrat/issues/1096)), P8 boundary hardening ([#1097](https://github.com/selmanays/nodrat/issues/1097)), N+1 model relocation ([#1098](https://github.com/selmanays/nodrat/issues/1098)) — blocked by T8 |
| Tamamlanan | **P0** [#1088](https://github.com/selmanays/nodrat/issues/1088) (merged `72b68c3`), **P1** [#1089](https://github.com/selmanays/nodrat/issues/1089) (merged `5a67e06`), **P2** [#1090](https://github.com/selmanays/nodrat/issues/1090) (closed `09efce1`), **P3** ana cycle (sources/articles/embedding/ops kernel+middle+cross-cutting; 6 PR), **P5 mini-cycle** (raptor PR #1143), **P6 mini-cycle** (agenda PR #1141, cluster_assigner PR #1142, A1 decoupling PR #1140), **P4 T6 PR-A** (extractor characterization PR #1144) |
| Aktif tracking | T1 [#1080](https://github.com/selmanays/nodrat/issues/1080), T2 [#1082](https://github.com/selmanays/nodrat/issues/1082), T3 [#1081](https://github.com/selmanays/nodrat/issues/1081), T4 [#1083](https://github.com/selmanays/nodrat/issues/1083), T5 [#1084](https://github.com/selmanays/nodrat/issues/1084), **T6 [#1085](https://github.com/selmanays/nodrat/issues/1085) — OPEN** (P4 PR-A merged; PR-B facade + caller flip pending; retrieval + SSE + api.ts + rag-page god-file'lar henüz başlamadı), **T7 [#1086](https://github.com/selmanays/nodrat/issues/1086) — OPEN** (settings_store + prompts_store ✅ P2'de; cost_tracker scope analizi pending), **T8 [#1087](https://github.com/selmanays/nodrat/issues/1087) — OPEN/BLOCKED** (5 ön-koşul; read-only prerequisite script pending) |
| Son güncelleme | 2026-05-21 — **Phase 5/6 mini-cycle + P4 T6 PR-A tamamlandı** (5 PR merged: #1140 A1 decoupling `d3ac330`, #1141 agenda `4e4e74c`, #1142 cluster_assigner `ec3ad2c`, #1143 raptor `6dbf378`, #1144 extractor characterization `168acab`). 4 auto-merge gate PASS (CI 10/10 + import-linter 13/13 + 5-form clean + no DB/ignore_imports/state-change + smoke 6/6 PASS). Worker task migration tamamen tamam: `workers/tasks/` artık yalnız `__init__.py`. **Smoke probe disipline dersi** ortaya çıktı (force-load gerekir) — `refactor-pr-checklist §13.3`'e eklendi. **Veri güvenliği invariant — KORUNDU** (5 PR'da hiç chunk/embedding/index müdahalesi, manual rechunk/reembed, direct DB/Redis, state-changing smoke yok). |
| Bir sonraki adım | **B seçeneği master plan sırası (kullanıcı 2026-05-21 onayı):** (1) Bu closure docs PR (PR #1141-#1144 cumulative + §13.3 force-load dersi); (2) Phase 4 PR-B — `modules/crawler/extractor/__init__.py` re-export facade + 3 production caller flip; (3) Phase 4 PR-C — extractor internal split (characterization safety-net üzerinden, küçük); (4) Phase 5 retrieval PR-A characterization tests (2174 LoC `core/retrieval.py`); (5) Phase 5 retrieval facade/split; (6) Phase 6 SSE characterization/replay; (7) Phase 6 SSE facade/split; (8-9) Phase 7a/7b frontend; (10) Phase 8 boundary hardening; (11) N+1 model relocation — yalnız T8 ön-şartları sağlanırsa. |
