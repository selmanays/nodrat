...
| Son güncelleme | 2026-05-20 — **Phase 2 PR 8a merged + passive smoke PASS** ([#1118](https://github.com/selmanays/nodrat/pull/1118), main HEAD `008d6de`). CI/CD ordering empirical PASS (2. başarılı test PR #1113 fix sonrası): CI 12:25:54Z → Deploy 12:29:04Z (workflow_run + head_sha pinning). 11/11 acceptance: import OK, ModuleNotFoundError, singleton, NUMSUB=2, 7 container × 5+ pattern = 0, Beat fire + 2 task succeeded. §9.5 fallback reporting: app_prompts total_rows=0 (DB boş; behavior-preserving). Active write smoke → PR 8b. |
| Bir sonraki adım | Phase 2 PR 8b: `api/admin_prompts.py` (657 LoC) → `modules/prompts_admin/routes.py` (admin route ownership + **active write smoke**: write → same-process read → worker invalidation → restore → logs clean). Sonra Phase 2 closure + [#1114](https://github.com/selmanays/nodrat/issues/1114) paths-ignore optimization. |
