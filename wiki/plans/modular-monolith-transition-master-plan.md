...
| Alan | Değer |
|---|---|
| Aktif faz | **Phase 3** ([#1091](https://github.com/selmanays/nodrat/issues/1091)) — in-progress; **PR 1a + 1b + 2a + 2b + 3 merged** (sources kernel + articles kernel + embedding middle-layer aktif); **transient `ignore_imports` muafiyeti yok**; import-linter **13 contracts, 0 broken** muafiyetsiz; next: Phase 3 next migration candidate için mini plan required (clusters/entities/accounts/billing/ops/public) |
...
| Son güncelleme | 2026-05-21 — **Phase 3 PR 3 merged + PR #1134 CI recovery + smoke PASS** (main `42c4dcd`). Embedding middle-layer aktive (modules/embedding/tasks/embedding.py); workers/tasks/embedding.py GONE; entities.py helper resolution direct paths; SHA pinning 3-way match doğrulandı; veri güvenliği invariant PRESERVED (pre-existing per-article re-chunk DOKUNULMADI). CI recovery: ci.yml workflow_dispatch eklendi (PR #1133 sonrası push:main anomaly recovery yolu); recovery mekanizması test edilmedi çünkü PR #1134 merge sonrası push:main otomatik tetiklendi. |
| Bir sonraki adım | **Phase 3 next migration mini plan required** — sources/articles/embedding tamamlandı; sırada hangi modül (clusters/entities/accounts/billing/ops/public) öncelikli? Kullanıcı kararı bekleniyor. |
