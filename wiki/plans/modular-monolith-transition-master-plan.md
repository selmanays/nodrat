...
- 2026-05-20: **Phase 3 PR 1b merged + smoke PARTIAL PASS / ACCEPTED WITH RESIDUAL** ([#1127](https://github.com/selmanays/nodrat/pull/1127), squash merge `cf07ef9`). Kullanıcı explicit merge onayı alındı. **CI/CD ordering:** CI 10/10 yeşil → Deploy 2-job (detect + deploy-vps) both success, aynı SHA `cf07ef9`. **Passive smoke 8/8 PASS:** yeni path'ler import OK, eski path'ler temiz, Celery registry 6 task, Beat schedule 3 entry, queue routing crawl_queue, 7 container log scan clean. **Active source route smoke CREATE/READ/UPDATE PASS** (Playwright MCP, test source `__SMOKE_TEST_PR_1B__`): CREATE 201 + READ list 28 + UPDATE PATCH 200. **RESTORE/DELETE not completed — admin DELETE endpoint kasıtlı olarak mevcut değil (compliance/legal); production state NOT fully restored, test source residual pasif olarak prod'da kaldı**. Source count 27 → 28 → 28 (beklenen 28 → 27 olmadı). **Process lesson:** Source admin modülünde DELETE yoksa "create then delete" smoke varsayımı yanlış; sources smoke "create + disable + explicit cleanup decision" olarak sınıflandırılmalı. Follow-up cleanup/policy decision: [#1129](https://github.com/selmanays/nodrat/issues/1129). **`ignore_imports` exception aktif** — Phase 3 articles/embedding migration sırasında kaldırılacak ([T6 #1085](https://github.com/selmanays/nodrat/issues/1085) tracking).

| Alan | Değer |
|---|---|
| Aktif faz | **Phase 3** ([#1091](https://github.com/selmanays/nodrat/issues/1091)) — in-progress; PR 1a + PR 1b merged; next: articles mini plan (önce PR #1128 closure doğruluğu + #1129 cleanup policy) |
...
| Son güncelleme | 2026-05-20 — **Phase 3 PR 1b merged + smoke PARTIAL PASS** (`cf07ef9`). Sources modülü migrate. Test source residual (#1129 cleanup follow-up). `ignore_imports` exception aktif (T6 tracking). |
| Bir sonraki adım | (1) PR #1128 closure doğruluğu; (2) #1129 cleanup/policy kararı; (3) Phase 3 PR 2 articles mini plan — `ignore_imports` exception bu PR'da tekrar değerlendirilecek. |
