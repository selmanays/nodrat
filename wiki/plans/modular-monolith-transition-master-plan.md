...
| Alan | Değer |
|---|---|
| Aktif faz | **Phase 4/5/6 paralel** — P4 T6: char (PR #1144) + facade scaffold (PR #1146, boundary açık) + internal split (PR #1147); P5 T6: char (PR #1148) + internal split (PR #1149); P6 T6: char (PR #1150); P5+P6 worker mini-cycle done (önceki); `modules/generations` + `modules/rag` + `modules/crawler` Phase 1 scaffold → active facade; **workers/tasks/ artık yalnız `__init__.py`**; import-linter **13 contracts, 0 broken** muafiyetsiz |
| Bekleyen | P4 PR-D caller flip (boundary kararına bağlı; PR #1146 açık sorular); P5 PR-C scoring helpers split (önerilen); P6 PR-A1 async SSE helpers (mock infra); P6 PR-B internal split; P6 PR-C+ replay tests; P7a frontend api.ts ([#1095](https://github.com/selmanays/nodrat/issues/1095)); P7b frontend rag-page ([#1096](https://github.com/selmanays/nodrat/issues/1096)); P8 boundary hardening ([#1097](https://github.com/selmanays/nodrat/issues/1097)); N+1 model relocation ([#1098](https://github.com/selmanays/nodrat/issues/1098)) — blocked by T8 |
| Tamamlanan | **P0/1/2** kapandı (#1088/89/90); **P3** ana cycle (sources/articles/embedding/ops; 6 PR); **P5 mini-cycle** worker (raptor #1143); **P6 mini-cycle** worker (agenda #1141, cluster_assigner #1142, A1 decoupling #1140); **P4 god-file T6 PR-A/B/C** (#1144 char + #1146 facade scaffold + #1147 internal split); **P5 god-file T6 PR-A/B** (#1148 char + #1149 internal split); **P6 god-file T6 PR-A** (#1150 SSE pure-helper char) |
| Aktif tracking | T1 [#1080](https://github.com/selmanays/nodrat/issues/1080), T2 [#1082](https://github.com/selmanays/nodrat/issues/1082), T3 [#1081](https://github.com/selmanays/nodrat/issues/1081), T4 [#1083](https://github.com/selmanays/nodrat/issues/1083), T5 [#1084](https://github.com/selmanays/nodrat/issues/1084), **T6 [#1085](https://github.com/selmanays/nodrat/issues/1085) — OPEN** (P4: 3 PR ✅ + caller flip ertelendi (boundary açık); P5: 2 PR ✅ + scoring split önerildi; P6: 1 PR ✅ + async/replay pending; P7a/b: 0), **T7 [#1086](https://github.com/selmanays/nodrat/issues/1086) — OPEN** (settings+prompts P2'de ✅; cost_tracker pending), **T8 [#1087](https://github.com/selmanays/nodrat/issues/1087) — OPEN/BLOCKED** (5 ön-koşul) |
| Son güncelleme | 2026-05-21 — **B seçeneği uzun tur: 7 PR merged** (PR #1144 char `168acab` + #1145 closure docs `a6b6e18` + #1146 P4 facade scaffold `69a2d77` + #1147 P4 internal split `062fe9e` + #1148 P5 char `9cccb0d` + #1149 P5 internal split `31de4bb` + #1150 P6 SSE char `5bfac73`). God-file LoC reduction: extractor 1189 → 1019 (-170; helpers 212 ayrı), retrieval 2174 → 1980 (-194; helpers 234 ayrı), SSE 1440 → 1440 (DOKUNULMADI; char only). +58 yeni pure-function characterization test (15 extractor + 25 retrieval + 18 SSE). Bg notification fix: `tail -F | grep -m 1` zombie pattern'i → polling pattern (`while ! grep -q...; do sleep 15`). **Veri güvenliği invariant — KORUNDU** (7 PR'da hiç chunk/embedding/index müdahalesi, manual rechunk/reembed/backfill, direct DB/Redis, state-changing smoke yok). |
| Açık karar maddesi | **Extractor boundary** (PR #1146 closure not): (1) Extractor `modules/crawler/` mi yoksa `shared/extraction/`'a mı? (2) Kernel modülleri (articles, sources) crawler middle layer'ı import edemez (master plan §3.2). (3) 3 caller hangi extractor path'inden import edecek? Phase 4 full migration öncesi karar gerek. |
| Bir sonraki adım | **B seçeneği devamı (kullanıcı 2026-05-21 onayı):** (1) **Bu closure docs PR** (PR #1146-#1150 cumulative); (2) Phase 5 PR-C — retrieval scoring helpers split (`_retrieval_scoring.py`: `freshness_decay`, `compute_final_score`, `WEIGHTS_*`, dataclasses); (3) Phase 6 SSE PR-A1 / replay tests — DUR, kullanıcı onayı bekle (orta riskli, async mock infra gerekli); (4) P4 PR-D caller flip — boundary kararı ÇIKARSA; (5) Phase 7a/7b frontend; (6) Phase 8 boundary hardening; (7) N+1 model relocation. |
