...
| Alan | Değer |
|---|---|
| Aktif faz | **Phase 3** ([#1091](https://github.com/selmanays/nodrat/issues/1091)) — in-progress (PR 1a açılıyor: shared worker DB/session helpers extraction) |
...
| Son güncelleme | 2026-05-20 — **Phase 3 PR 1a açılıyor:** `workers/tasks/sources.py` içindeki shared helper'ları (`_get_session_factory`/`_run_async`/`open_session`) → `app/shared/workers/db_session.py`. 9 caller path update. Behavior-preserving. Sources module taşıması (admin_sources.py + sources tasks) → PR 1b. Önceki: Phase 2 retrospective merged ([commit `356c1e0`](https://github.com/selmanays/nodrat/pull/1123)) + #1122 .gitignore merged ([commit `d56202c`](https://github.com/selmanays/nodrat/pull/1124)) + #1114 docs-only deploy skip merged ([commit `166a9c0`](https://github.com/selmanays/nodrat/pull/1125)) — two-job structure self-merge PASS. |
| Bir sonraki adım | Phase 3 PR 1a review + merge + passive smoke → Phase 3 PR 1b mini plan revize (`api/admin_sources.py` + `workers/tasks/sources.py` → `modules/sources/`) → PR 1b merge + active write smoke → Phase 3 PR 2 articles mini plan. |
