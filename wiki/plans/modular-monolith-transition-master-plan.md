...
| Alan | Değer |
|---|---|
| Aktif faz | **Phase 4/5/6/7a paralel** — P4 T6: char (#1144) + facade scaffold (#1146, boundary açık) + internal split (#1147); P5 T6: char (#1148) + phrase+vector split (#1149) + scoring split (#1152); **P6 T6: 11 PR ✅** (#1150 + #1153 + #1155 + #1157 + #1159 + #1160 + #1162 + #1164 + #1166 + #1168 + #1170); **P7a T6: 12 PR ✅** test infra (#1172) + public (#1173) + disk (#1174) + auth (#1175) + verifyResend mini (#1177) + admin users (#1178) + admin audit (#1180) + admin /system (#1181) + admin media (#1183) + buildQuery shared housekeeping (#1184) + admin legal (#1186) + admin articles (#1187); `api.ts` facade/re-export pattern **10 kez doğrulandı** (public + disk + auth + verifyResend + admin-users + admin-audit + admin-system + admin-media + admin-legal + admin-articles); 60 caller import path DEĞİŞMEDİ; **`apiFetch` + `ApiException` + token storage + `attemptTokenRefresh` core'da KALDI** (P7a hard kural); **`buildQuery` shared `api/_query.ts`** (leaf, internal, re-export edilmez; admin-legal + admin-articles doğrudan tüketti, yeni kopya yok); **`getMyQuota` Articles section dışı, api.ts'te kaldı**; `modules/generations` + `modules/rag` + `modules/crawler` Phase 1 scaffold → active facade; **workers/tasks/ artık yalnız `__init__.py`**; import-linter **13 contracts, 0 broken** muafiyetsiz |
| Bekleyen | P4 PR-D caller flip (boundary kararına bağlı; PR #1146 açık sorular); **P7a PR-7a-12 sırada** (getMyQuota mini-extract → `api/account.ts`); **P7a kalan**: Admin Sources (alt-bölünmeli) / Queue (manual task trigger) / Settings (runtime config) / Account-Me delete/export/update (deleteMe) / RAG (381 LoC); **P7a Research section EN SONA** (691 LoC / 11+ caller; SSE client coupling); P6 PR-C+ full SSE integration replay (TestClient endpoint, full transcript with real research_tools mocks; RC3-B orchestrator coupling + tool-loop timeout deep coverage dahil); P7b frontend rag-page ([#1096](https://github.com/selmanays/nodrat/issues/1096)); P8 boundary hardening ([#1097](https://github.com/selmanays/nodrat/issues/1097)); N+1 model relocation ([#1098](https://github.com/selmanays/nodrat/issues/1098)) — blocked by T8 |
| Tamamlanan | **P0/1/2** kapandı (#1088/89/90); **P3** ana cycle (6 PR); **P5 mini-cycle** worker (#1143); **P6 mini-cycle** worker (#1140-#1142); **P4 god-file T6 PR-A/B/C** (#1144 + #1146 + #1147); **P5 god-file T6 PR-A/B/C** (#1148 + #1149 + #1152); **P6 god-file T6 PR-A/B/A1/A2a/A2b/A3/A4/A5/A6/A7/A8** (#1150 + #1153 + #1155 + #1157 + #1159 + #1160 + #1162 + #1164 + #1166 + #1168 + #1170); **P7a god-file T6 PR-7a-0..11** (#1172 + #1173 + #1174 + #1175 + #1177 + #1178 + #1180 + #1181 + #1183 + #1184 + #1186 + #1187); **15 closure docs** (#1145 + #1151 + #1154 + #1156 + #1158 + #1161 + #1163 + #1165 + #1167 + #1169 + #1171 + #1176 + #1179 + #1182 + #1185) |
| Aktif tracking | T1-T5 (#1080-#1084), **T6 [#1085](https://github.com/selmanays/nodrat/issues/1085) — OPEN** (P4: 3 PR ✅ + caller flip ertelendi; P5: 3 PR ✅; P6: 11 PR ✅ + RC3-B orchestrator coupling + tool-loop timeout + full SSE integration pending → PR-C+; **P7a: 12 PR ✅** (test infra + public + disk + auth + verifyResend + admin-users + admin-audit + admin-system + admin-media + buildQuery housekeeping + admin-legal + admin-articles); PR-7a-12 getMyQuota mini sırada), **T7 [#1086](https://github.com/selmanays/nodrat/issues/1086) — OPEN** (cost_tracker pending), **T8 [#1087](https://github.com/selmanays/nodrat/issues/1087) — OPEN/BLOCKED** (5 ön-koşul). **Phase 6 hâlâ tamamlanmadı** — derin orchestration + endpoint TestClient kalır. **Phase 7a devam ediyor** — Research EN SONA, küçük mini-extract adayları sırada. **SSE replay coverage 10/10 senaryo HEDEF TAMAMLANDI** + 1 bonus. **RC3-B helper-level lock tamam.** |
| Son güncelleme | 2026-05-22 — **B seçeneği uzun tur 39 PR cumulative** (#1144-#1187 + closure docs v16). PR #1185 closure docs v15 (sadece wiki) + PR #1186 P7a PR-7a-10 Legal admin extract (71 LoC → api/admin/legal.ts; +4 char test; 1 state-changing `updateTakedownRequest` smoke-skip; buildQuery shared'ı tüketen ilk extract) + PR #1187 P7a PR-7a-11 Admin Articles extract (149 LoC → api/admin/articles.ts; 11 interface + 1 type + 6 fonksiyon; +7 char test; 1 state-changing `reprocessArticle` smoke-skip; getMyQuota DOKUNULMADI) eklendi. **Phase 7a 12 PR DONE.** **Frontend characterization 44 test** (5 + 2 + 2 + 4 + 3 + 6 + 4 + 3 + 4 + 0 + 4 + 7 cumulative). God-file LoC reduction sabit: extractor 1189→1019, retrieval 2174→1911, SSE 1440→1406; `api.ts` 2041→1559 LoC (PR-7a-11 sonrası -482 net). **Toplam 168 characterization test** safety-net (backend 124 + frontend 44). **Veri güvenliği invariant — KORUNDU** (39 PR'da hiç chunk/embedding/index müdahalesi, manual rechunk/reembed/backfill, direct DB/Redis, manual production task trigger, state-changing smoke yok; **PR #1186 `updateTakedownRequest` + PR #1187 `reprocessArticle` production'a YOLLANMADI**). PR #1186 + #1187 production behavior değişikliği YOK (endpoint + path + method + body özdeş; caller import path DEĞİŞMEDİ). **`apiFetch` + `ApiException` + token storage + `attemptTokenRefresh` core'da KALDI.** **`getMyQuota` + `QuotaResponse` Articles PR'da dokunulmadan api.ts'te kaldı (PR-7a-12 mini-extract adayı). buildQuery shared `_query.ts` admin-legal + admin-articles tarafından doğrudan tüketildi (yeni kopya yok).** |
...
| Bir sonraki adım | **B seçeneği devamı (kullanıcı 2026-05-22 onayı):** (1) **Bu closure docs v16 PR** (PR #1186 + #1187 cumulative); (2) **PR-7a-12 getMyQuota mini-extract** — scope doğrulama + implementation (`QuotaResponse` + `getMyQuota` → `api/account.ts`; read-only `/app/quota`; user-facing); (3) Sonrasında DUR — PR-7a-N kalan extract'lar (Admin Sources alt-bölünmeli / Queue / Settings / Account-Me delete/export/update / RAG), Phase 6 PR-C+ (full SSE integration / RC3-B orchestrator coupling / tool-loop timeout deep), extractor boundary, Phase 7b/8 kararına otomatik geçilmez; kullanıcı açık onayı beklenir. **Hard kurallar (P7a):** Research section'a DOKUNMA; `apiFetch`/core/token refresh TAŞIMA; caller import path DEĞİŞTİRME; auth/session behavior DEĞİŞTİRME; production state-changing action tetikleme YOK; backend code YOK; DB/Redis YOK; frontend tests + lint + type-check + build PASS hedefi korunacak. |
