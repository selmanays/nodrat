...
| Son güncelleme | 2026-05-20 — Phase 2 PR 7b açılıyor: `api/admin_settings.py` → `modules/settings_admin/routes.py` (1551 LoC, admin route ownership taşıması; settings_store altyapısı PR 7a'da hazır); Phase 2 PR 7a ([#1107](https://github.com/selmanays/nodrat/pull/1107)) merged → main HEAD `bda2c03` + **passive runtime smoke PASS** (server-side 8/8 SSH verification). |
| Bir sonraki adım | Phase 2 PR 7b review + merge + **active write smoke** (PR 7a'dan deferred — admin route taşındıktan sonra anlamlı) → Phase 2 PR 8a/b (prompts_admin + shared/runtime_config/prompts_store, aynı pattern). |
