...
| Son güncelleme | 2026-05-20 — Phase 2 PR 7a açılıyor: `shared/runtime_config/settings_store` taşıması (Redis pub/sub altyapı; 46 caller path update, 30 dosya); Phase 2 PR 6 ([#1106](https://github.com/selmanays/nodrat/pull/1106)) merged scope-revized → main HEAD `649bf6d`. **PR 7'yi 2'ye böldük (kullanıcı kararı):** PR 7a = shared/runtime_config; PR 7b = modules/settings_admin. |
| Bir sonraki adım | Phase 2 PR 7a review + merge → Phase 2 PR 7b (`modules/settings_admin/`) → Phase 2 PR 8a/b (prompts_admin + shared/runtime_config/prompts_store, aynı pattern). |
