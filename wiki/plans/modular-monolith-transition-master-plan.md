...
| Alan | Değer |
|---|---|
| Aktif faz | **Phase 4/5/6/7a paralel** — P4 T6: char (#1144) + facade scaffold (#1146, boundary açık) + internal split (#1147); P5 T6: char (#1148) + phrase+vector split (#1149) + scoring split (#1152); **P6 T6: 11 PR ✅** (#1150 + #1153 + #1155 + #1157 + #1159 + #1160 + #1162 + #1164 + #1166 + #1168 + #1170); **P7a T6: 8 PR ✅** test infra (#1172) + public search (#1173) + admin disk (#1174) + auth login/register/logout (#1175) + verifyResend mini (#1177) + admin users (#1178) + admin audit (#1180) + admin /system (#1181); `api.ts` facade/re-export pattern **7 kez doğrulandı** (public + disk + auth + verifyResend + admin-users + admin-audit + admin-system); 60 caller import path DEĞİŞMEDİ; **`apiFetch` + `ApiException` + token storage + `attemptTokenRefresh` core'da KALDI** (P7a hard kural); `buildQuery` admin-users + admin-audit içine non-exported kopyalandı (shared helper deferred); `modules/generations` + `modules/rag` + `modules/crawler` Phase 1 scaffold → active facade; **workers/tasks/ artık yalnız `__init__.py`**; import-linter **13 contracts, 0 broken** muafiyetsiz |
| Bekleyen | P4 PR-D caller flip (boundary kararına bağlı; PR #1146 açık sorular); **P7a PR-7a-8 sırada** (scope analizi: 8 aday — Admin Media/Legal/Settings/Account-Me/Queue/Articles/Sources/RAG — implementation YOK, sadece rapor); **P7a Research section EN SONA** (691 LoC / 11+ caller; SSE client coupling); P6 PR-C+ full SSE integration replay (TestClient endpoint, full transcript with real research_tools mocks; RC3-B orchestrator coupling + tool-loop timeout deep coverage dahil); P7b frontend rag-page ([#1096](https://github.com/selmanays/nodrat/issues/1096)); P8 boundary hardening ([#1097](https://github.com/selmanays/nodrat/issues/1097)); N+1 model relocation ([#1098](https://github.com/selmanays/nodrat/issues/1098)) — blocked by T8 |
| Tamamlanan | **P0/1/2** kapandı (#1088/89/90); **P3** ana cycle (6 PR); **P5 mini-cycle** worker (#1143); **P6 mini-cycle** worker (#1140-#1142); **P4 god-file T6 PR-A/B/C** (#1144 + #1146 + #1147); **P5 god-file T6 PR-A/B/C** (#1148 + #1149 + #1152); **P6 god-file T6 PR-A/B/A1/A2a/A2b/A3/A4/A5/A6/A7/A8** (#1150 + #1153 + #1155 + #1157 + #1159 + #1160 + #1162 + #1164 + #1166 + #1168 + #1170); **P7a god-file T6 PR-7a-0/1/2/3/4/5/6/7** (#1172 + #1173 + #1174 + #1175 + #1177 + #1178 + #1180 + #1181); **13 closure docs** (#1145 + #1151 + #1154 + #1156 + #1158 + #1161 + #1163 + #1165 + #1167 + #1169 + #1171 + #1176 + #1179) |
| Aktif tracking | T1-T5 (#1080-#1084), **T6 [#1085](https://github.com/selmanays/nodrat/issues/1085) — OPEN** (P4: 3 PR ✅ + caller flip ertelendi; P5: 3 PR ✅; P6: 11 PR ✅ + RC3-B orchestrator coupling + tool-loop timeout + full SSE integration pending → PR-C+; **P7a: 8 PR ✅** (test infra + public + disk + auth + verifyResend + admin-users + admin-audit + admin-system); PR-7a-8 scope analizi sırada), **T7 [#1086](https://github.com/selmanays/nodrat/issues/1086) — OPEN** (cost_tracker pending), **T8 [#1087](https://github.com/selmanays/nodrat/issues/1087) — OPEN/BLOCKED** (5 ön-koşul). **Phase 6 hâlâ tamamlanmadı** — derin orchestration + endpoint TestClient kalır. **Phase 7a devam ediyor** — Research EN SONA, küçük mini-extract adayları sırada. **SSE replay coverage 10/10 senaryo HEDEF TAMAMLANDI** + 1 bonus. **RC3-B helper-level lock tamam.** |
| Son güncelleme | 2026-05-22 — **B seçeneği uzun tur 35 PR cumulative** (#1144-#1181 + closure docs v14). PR #1179 closure docs v13 (sadece wiki) + PR #1180 P7a PR-7a-6 Admin Audit extract (41 LoC → api/admin/audit.ts; +4 char test; read-only; buildQuery non-exported kopya 2. uygulama) + PR #1181 P7a PR-7a-7 Admin /system extract (77 LoC → api/admin/system.ts; 11 interface + 1 fonksiyon; +3 char test; read-only; buildQuery GEREK YOK) eklendi. **Phase 7a 8 PR DONE.** **Frontend characterization 29 test** (5 + 2 + 2 + 4 + 3 + 6 + 4 + 3 cumulative). God-file LoC reduction sabit: extractor 1189→1019, retrieval 2174→1911, SSE 1440→1406; `api.ts` 2041→1786 LoC (PR-7a-7 sonrası -255 net). **Toplam 153 characterization test** safety-net (backend 124 + frontend 29). **Veri güvenliği invariant — KORUNDU** (35 PR'da hiç chunk/embedding/index müdahalesi, manual rechunk/reembed/backfill, direct DB/Redis, manual production task trigger, state-changing smoke yok; **PR #1180 + #1181 endpoint'leri zaten read-only — state-changing smoke YAPILMADI**). PR #1180 + #1181 production behavior değişikliği YOK (endpoint + path + method özdeş; caller import path DEĞİŞMEDİ). **`apiFetch` + `ApiException` + token storage + `attemptTokenRefresh` core'da KALDI.** **`buildQuery` Admin Users + Admin Audit içinde non-exported kopya; Admin /system query param kullanmadığı için buildQuery'ye ihtiyaç duymadı; shared `_query.ts` deferred — Admin Sources extract aynı ihtiyaca düşerse housekeeping PR.** |
...
| Bir sonraki adım | **B seçeneği devamı (kullanıcı 2026-05-22 onayı):** (1) **Bu closure docs v14 PR** (PR #1180 + #1181 cumulative); (2) **PR-7a-8 scope analizi** (implementation YOK, sadece rapor) — 8 aday karşılaştırma: Admin Media / Legal / Settings / Account-Me / Queue / Articles / Sources / RAG; her aday için line range, LoC, interface/function sayısı, caller, read-only vs state-changing endpoint dağılımı, smoke güvenliği, test feasibility, apiFetch dep, buildQuery/internal helper ihtiyacı, özel risk (account deletion / runtime config / manual task trigger / RAG-research), hedef dosya, risk; PR-7a-8 önerisi + alternatif 2. aday + Admin Sources/Queue/Settings/Account-Me neden şimdi değil; rapor + öneri + DUR; (3) Sonrasında DUR — PR-7a-N extract'lar, Phase 6 PR-C+ (full SSE integration / RC3-B orchestrator coupling / tool-loop timeout deep), extractor boundary, Phase 7b/8 kararına otomatik geçilmez; kullanıcı açık onayı beklenir. **Hard kurallar (P7a):** Research section'a DOKUNMA; `apiFetch`/core/token refresh TAŞIMA; caller import path DEĞİŞTİRME; auth/session behavior DEĞİŞTİRME; production state-changing action tetikleme YOK; backend code YOK; DB/Redis YOK; frontend tests + lint + type-check + build PASS hedefi korunacak. |
