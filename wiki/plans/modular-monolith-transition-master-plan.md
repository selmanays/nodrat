...
| Alan | Değer |
|---|---|
| Aktif faz | **Phase 4/5/6/7a paralel** — P4 T6: char (#1144) + facade scaffold (#1146, boundary açık) + internal split (#1147); P5 T6: char (#1148) + phrase+vector split (#1149) + scoring split (#1152); **P6 T6: 11 PR ✅** (#1150 + #1153 + #1155 + #1157 + #1159 + #1160 + #1162 + #1164 + #1166 + #1168 + #1170); **P7a T6: 6 PR ✅** test infra (#1172) + public search extract (#1173) + admin disk extract (#1174) + auth extract login/register/logout (#1175) + verifyResend mini-extract (#1177) + admin users extract (#1178); `api.ts` facade/re-export pattern **5 kez doğrulandı** (public + disk + auth + verifyResend + admin-users); 60 caller import path DEĞİŞMEDİ; **`apiFetch` + `ApiException` + token storage + `attemptTokenRefresh` core'da KALDI** (P7a hard kural); `buildQuery` admin-users içine non-exported kopyalandı (shared helper deferred); `modules/generations` + `modules/rag` + `modules/crawler` Phase 1 scaffold → active facade; **workers/tasks/ artık yalnız `__init__.py`**; import-linter **13 contracts, 0 broken** muafiyetsiz |
| Bekleyen | P4 PR-D caller flip (boundary kararına bağlı; PR #1146 açık sorular); **P7a PR-7a-6 sırada** (scope analizi: 8 aday — Admin Articles/Sources/Queue/Audit/Account-Me/Legal/Settings/Media-System — implementation YOK, sadece rapor); **P7a Research section EN SONA** (691 LoC / 11+ caller; SSE client coupling); P6 PR-C+ full SSE integration replay (TestClient endpoint, full transcript with real research_tools mocks; RC3-B orchestrator coupling + tool-loop timeout deep coverage dahil); P7b frontend rag-page ([#1096](https://github.com/selmanays/nodrat/issues/1096)); P8 boundary hardening ([#1097](https://github.com/selmanays/nodrat/issues/1097)); N+1 model relocation ([#1098](https://github.com/selmanays/nodrat/issues/1098)) — blocked by T8 |
| Tamamlanan | **P0/1/2** kapandı (#1088/89/90); **P3** ana cycle (6 PR); **P5 mini-cycle** worker (#1143); **P6 mini-cycle** worker (#1140-#1142); **P4 god-file T6 PR-A/B/C** (#1144 + #1146 + #1147); **P5 god-file T6 PR-A/B/C** (#1148 + #1149 + #1152); **P6 god-file T6 PR-A/B/A1/A2a/A2b/A3/A4/A5/A6/A7/A8** (#1150 + #1153 + #1155 + #1157 + #1159 + #1160 + #1162 + #1164 + #1166 + #1168 + #1170); **P7a god-file T6 PR-7a-0/1/2/3/4/5** (#1172 + #1173 + #1174 + #1175 + #1177 + #1178); **12 closure docs** (#1145 + #1151 + #1154 + #1156 + #1158 + #1161 + #1163 + #1165 + #1167 + #1169 + #1171 + #1176) |
| Aktif tracking | T1-T5 (#1080-#1084), **T6 [#1085](https://github.com/selmanays/nodrat/issues/1085) — OPEN** (P4: 3 PR ✅ + caller flip ertelendi; P5: 3 PR ✅; P6: 11 PR ✅ + RC3-B orchestrator coupling + tool-loop timeout + full SSE integration pending → PR-C+; **P7a: 6 PR ✅** (test infra + public + disk + auth + verifyResend + admin-users); PR-7a-6 scope analizi sırada), **T7 [#1086](https://github.com/selmanays/nodrat/issues/1086) — OPEN** (cost_tracker pending), **T8 [#1087](https://github.com/selmanays/nodrat/issues/1087) — OPEN/BLOCKED** (5 ön-koşul). **Phase 6 hâlâ tamamlanmadı** — derin orchestration + endpoint TestClient kalır. **Phase 7a devam ediyor** — Research EN SONA, küçük mini-extract adayları sırada. **SSE replay coverage 10/10 senaryo HEDEF TAMAMLANDI** + 1 bonus. **RC3-B helper-level lock tamam.** |
| Son güncelleme | 2026-05-21 — **B seçeneği uzun tur 33 PR cumulative** (#1144-#1178 + closure docs v13). PR #1176 closure docs v12 (sadece wiki) + PR #1177 P7a PR-7a-4 verifyResend mini-extract (12 LoC → api/auth.ts; +3 char test) + PR #1178 P7a PR-7a-5 Admin Users extract (90 LoC → api/admin/users.ts; +6 char test; buildQuery non-exported kopya) eklendi. **Phase 7a 6 PR DONE.** **Frontend characterization 22 test** (5 + 2 + 2 + 4 + 3 + 6 cumulative). God-file LoC reduction sabit: extractor 1189→1019, retrieval 2174→1911, SSE 1440→1406; `api.ts` 2041→1868 LoC (PR-7a-5 sonrası -173 net). **Toplam 146 characterization test** safety-net (backend 124 + frontend 22). **Veri güvenliği invariant — KORUNDU** (33 PR'da hiç chunk/embedding/index müdahalesi, manual rechunk/reembed/backfill, direct DB/Redis, manual production task trigger, state-changing smoke yok; **PR #1175 + PR #1177 sırasında auth/email production action TETİKLENMEDİ**; **PR #1178 sırasında `updateAdminUser`/`restoreAdminUser` PATCH/POST production'a ÇAĞRILMADI** — yalnız sayfa render 200). PR #1177 + #1178 production behavior değişikliği YOK (function signature + endpoint + body + auth semantik özdeş; caller import path DEĞİŞMEDİ). **`apiFetch` + `ApiException` + token storage + `attemptTokenRefresh` core'da KALDI.** **`buildQuery` non-exported kopya yaklaşımı** (shared `_query.ts` deferred — Admin Sources extract aynı ihtiyaca düşerse housekeeping PR). |
...
| Bir sonraki adım | **B seçeneği devamı (kullanıcı 2026-05-21 onayı):** (1) **Bu closure docs v13 PR** (PR #1177 + #1178 cumulative); (2) **PR-7a-6 scope analizi** (implementation YOK, sadece rapor) — 8 aday karşılaştırma: Admin Articles / Sources / Queue / Audit / Account-Me / Legal / Settings/Admin Settings / Media-System; her aday için line range, LoC, interface/function sayısı, caller, read-only vs state-changing endpoint dağılımı, test feasibility, smoke güvenliği, apiFetch dep, buildQuery/internal helper ihtiyacı, Research/auth/core coupling, hedef dosya, risk; PR-7a-6 önerisi + alternatif 2. aday + Admin Sources neden şimdi/sonra; rapor + öneri + DUR; (3) Sonrasında DUR — PR-7a-N extract'lar, Phase 6 PR-C+ (full SSE integration / RC3-B orchestrator coupling / tool-loop timeout deep), extractor boundary, Phase 7b/8 kararına otomatik geçilmez; kullanıcı açık onayı beklenir. **Hard kurallar (P7a):** Research section'a DOKUNMA; `apiFetch`/core/token refresh TAŞIMA; caller import path DEĞİŞTİRME; auth/session behavior DEĞİŞTİRME; production state-changing action tetikleme YOK; backend code YOK; DB/Redis YOK; frontend tests + lint + type-check + build PASS hedefi korunacak. |
