...
| Alan | Değer |
|---|---|
| Aktif faz | **Phase 4/5/6/7a paralel** — P4 T6: char (#1144) + facade scaffold (#1146, boundary açık) + internal split (#1147); P5 T6: char (#1148) + phrase+vector split (#1149) + scoring split (#1152); **P6 T6: 11 PR ✅** (#1150 + #1153 + #1155 + #1157 + #1159 + #1160 + #1162 + #1164 + #1166 + #1168 + #1170); **P7a T6: 14 PR ✅** test infra (#1172) + public (#1173) + disk (#1174) + auth (#1175) + verifyResend mini (#1177) + admin users (#1178) + admin audit (#1180) + admin /system (#1181) + admin media (#1183) + buildQuery shared housekeeping (#1184) + admin legal (#1186) + admin articles (#1187) + getMyQuota mini (#1189) + account/me (#1190); `api.ts` facade/re-export pattern **11 kez doğrulandı** (public + disk + auth + verifyResend + admin-users + admin-audit + admin-system + admin-media + admin-legal + admin-articles + account/me); 60 caller import path DEĞİŞMEDİ; **`apiFetch` + `ApiException` + token storage + `attemptTokenRefresh` core'da KALDI** (P7a hard kural); **`buildQuery` shared `api/_query.ts`** (leaf, internal, re-export edilmez); **`api/account.ts` user-facing account domain** (quota + me/profile/export/delete birleşik); `modules/generations` + `modules/rag` + `modules/crawler` Phase 1 scaffold → active facade; **workers/tasks/ artık yalnız `__init__.py`**; import-linter **13 contracts, 0 broken** muafiyetsiz |
| Bekleyen | P4 PR-D caller flip (boundary kararına bağlı; PR #1146 açık sorular); **P7a kalan adaylar** (PR-7a-14 scope analizi sırada): Admin Sources (alt-bölünmeli) / Queue (manual task trigger) / Settings (runtime config) / RAG (381 LoC, research-adjacent); **P7a Research section EN SONA** (691 LoC / 11+ caller; SSE client coupling); P6 PR-C+ full SSE integration replay (TestClient endpoint, full transcript with real research_tools mocks; RC3-B orchestrator coupling + tool-loop timeout deep coverage dahil); P7b frontend rag-page ([#1096](https://github.com/selmanays/nodrat/issues/1096)); P8 boundary hardening ([#1097](https://github.com/selmanays/nodrat/issues/1097)); N+1 model relocation ([#1098](https://github.com/selmanays/nodrat/issues/1098)) — blocked by T8 |
| Tamamlanan | **P0/1/2** kapandı (#1088/89/90); **P3** ana cycle (6 PR); **P5 mini-cycle** worker (#1143); **P6 mini-cycle** worker (#1140-#1142); **P4 god-file T6 PR-A/B/C** (#1144 + #1146 + #1147); **P5 god-file T6 PR-A/B/C** (#1148 + #1149 + #1152); **P6 god-file T6 PR-A/B/A1/A2a/A2b/A3/A4/A5/A6/A7/A8** (#1150 + #1153 + #1155 + #1157 + #1159 + #1160 + #1162 + #1164 + #1166 + #1168 + #1170); **P7a god-file T6 PR-7a-0..13** (#1172 + #1173 + #1174 + #1175 + #1177 + #1178 + #1180 + #1181 + #1183 + #1184 + #1186 + #1187 + #1189 + #1190); **16 closure docs** (#1145 + #1151 + #1154 + #1156 + #1158 + #1161 + #1163 + #1165 + #1167 + #1169 + #1171 + #1176 + #1179 + #1182 + #1185 + #1188) |
| Aktif tracking | T1-T5 (#1080-#1084), **T6 [#1085](https://github.com/selmanays/nodrat/issues/1085) — OPEN** (P4: 3 PR ✅ + caller flip ertelendi; P5: 3 PR ✅; P6: 11 PR ✅ + RC3-B orchestrator coupling + tool-loop timeout + full SSE integration pending → PR-C+; **P7a: 14 PR ✅** (test infra + public + disk + auth + verifyResend + admin-users + admin-audit + admin-system + admin-media + buildQuery housekeeping + admin-legal + admin-articles + getMyQuota mini + account/me); PR-7a-14 scope analizi sırada — Admin Sources/Queue/Settings/RAG), **T7 [#1086](https://github.com/selmanays/nodrat/issues/1086) — OPEN** (cost_tracker pending), **T8 [#1087](https://github.com/selmanays/nodrat/issues/1087) — OPEN/BLOCKED** (5 ön-koşul). **Phase 6 hâlâ tamamlanmadı** — derin orchestration + endpoint TestClient kalır. **Phase 7a devam ediyor** — Research EN SONA. **SSE replay coverage 10/10 senaryo HEDEF TAMAMLANDI** + 1 bonus. **RC3-B helper-level lock tamam.** |
| Son güncelleme | 2026-05-22 — **B seçeneği uzun tur 40 PR cumulative** (#1144-#1190 + closure docs v17). PR #1188 closure docs v16 (sadece wiki) + PR #1189 P7a PR-7a-12 getMyQuota mini-extract (15 LoC → api/account.ts yeni dosya; +2 char test) + PR #1190 P7a PR-7a-13 Account/Me extract (68 LoC → mevcut api/account.ts; 4 interface + 4 fonksiyon; +6 char test; updateMe/exportMe/deleteMe smoke-skip) eklendi. **Phase 7a 14 PR DONE.** **Frontend characterization 52 test** (5 + 2 + 2 + 4 + 3 + 6 + 4 + 3 + 4 + 0 + 4 + 7 + 2 + 6 cumulative). God-file LoC reduction sabit: extractor 1189→1019, retrieval 2174→1911, SSE 1440→1406; **`api.ts` 2041→1502 LoC (PR-7a-13 sonrası -539 net, ~%26 küçülme).** **Toplam 176 characterization test** safety-net (backend 124 + frontend 52). **Veri güvenliği invariant — KORUNDU** (40 PR'da hiç chunk/embedding/index müdahalesi, manual rechunk/reembed/backfill, direct DB/Redis, manual production task trigger, state-changing smoke yok; **PR #1190 `updateMe`/`exportMe`/`deleteMe` production'a YOLLANMADI — PII export YOK, account deletion YOK**). PR #1189 + #1190 production behavior değişikliği YOK (endpoint + path + method + body özdeş; caller import path DEĞİŞMEDİ). **`apiFetch` + `ApiException` + token storage + `attemptTokenRefresh` core'da KALDI.** **`api/account.ts` user-facing account domain birleşik (quota + me/profile/export/delete).** |
...
| Bir sonraki adım | **B seçeneği devamı (kullanıcı 2026-05-22 onayı):** (1) **Bu closure docs v17 PR** (PR #1190 Account/Me tekli); (2) **Kalan P7a adayları scope analizi** (PR-7a-14 için, implementation YOK) — Admin Sources / Queue / Settings / RAG / Research risk karşılaştırma: line range, LoC, interface/function, caller, read-only vs state-changing, manual task trigger / runtime config / RAG-research pipeline riski, buildQuery, hedef dosya, tek/alt PR, smoke güvenliği, test feasibility, risk; PR-7a-14 önerisi + alternatif + DUR; (3) Sonrasında DUR — PR-7a-N extract'lar, Phase 6 PR-C+ (full SSE integration / RC3-B orchestrator coupling / tool-loop timeout deep), extractor boundary, Phase 7b/8 kararına otomatik geçilmez; kullanıcı açık onayı beklenir. **Hard kurallar (P7a):** Research section'a DOKUNMA; `apiFetch`/core/token refresh TAŞIMA; caller import path DEĞİŞTİRME; auth/session behavior DEĞİŞTİRME; production state-changing/manual-task/runtime-config/RAG-trigger action tetikleme YOK; backend code YOK; DB/Redis YOK; frontend tests + lint + type-check + build PASS hedefi korunacak. |
