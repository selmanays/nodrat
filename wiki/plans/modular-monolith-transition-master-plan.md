...
| Alan | Değer |
|---|---|
| Aktif faz | **Phase 4/5/6 paralel** — P4 T6: char (#1144) + facade scaffold (#1146, boundary açık) + internal split (#1147); P5 T6: char (#1148) + phrase+vector split (#1149) + scoring split (#1152); **P6 T6: 11 PR ✅** char (#1150) + pure-helper split (#1153) + async helper char (#1155) + heavy mock async `_generate_followups` (#1157) + heavy mock async `_tracked_chat_generate` (#1159) + minimal replay (#1160) + replay expansion 4-scenario (#1162) + orchestration first-yield char (#1164) + replay/edge 3-invariant (#1166) + replay/golden 10. senaryo + bonus (#1168) + **`_has_reconstruction_marker` helper char (#1170)**; P5+P6 worker mini-cycle done; `modules/generations` + `modules/rag` + `modules/crawler` Phase 1 scaffold → active facade; **workers/tasks/ artık yalnız `__init__.py`**; import-linter **13 contracts, 0 broken** muafiyetsiz |
| Bekleyen | P4 PR-D caller flip (boundary kararına bağlı; PR #1146 açık sorular); **P7a frontend `api.ts` split** ([#1095](https://github.com/selmanays/nodrat/issues/1095); [[phase7a-frontend-mini-plan]] playbook yazıldı; **PR-7a-0 önce test infra bootstrap** Vitest+jsdom + ≤5 helper char); P6 PR-C+ full SSE integration replay (TestClient endpoint, full transcript with real research_tools mocks; RC3-B orchestrator coupling + tool-loop timeout deep coverage dahil); P7b frontend rag-page ([#1096](https://github.com/selmanays/nodrat/issues/1096)); P8 boundary hardening ([#1097](https://github.com/selmanays/nodrat/issues/1097)); N+1 model relocation ([#1098](https://github.com/selmanays/nodrat/issues/1098)) — blocked by T8 |
| Tamamlanan | **P0/1/2** kapandı (#1088/89/90); **P3** ana cycle (6 PR); **P5 mini-cycle** worker (#1143); **P6 mini-cycle** worker (#1140-#1142); **P4 god-file T6 PR-A/B/C** (#1144 + #1146 + #1147); **P5 god-file T6 PR-A/B/C** (#1148 + #1149 + #1152); **P6 god-file T6 PR-A/B/A1/A2a/A2b/A3/A4/A5/A6/A7/A8** (#1150 + #1153 + #1155 + #1157 + #1159 + #1160 + #1162 + #1164 + #1166 + #1168 + #1170); **10 closure docs** (#1145 + #1151 + #1154 + #1156 + #1158 + #1161 + #1163 + #1165 + #1167 + #1169) |
| Aktif tracking | T1-T5 (#1080-#1084), **T6 [#1085](https://github.com/selmanays/nodrat/issues/1085) — OPEN** (P4: 3 PR ✅ + caller flip ertelendi; P5: 3 PR ✅; P6: 11 PR ✅ (char + split + async char + 2 heavy mock async + 2 replay + orchestration first-yield + 3 replay/edge + 2 replay/golden + marker helper char) + RC3-B orchestrator coupling + tool-loop timeout + full SSE integration pending → PR-C+; P7a frontend mini plan yazıldı, PR-7a-0 test infra bootstrap sırada), **T7 [#1086](https://github.com/selmanays/nodrat/issues/1086) — OPEN** (cost_tracker pending), **T8 [#1087](https://github.com/selmanays/nodrat/issues/1087) — OPEN/BLOCKED** (5 ön-koşul). **Phase 6 hâlâ tamamlanmadı** — derin orchestration + endpoint TestClient kalır. **SSE replay coverage 10/10 senaryo HEDEF TAMAMLANDI** + 1 bonus. **RC3-B helper-level lock tamam.** |
| Son güncelleme | 2026-05-21 — **B seçeneği uzun tur 27 PR cumulative** (#1144-#1170). PR #1169 closure docs v10 + PR #1170 P6 PR-A8 `_has_reconstruction_marker` helper-level characterization (15 test, RC3-B regex katalogu lock) eklendi. **Phase 7a kalıcı mini plan playbook yazıldı** ([[phase7a-frontend-mini-plan]]). God-file LoC reduction sabit: extractor 1189→1019 (-170; helpers 212), retrieval 2174→1911 (-263; helpers 373), SSE 1440→1406 (-34; helpers 64). **Toplam 124 characterization test** safety-net (extractor 15 + retrieval 25 + SSE pure 33 + SSE async light 17 + SSE async heavy 9 + 12 + SSE replay 2 + 4 + SSE orchestration 2 + SSE replay/edge 3 + SSE replay/golden 2). **Veri güvenliği invariant — KORUNDU** (27 PR'da hiç chunk/embedding/index müdahalesi, manual rechunk/reembed/backfill, direct DB/Redis, manual production task trigger, state-changing smoke yok). PR #1170 test-only PR + production behavior değişikliği YOK. **SSE replay coverage: 10/10 senaryo hedef TAMAMLANDI + 1 bonus. RC3-B helper-level regex katalogu lock'lu.** |
...
| Bir sonraki adım | **B seçeneği devamı (kullanıcı 2026-05-21 onayı):** (1) **Bu closure docs v11 PR** (PR #1169 + #1170 + Phase 7a [[phase7a-frontend-mini-plan]] cumulative); (2) **PR-7a-0 frontend test infra bootstrap** — Vitest + jsdom (veya alternatif PR-7a-0 reality check sonucu) + ≤5 helper char test (`ApiException`, token storage, `apiFetch` mock); production behavior change YOK; api.ts split YOK; CI'a yeni test step eklenir; (3) Sonrasında DUR — PR-7a-1 (Public search extract), Phase 6 PR-C+ (full SSE integration / RC3-B orchestrator coupling / tool-loop timeout deep), extractor boundary, Phase 8 kararına otomatik geçilmez; kullanıcı açık onayı beklenir. |
