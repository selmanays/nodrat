...
| Alan | Değer |
|---|---|
| Aktif faz | **Phase 3** ([#1091](https://github.com/selmanays/nodrat/issues/1091)) — in-progress; **PR 1a + 1b + 2a + 2b + 3 + ops/maintenance merged** (sources kernel + articles kernel + embedding middle-layer + ops/maintenance cross-cutting aktif); **transient `ignore_imports` muafiyeti yok**; import-linter **13 contracts, 0 broken** muafiyetsiz; kalan workers/tasks/: agenda + cluster_assigner + raptor (hepsi Phase 5/6 god-file scope — mini plan only, user kuralı) |
...
| Aktif tracking | T1 [#1080](https://github.com/selmanays/nodrat/issues/1080), T2 [#1082](https://github.com/selmanays/nodrat/issues/1082), T3 [#1081](https://github.com/selmanays/nodrat/issues/1081), T4 [#1083](https://github.com/selmanays/nodrat/issues/1083), T5 [#1084](https://github.com/selmanays/nodrat/issues/1084), **T6 [#1085](https://github.com/selmanays/nodrat/issues/1085) — OPEN** (Phase 4-7 god-file facade migrations pending: `core/extractor.py`/Phase 4, `core/retrieval.py`/Phase 5, `api/app_research_stream.py`/Phase 6, `src/lib/api.ts`/Phase 7a, `src/app/admin/rag/page.tsx`/Phase 7b — hiçbiri henüz başlamadı; **T6 asıl scope'u god-file facade-first pipeline, Phase 3 sources/articles/embedding migration KAPSAM DIŞI**; transient `ignore_imports` cycle dersi `refactor-pr-checklist §6.9`'da kayıtlı), T7 [#1086](https://github.com/selmanays/nodrat/issues/1086), T8 [#1087](https://github.com/selmanays/nodrat/issues/1087) |
| Son güncelleme | 2026-05-21 — **Phase 3 ops sub-cycle merged** (PR #1137 `ad98ef7`; gece otonom batch mode auto-merge). modules/ops/tasks/maintenance.py aktive (713 LoC, 6 task names korundu, 100% similarity). Auto-merge gate sağlandı (CI 10/10 + 5-form 0 + 13/13 + no new contract + no ignore_imports + scope düşük + data safety preserved). SHA pinning 3-way match. VPS filesystem post-deploy doğrulandı (modules/ops var, workers/tasks/maintenance.py GONE). Erken false-fail timing dersi çıktı — refactor checklist §13.1'e eklendi. |
| Bir sonraki adım | **Phase 4/5/6/7 mini-plan-only docs PR**'ları (kullanıcı kuralı: god-file parçalama + scope expansion = mini plan only): agenda → generations Phase 6; cluster_assigner → generations Phase 6; raptor → rag Phase 5; extraction/SSE/api.ts/rag-page → T6 #1085 god-file checklist'i. Implementation kullanıcı talimatı bekler. |
