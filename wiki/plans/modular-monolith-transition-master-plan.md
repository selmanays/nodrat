...
| Son güncelleme | 2026-05-20 — **Phase 3 PR 1b CI 10/10 yeşil** ([#1127](https://github.com/selmanays/nodrat/pull/1127)). Temporary `ignore_imports` exception: `sources → workers.tasks.articles` transitif legacy chain (articles/embedding henüz migrate olmadı). Hidden/bidi audit clean (false positives only). Explicit merge onayı bekleniyor. |
| Bir sonraki adım | Kullanıcı explicit "merge et" → merge + CI/CD ordering observation + passive smoke + active write smoke via Playwright (test source CRUD `__SMOKE_TEST_PR_1B__`) → closure PR → Phase 3 PR 2 articles mini plan (articles migration sırasında `ignore_imports` exception tekrar değerlendirilecek). |
