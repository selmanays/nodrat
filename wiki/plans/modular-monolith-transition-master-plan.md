...
| Alan | Değer |
|---|---|
| Aktif faz | **Phase 4/5/6 paralel** — P4 T6: char (#1144) + facade scaffold (#1146, boundary açık) + internal split (#1147); P5 T6: char (#1148) + phrase+vector split (#1149) + scoring split (#1152); **P6 T6: 9 PR ✅** char (#1150) + pure-helper split (#1153) + async helper char (#1155) + heavy mock async `_generate_followups` (#1157) + heavy mock async `_tracked_chat_generate` (#1159) + minimal replay (#1160) + replay expansion 4-scenario (#1162) + orchestration first-yield char (#1164) + **replay/edge 3-invariant (#1166)**; P5+P6 worker mini-cycle done; `modules/generations` + `modules/rag` + `modules/crawler` Phase 1 scaffold → active facade; **workers/tasks/ artık yalnız `__init__.py`**; import-linter **13 contracts, 0 broken** muafiyetsiz |
| Bekleyen | P4 PR-D caller flip (boundary kararına bağlı; PR #1146 açık sorular); P6 PR-A7 SSE replay/golden coverage 10. senaryo (kullanıcı plan: 1-2 yeni test; 10 senaryo hedefinden 9'da tamamlanmış); P6 PR-C+ full SSE integration replay (TestClient endpoint, full transcript with real research_tools mocks); P7a frontend api.ts ([#1095](https://github.com/selmanays/nodrat/issues/1095)); P7b frontend rag-page ([#1096](https://github.com/selmanays/nodrat/issues/1096)); P8 boundary hardening ([#1097](https://github.com/selmanays/nodrat/issues/1097)); N+1 model relocation ([#1098](https://github.com/selmanays/nodrat/issues/1098)) — blocked by T8 |
| Tamamlanan | **P0/1/2** kapandı (#1088/89/90); **P3** ana cycle (6 PR); **P5 mini-cycle** worker (#1143); **P6 mini-cycle** worker (#1140-#1142); **P4 god-file T6 PR-A/B/C** (#1144 + #1146 + #1147); **P5 god-file T6 PR-A/B/C** (#1148 + #1149 + #1152); **P6 god-file T6 PR-A/B/A1/A2a/A2b/A3/A4/A5/A6** (#1150 + #1153 + #1155 + #1157 + #1159 + #1160 + #1162 + #1164 + #1166); **8 closure docs** (#1145 + #1151 + #1154 + #1156 + #1158 + #1161 + #1163 + #1165) |
| Aktif tracking | T1-T5 (#1080-#1084), **T6 [#1085](https://github.com/selmanays/nodrat/issues/1085) — OPEN** (P4: 3 PR ✅ + caller flip ertelendi; P5: 3 PR ✅; P6: 9 PR ✅ (char + split + async char + 2 heavy mock async + 2 replay + orchestration first-yield + 3 replay/edge) + replay golden 10. senaryo (PR-A7) + full SSE integration pending; P7a/b: 0), **T7 [#1086](https://github.com/selmanays/nodrat/issues/1086) — OPEN** (cost_tracker pending), **T8 [#1087](https://github.com/selmanays/nodrat/issues/1087) — OPEN/BLOCKED** (5 ön-koşul). **Phase 6 hâlâ tamamlanmadı** — derin orchestration (planner/condense/retrieval/persist) + endpoint TestClient kalır. **SSE replay coverage 9/10 senaryo** (PR-A7 ile 10/10 hedefi). |
| Son güncelleme | 2026-05-21 — **B seçeneği uzun tur 23 PR cumulative** (#1144-#1166). PR #1165 closure docs v8 + PR #1166 P6 PR-A6 minimal SSE replay/edge characterization (3 structural invariant) eklendi. God-file LoC reduction sabit: extractor 1189→1019 (-170; helpers 212), retrieval 2174→1911 (-263; helpers 373), SSE 1440→1406 (-34; helpers 64). **Toplam 107 characterization test** safety-net (extractor 15 + retrieval 25 + SSE pure 18 + SSE async light 17 + SSE async heavy 9 + 12 + SSE replay 2 + 4 + SSE orchestration 2 + SSE replay/edge 3). **Veri güvenliği invariant — KORUNDU** (23 PR'da hiç chunk/embedding/index müdahalesi, manual rechunk/reembed/backfill, direct DB/Redis, manual production task trigger, state-changing smoke yok). PR #1166 test-only PR + production behavior değişikliği YOK. |
...
| Bir sonraki adım | **B seçeneği devamı (kullanıcı 2026-05-21 onayı):** (1) **Bu closure docs v9 PR** (PR #1166 cumulative); (2) PR-A7 SSE replay/golden coverage 10. senaryoyu tamamla — 1-2 yeni test (düşük riskli adaylar: progress/metadata, warning/empty-source, done payload variant, chunk boundary edge, source+followup+done farklı kombinasyon); RC3-B marker veya tool-loop timeout minimal replay seviyesinde güvenli ise 1 test, değilse "deferred: deep orchestration required"; (3) Sonrasında DUR — full SSE integration, Phase 7 frontend, extractor boundary, Phase 8 kararına otomatik geçilmez; kullanıcı açık onayı beklenir. |
