...
| Son güncelleme | 2026-05-20 — Phase 2 PR 6 scope revize (kullanıcı feedback): `modules/clusters/` artık **2 dosya** (core + tasks); `admin_clusters.py` legacy kalır (research-domain — Phase 6 generations follow-up); Phase 2 PR 5 ([#1105](https://github.com/selmanays/nodrat/pull/1105)) merged → main HEAD `9991251` |
| Bir sonraki adım | Phase 2 PR 6 (scope revize) review + merge → runtime-sensitive PR 7 (settings_admin) + PR 8 (prompts_admin). Pattern olgunlaştı, Phase 2 son iki PR runtime/Redis pub/sub doğrulamasıyla. |
