...
| Son güncelleme | 2026-05-20 — Phase 2 PR 8b açılıyor: `apps/api/app/api/admin_prompts.py` → `apps/api/app/modules/prompts_admin/routes.py` (657 LoC, admin route ownership). Tek dosya 1-to-1 git mv + main.py wiring + scaffold __init__/README aktive. Audit: §6.7 0 + §6.8 0/0/0. Active write smoke acceptance — Playwright MCP üzerinden agenda_card override → restore (DB row 0→1→0). Önceki: PR 8a merged ([commit `008d6de`](https://github.com/selmanays/nodrat/pull/1118)) + passive smoke 11/11 PASS + CI/CD ordering 2. test PASS. PR #1119 closure merged ([commit `9848c5b`](https://github.com/selmanays/nodrat/pull/1119)). |
| Bir sonraki adım | Phase 2 PR 8b review + merge + **active write smoke via Playwright MCP** (agenda_card 6-step: READ → WRITE → READ same-process → READ worker invalidation → RESTORE → READ final). Sonra Phase 2 closure + [#1114](https://github.com/selmanays/nodrat/issues/1114) paths-ignore optimization. |
