...
> ✅ **Ölçüldü (2026-06-11):** İlk only-do-if koşulu read-only analizle veriye bağlandı — admin yarı-gerçek kohortta (N=63) production `decompose_heuristic` multi-intent oranı **≤%4.8** (eşik %10-15'in altında) → **#619 rafta kalır**. En büyük gözlenen failure yüzeyi decomposition değil **citation-loss**; follow-up vaka-incelemesi başlangıçtaki %19'u düzeltti: 8/12 tarihsel RC3-B v1 reframe kümesi (#1076 ile kapatılmış), **aktif organik silent-uncited ≈%6.3** (aktif problemler: retrieval gündem-dump + citation enforcement asimetrisi). Tek zero-source vaka da expansion değil sınıf-3 retrieval-miss çıktı. Detay: [[query-failure-analysis-2026-06]] §8.
