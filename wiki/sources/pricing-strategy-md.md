---
type: source
title: "docs/strategy/pricing-strategy.md — Pricing Strategy"
slug: "pricing-strategy-md"
status: "ingested-summary"
source_path: "docs/strategy/pricing-strategy.md"
source_version: "v0.2 (2026-05-08 — USD primary + Lemon Squeezy MoR, Epic [#448](https://github.com/selmanays/nodrat/issues/448))"
source_updated: "n/a"
created: "2026-05-11"
updated: "2026-05-12"
ingest_method: "summary-only (bulk auto-generated, #696 D16 continuation)"
tags: ['docs', 'strategy', 'strategy', 'business']
---

# Source: docs/strategy/pricing-strategy.md

> **TL;DR:** Pricing tier'ları: Trial / Free / Starter $8 / Pro $24 / Agency $79. USD primary, TL display locale. 715 satır, 14 ana bölüm.
>
> ⚠️ **MVP-1 reality (#720, 2026-05-12):** §2.4 + §2.5'e footnote eklendi — Claude Haiku 4.5 + Sonnet 4.6 entegrasyonu Faz 2'de devreye alınacak; MVP-1'de tüm tier'lar DeepSeek V4 Flash kullanıyor. UI/pricing sayfası "Faz 2'de aktif" notu ile şeffaf iletiyor. Detay: [[anthropic-adapter-planned]].

## Section Map

| § | Başlık |
|---|---|
| §1 | 0. Yönetici Özeti |
| §2 | 1. Pricing Felsefesi |
| §3 | 2. Tier Yapısı |
| §4 | 3. Tier Karşılaştırma Matrisi |
| §5 | 4. Trial Mekanikleri ve Conversion Funnel |
| §6 | 5. Yıllık vs Aylık |
| §7 | 6. Geographic Pricing — Lemon Squeezy MoR (global) |
| §8 | 7. Pricing Display Stratejisi |
| §9 | 8. Kullanım Aşımı (Overage) Politikası |
| §10 | 9. Kötüye Kullanım Koruması |
| §11 | 10. Refund ve Cancellation |
| §12 | 11. Pricing Test ve Iterasyon |
| §13 | 12. Karar Noktaları |
| §14 | 13. Çapraz Referans |

## İlişkiler

(Detay entity/concept extraction sonraki sprintte — bu sayfa minimum-viable
source özet'i. Direct backlink'ler kategori bazında [[wiki/index|index]]'te kataloglu.)

## Versiyon takibi

| Doküman v | Tarih | Notlar |
|---|---|---|
| v0.2 (2026-05-08 — USD primary + Lemon Squeezy MoR, Epic [#448](https://github.com/selmanays/nodrat/issues/448)) | n/a | İlk ingest (auto-generated) |

## Açık takip

1. **Detay entity/concept extraction** — bu sayfa sadece source özet'i. Her ana bölüm için kendi wiki sayfası gelecek sprintte yapılacak (varlık/kavram/karar çıkarımı).
2. **Backlink integrity** — wiki/decisions ve wiki/concepts'ten bu source'a ters yön backlink eklenmesi
3. **Versiyon takibi** — kaynak dosya güncellendiğinde source_version + source_updated bump

## Kaynak

- [docs/strategy/pricing-strategy.md](../../docs/strategy/pricing-strategy.md)
