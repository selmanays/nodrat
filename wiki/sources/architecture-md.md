...
> 🟡 **Açık operasyonel migration — Embedding model (#345/#346 scaffold tamam, DB re-embed bekliyor):** §4.2 + §5.6 tutarlı şekilde "NIM `nim_bge_m3` aslında BAAI/bge-m3'ten farklı bir model serve ediyor (`nvidia/nv-embedqa-e5-v5`, cosine ≈ 0 orthogonal)" diyor — bu wiki ↔ docs çelişkisi **değil**, dokümante edilmiş bilinen operasyonel durum. LocalBgeM3Provider scaffold #345/#346 ile merged (commit `2c8e1a2` + `bb230ae`); ancak `apps/api/app/config.py:128-146` `use_local_embedding: bool = False` default'u korunuyor (re-embed task production'da koşturulmadı). Wiki ve kaynak doküman tutarlı; gerçek "kapanış" DB chunks + agenda_cards re-embed migration task'ının çalıştırılmasıyla olur. [[nim-bge-m3]] entity sayfasında operasyonel detay.
