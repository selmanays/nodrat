...
## [2026-05-20] phase2-pr6 | Modular Monolith Phase 2 PR 6 — modules/clusters altıncı modül taşıma (article-event only, behavior-preserving)

- **Kaynak/Tetikleyici:** Phase 2 PR 5 ([#1105](https://github.com/selmanays/nodrat/pull/1105)) merged (main HEAD `9991251`). Kullanıcı tercih sırası: media ✅ → clusters → runtime-sensitive (settings_admin, prompts_admin).
- **Hedef:** `modules/clusters/` — yalnız **article-event clustering**. Kullanıcı net kontrol: "RAPTOR clustering rag alanında kalmalı; clusters sadece article-level event clustering olmalı". Pivot research clustering generations Phase 6'da taşınacak.
- **Etkilenen sayfalar:** [[modular-monolith-transition-master-plan]] §13 + §12.3 changelog (PR 5 merged + §2.4 ownership revize: research_cluster → generations). **Yeni sayfa: 0**.
- **Teslim — 1-to-1 dosya taşıması (git mv, 3 dosya):**
  ...
  - `apps/api/app/api/admin_clusters.py` (188 sat) → `modules/clusters/admin/routes.py` (`/admin/clusters/*`)
  - `modules/clusters/__init__.py` → `admin_router` re-export facade
  - `modules/clusters/README.md` → status active + **scope sınırı açıklaması** (RAPTOR + research_clustering + cluster_assigner explicit hariç)
...
- **External caller updates:**
  - `apps/api/app/main.py`: `admin_clusters,` `api/` listede çıkar; `from app.modules import clusters, legal, media, sft, style_profiles` (alfabetik); include `admin_clusters.router` → `clusters.admin_router`
...
- **No alias-debt:** Broader grep audit:
  - `from app.<x>(.<y>)? import` clustering | admin_clusters → 0 sonuç (research_clustering hariç — kasıtlı)
  - `import app.<x>` clustering | admin_clusters → 0 sonuç
