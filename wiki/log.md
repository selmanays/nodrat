...
## [2026-05-09] hotfix | SSE streaming buffer'lanıyor — Caddy encode bypass + flush_interval (#531, PR #532)
...
- **Etkilenen sayfa:** [[sse-streaming-default]] — Implementation note: Caddy gzip/zstd compression SSE'yi buffer'lar; bypass gerekli (yeni proxy disiplini).
- **Yeni:** 0 wiki page
- **Güncellendi:** Aşağıdaki hotfix log girişi.
...
### Fix (PR [#532](https://github.com/selmanays/nodrat/pull/532), commit `706f71c1`)
...
2. **`apps/api/app/api/app_generate_stream.py`** — StreamingResponse headers:
   ...
   - `Content-Encoding: identity` (gzip/zstd bypass garantisi — middleware'lar compression denerse atlasın)

### Deploy gotcha'lar (manuel SSH)

İki yan sorun çıktı:

1. **API container `--force-recreate` rebuild yetmedi:** Mevcut image hash aynıydı, container restart oldu ama yeni kod load edilmedi. `docker compose build` cache'li layer kullandı. Çözüm: **`--no-cache` rebuild zorunlu** (container içindeki `main.py` import'u kontrol ile doğrula).
2. **Caddy named matcher syntax:** İlk denemede `encode { match { not path ... } }` yazdım — `Error: unrecognized response matcher 'not'`. Caddy v2 syntax: **named matcher tanımla, sonra encode'a geç:**
   ...
   ```
   Site ~30 saniye down kaldı; düzeltme + force-recreate sonrası geri geldi.
...
- Her iki durumda da: `docker exec <container> grep <change-token> /path` ile değişikliğin gerçekten container'a girip girmediğini doğrula.
...
- Kullanıcı tarayıcı testi pending (bu giriş yazılırken).
