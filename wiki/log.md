...
updated: 2026-05-28
---
<!-- v82: 🛑 **T8-11a HARD-STOP — DEFERRED (not failed) — `domain modules must not import ops/` contract analog of v77/v78 core/ pattern** (docs-only). v81 closure (T8-10 agenda) merge sonrası PR-T8-11a candidate triage yapıldı: failed_job + admin_audit_log + alternatives. **🛑 YENİ HARD-STOP DİSCOVERY:** FailedJob → `modules/ops/models.py` taşıma `domain modules must not import ops/` import-linter contract'ını ihlal edecek. v77/v78'in `core/* must not import modules/*` paterninin **analog'u** — bu sefer domain modules consumer'lar sorun. **FailedJob caller breakdown:** `apps/api/app/modules/articles/tasks/articles.py:53` (module-level eager — articles modülü domain modules contract forbidden source list'inde) + `apps/api/app/modules/sources/tasks/sources.py:195` (lazy in-function — sources modülü contract forbidden source list'inde) + `apps/api/app/api/admin_queue.py:47` (api/ → ops izinli, contract scope dışında) + 2 test. Modules/articles ve modules/sources her ikisi de import-linter `domain modules must not import ops/` source_modules listesinde — FailedJob ops/'a taşınırsa **hard-stop contract break** (T8-7 v77 paterninin analog'u). **AdminAuditLog daha kötü durumda:** 11 production caller (modules/articles/admin, legal, prompts_admin, settings_admin, sft/admin, sources/admin hepsi domain modules list'inde) → **DOUBLE BLOCK** (hem caller > 8 budget aşar HEM domain modules ops/ contract ihlali). **3-aday triage sonucu:** failed_job 🛑 BLOCKED (domain modules ops/ contract); admin_audit_log 🛑 DOUBLE BLOCK (caller > 8 + domain modules); ProviderCallLog 🛑 BLOCKED (core/ cost_tracker v77). **Other remaining models (12 → audit):** article 13 caller sub-PR split gerekiyor (büyük scope); billing/conversation/generation/research_cache_telemetry/source/user — hepsi core/ consumer BLOCKED; email — shared/Base prerequisite BLOCKED (v80). Wave D vector hardening T8-19 (article.summary_embedding) BLOCKED (article move yapılmadı, T8-12 prereq); T8-20 (EventCluster.embedding) "re-verify only" — class T8-8'de modules/clusters/models.py'ye taşındı + Phase 8.2-11 ORM hardened → re-verify pratik olarak NOP PR (her PR'ın mapper_resolution + TAM suite zaten doğruluyor). **Sonuç: T8 SAFE pool DEPLETED at 10/22 (~45%).** **🔑 v82 capstone insight — YENİ HARD-STOP PATTERN (refactor-pr-checklist eklenecek):** "Pre-PR audit zorunlu adımı genişletilmeli — target module'ün bir import-linter forbidden contract'ında olduğu durumda her caller modülünü contract source_modules listesi ile cross-reference yap. Eğer herhangi bir caller modülü source listede ise → STOP/DEFER (v77 paterninin analog'u)." Spesifik komut: `git grep -lE "from app.models.X|from app.models import .*\bClassName\b" apps/api/app/modules` çıktısını import-linter pyproject.toml `[[tool.importlinter.contracts]]` source_modules listeleriyle eşleştir. **5 hard-stop tetiği listesi (kümülatif v77+v78+v82):** (1) target module core/ tarafından import ediliyorsa = `core/* must not import modules/*` ihlali (v77); (2) facade path da `core/* → app.models → app.modules.*` transitive ihlali (v78 — POISONED facade insight); (3) target module domain modules tarafından import ediliyorsa ve target import-linter `domain modules must not import <target>/` contract'ında ise (v82 — bu); (4) target shared/* paketinde ve model `from app.models.base import Base` import ediyorsa = `shared/* must not import legacy core/api/models` ihlali (v80 email keşfi); (5) caller bütçesi > 8 dosya = sub-PR split zorunlu (kullanıcı v78 hard kuralı). **Triage decision (per kullanıcı locked):** T8 implementation cycle SAFE pool dolduğunda DUR; kullanıcıya raporla. (a) **T7 cost_tracker initiative** [#1086](https://github.com/selmanays/nodrat/issues/1086) artık T8 ilerlemesi için critical prerequisite — kullanıcı yetki + scope onayı bekliyor. (b) **article sub-PR split planning** — 13 caller'ı 2-3 alt-PR'a böl (her biri ≤ 8); ayrı mini-plan gerekir. (c) **Wave D T8-20 NOP "re-verify" PR** — düşük değerli; deferred. **Behavior-preserving:** PR-T8-11a worktree pre-flight'a girmedi; kod main'e gönderilmedi; throwaway worktree force-removed; production state pre-T8-10 path'lerinde (T8-10 deploy'lu); DB/migration/runtime hiç değişmedi. **T8 cycle status (24 PR + 2 revert + 10 başarılı + 2 deferred + 1 local-proof FAIL):** T8-1 v1 #1298 reverted (v68) → T8-PRE-1 v1 #1301 reverted (v69) → T8-PRE-1 v2 ✅ (v70) → T8-1 v2 ✅ (v71) → T8-2 ✅ (v72) → T8-3 ✅ (v73) → T8-4 ✅ (v74) → T8-5 ✅ (v75) → T8-6 ✅ (v76) → T8-7 🛑 DEFERRED (v77 — core/ cost_tracker) → Option B local proof FAIL (v78) → T8-8 ✅ (v79) → T8-9 ✅ (v80) → T8-10 ✅ (v81) → **T8-11a 🛑 DEFERRED (v82 bu closure — domain modules ops/ contract analog)**. **Master Plan Progress Snapshot:** T6/Phase 7b/Phase 8/Phase 8.2 ✅ closed; **T8 10/22 PR ✅ (~45%)** + 2 deferred (T8-7 + T8-11a) + 1 FAIL; T7 [#1086](https://github.com/selmanays/nodrat/issues/1086) OPEN — **artık kritik prerequisite** (T8 SAFE pool depleted; her ilerleme T7 unblock veya sub-PR split planning bekliyor); Phase 8.1+ DEFERRED; Phase 8.3 DEFERRED. **Genel: ileri aşama; T8 SOFT CEILING ~45%** — T7 başlatılmadan veya article sub-PR split planlanmadan ilerleme yok. **Locked policy reaffirmed:** Option C YASAK; Option D otomatik başlamayacak; consumer-layer risk classification kalıcı; pre-PR audit zorunlu (v82 yeni adım: target module forbidden contract cross-reference); T8-7 + T8-11a DEFERRED; production smoke read-only. **Hard kural takibi (kullanıcı 2026-05-26+27+28):** docs/wiki sync cycle (v82 bu closure) tamamlandıktan sonra T8 implementation DUR — kullanıcıya rapor. **Sıradaki (kullanıcı kararı bekleniyor):** (1) **T7 cost_tracker initiative başlat** (7 BLOCKED model unblock yolu — yetki + scope onayı gerek); veya (2) **article sub-PR split mini-plan** (13 caller'ı 2-3 alt-PR; ayrı planning); veya (3) **T8 ilerlemesine duraksat** ve diğer initiative'lere geç (Phase 8.1+ vb.); veya (4) **T8-20 NOP "re-verify" PR** — düşük değerli ama bir cycle ilerletir. -->
<!-- next: v82 merge → kullanıcı kararı bekleniyor (T7 initiative veya article sub-PR split planning veya T8 implementation pause). -->

<!-- v81 (önceki — context için): ✅ **T8-10 ✅ TAMAMLANDI — Wave D ısınma (AgendaCard → modules/agenda/models.py, NEW module scaffold)** — v78 Option B FAIL sonrası T8 risk-classified mode altında **3. başarılı safe candidate** (T8-8 event, T8-9 research_cluster sonrası). PR [#1324](https://github.com/selmanays/nodrat/pull/1324) `3b1f17f` 2026-05-27 23:08 UTC merged → main CI **11/11 GREEN** + Deploy.yml **FULL success** + `/health=200` + production stable + read-only smoke OK. **`AgendaCard` ORM modeli artık `app/modules/agenda/models.py`'de** (önceden `app/models/agenda.py`); 100% rename, 173 satır, history preserved. **NEW module package scaffold:** `apps/api/app/modules/agenda/__init__.py` (lazy: docstring + `__all__: list[str] = []`) + `README.md` (Layout + Migration history). Kullanıcı locked decision (2026-05-26 v67) + master plan §2.4: `agenda` AYRI modül (generations'a değil). **Wave D ısınma** (Wave C T8-8/T8-9 sonrası Wave D'nin ilk başarılı PR'ı). **7 dosya değişiklik** (caller bütçesi ≤ 8): rename + facade + 3 caller flip + scaffold __init__.py + scaffold README + ruff isort auto-fix. **3 caller flip (eager all):** (a) `apps/api/app/api/admin_queue.py:46` — admin queue observation (FailedJob+AdminAuditLog dashboard'unda agenda kart durumu); (b) `apps/api/app/modules/generations/tasks/agenda.py:31` — Celery task agenda card generator pipeline; (c) `apps/api/app/modules/rag/tasks/raptor.py:29` — Celery task weekly summary (AgendaCard for hierarchical RAPTOR clustering). **Pre-PR audit (kullanıcı v78 hard kuralı §1) SAFE:** 3 production ORM caller + 0 core/ + 0 shared/; modules/agenda NEW module (B-group; `_purge_cached_modules` listesinde DEĞİL — T8-6 ders tetiklenmez); AgendaCard FK by table name (`event_clusters.id` CASCADE + `agenda_cards.id` SET NULL self-ref); `relationship()` YOK (yalnız FK column'ları — T8 string-form N/A); Vector(1024) embedding Phase 8.2 PR-8.2-11 hardened — class taşıma data-safety etkilemez. **Local pre-flight (8/8 PASS — kalıplaşmış matris, 9. iterasyon):** ruff ✅ (1 isort auto-fix) / 5-form grep `app.models.agenda` 0 stale ✅ / facade identity OK (`AgendaCard` facade ≡ `app.modules.agenda.models`; `__tablename__=agenda_cards`) ✅ / lint-imports **16/16 kept, 0 broken** ✅ (file count 202 → 203) / mapper_resolution 3/3 ✅ / module_init_lazy 9/9 ✅ / test_admin_rag --collect-only 10 NO ImportError ✅ / **TAM `pytest tests/unit/` 1186 passed 47.16s** ✅. **Behavior-preserving:** no migration write, no DB schema change, veri güvenliği invariant korunur (no rechunk/reembed/backfill; `agenda_cards` tablosuna dokunulmadı; Vector(1024) Phase 8.2-11 hardened; FK references unchanged; INSERT/UPDATE/UPSERT SQL string'leri caller'larda AYNEN; Celery task names `tasks.agenda.*` + `embedding_queue`/`event_queue` AYNEN). **Pattern T8-1 v2 + T8-2..T8-6 + T8-8 + T8-9 + T8-10 = 9 iterasyonda kalıcı.** **T8 cycle status (23 PR + 2 revert + 10 başarılı + 1 deferred + 1 local-proof FAIL):** T8-1 v1 #1298 reverted (v68) → T8-PRE-1 v1 #1301 reverted (v69) → T8-PRE-1 v2 ✅ (v70) → T8-1 v2 ✅ (v71) → T8-2 ✅ (v72) → T8-3 ✅ (v73) → T8-4 ✅ (v74) → T8-5 ✅ (v75) → T8-6 ✅ (v76) → T8-7 🛑 DEFERRED (v77) → Option B FAIL (v78) → T8-8 ✅ (v79) → T8-9 ✅ (v80) → **T8-10 #1324 ✅ DONE (v81 bu closure — Wave D ısınma)**. **Master Plan Progress Snapshot:** T6/Phase 7b/Phase 8/Phase 8.2 ✅ closed; **T8 10/22 PR ✅ (~45%)** + 1 deferred + 1 FAIL; T7 [#1086](https://github.com/selmanays/nodrat/issues/1086) OPEN prerequisite (7 BLOCKED model unblocker); Phase 8.1+ DEFERRED; Phase 8.3 DEFERRED. **Genel: ileri aşama**; ana kalan iş = T8 ~55% (12 PR) + T7 prerequisite. **Locked policy reaffirmed:** Option C YASAK; Option D otomatik başlamayacak; consumer-layer risk classification kalıcı; pre-PR audit zorunlu; T8-7 DEFERRED; production smoke read-only. **Hard kural takibi (kullanıcı 2026-05-26+27+28):** docs/wiki sync cycle (v81 bu closure) tamamlanmadan sonraki safe candidate'a (PR-T8-11 vb.) geçilmez. **Sıradaki:** PR-T8-11 next safe candidate seçimi (öneriler: (1) `failed_job` + `admin_audit_log` — T8-7 partial split; ProviderCallLog DEFERRED kalır ama job models SAFE olabilir — caller bütçesi audit gerek (>8 olabilir); (2) `article` — Wave C 13 caller sub-PR split — büyük scope; (3) Wave D vector hardening (T8-19 article.summary_embedding + T8-20 event.embedding idempotent re-verify) — HIGH RISK ama Phase 8.2'de ORM hardened; (4) `cluster_assignment` N/A model dosyası yok). Paralelde T7 cost_tracker initiative kullanıcı yetki + scope onayı bekliyor. -->
<!-- next: v81 merge → PR-T8-11 (öneri: failed_job+admin_audit_log sub-PR split veya Wave D vector hardening); T7 cost_tracker user yetki bekliyor. -->

<!-- v80 (önceki — context için): ✅ **T8-9 ✅ TAMAMLANDI — Wave C continued (research_cluster → modules/generations/models.py)** — v78 Option B FAIL sonrası T8 risk-classified mode altında **2. başarılı safe candidate** (T8-8 event sonrası). PR [#1322](https://github.com/selmanays/nodrat/pull/1322) `54083e9` 2026-05-27 22:45 UTC merged → main CI **11/11 GREEN** + Deploy.yml **FULL success** + `/health=200` + production stable + read-only smoke OK. **`ResearchCluster` + `MessageCluster` ORM modelleri artık `app/modules/generations/models.py`'de** (önceden `app/models/research_cluster.py`); 100% rename 149 satır history preserved. `app/models/__init__.py` facade `from app.modules.generations.models import MessageCluster, ResearchCluster` formuyla re-export ediyor. **Master plan §2.4 revize (2026-05-20) ownership = generations** kararı uygulandı. **6 dosya** (caller bütçesi ≤ 8): rename + facade + 3 caller flip + README + ruff isort auto-fix. **3 caller flip (eager all):** (a) `apps/api/app/api/admin_clusters.py:32` admin observation route; (b) `apps/api/app/api/app_me.py:50` KVKK md.11 export; (c) `apps/api/app/modules/generations/tasks/cluster_assigner.py:52` **same-module flip**. **3-aday audit triage:** research_cluster ✅ SEÇİLDİ (3 caller, 0 core/, target lazy + B-group, same-module easy flip, §2.4 ownership clear); **email 🛑 BLOCKED** (`shared/* must not import legacy core/api/models` ihlali — `Base` zinciri yasak; Base relocation prerequisite); agenda ⏸️ DEFER (Wave D HIGH risk vector(1024); NEW `modules/agenda/` scaffold gerekiyor). **🚨 YENİ KEŞIF (refactor-pr-checklist eklenebilir): shared/email → models.py taşıma Base relocation prerequisite.** **Local pre-flight 8/8 PASS — 8. iterasyon:** ruff ✅ (2 isort) / 5-form grep 0 stale ✅ / facade identity OK ✅ / lint-imports 16/16 ✅ / mapper 3/3 ✅ / module_init_lazy 9/9 ✅ / collect-only 10 NO ImportError ✅ / **TAM `pytest tests/unit/` 1186 passed 46.34s** ✅. **Behavior-preserving:** no migration write, no DB schema change, veri güvenliği invariant korunur (no rechunk/reembed/backfill; `research_clusters`+`message_clusters` tablolarına dokunulmadı; UPSERT pipeline + parent edges hierarchy AYNEN; `relationship()` string-form korunur; Celery task names `tasks.research_clustering.*` + `embedding_queue` AYNEN). **Pattern T8-1 v2 + T8-2..T8-6 + T8-8 + T8-9 = 8 iterasyonda kalıcı.** **T8 cycle status (22 PR + 2 revert + 9 başarılı + 1 deferred + 1 local-proof FAIL):** T8-1 v1 #1298 reverted (v68) → T8-PRE-1 v1 #1301 reverted (v69) → T8-PRE-1 v2 ✅ (v70) → T8-1 v2 ✅ (v71) → T8-2 ✅ (v72) → T8-3 ✅ (v73) → T8-4 ✅ (v74) → T8-5 ✅ (v75) → T8-6 ✅ (v76) → T8-7 🛑 DEFERRED (v77) → Option B FAIL (v78) → T8-8 ✅ (v79) → **T8-9 #1322 ✅ DONE (v80 bu closure)**. **Master Plan Progress Snapshot:** T6 ✅ closed (2026-05-23); Phase 7b ✅ closed; Phase 8 ✅ closed; Phase 8.2 ✅ closed; **T8 9/22 PR ✅ (~41%)** + 1 deferred + 1 local-proof FAIL; T7 [#1086](https://github.com/selmanays/nodrat/issues/1086) OPEN prerequisite (7 BLOCKED model unblocker); Phase 8.1+ DEFERRED; Phase 8.3 öneri DEFERRED. Genel: ileri aşama; ana kalan iş = T8 (~59%) + T7 prerequisite. **Locked policy reaffirmed:** Option C YASAK; Option D otomatik başlamayacak; consumer-layer risk classification kalıcı; pre-PR audit zorunlu; T8-7 DEFERRED; production smoke read-only. **Hard kural takibi (kullanıcı 2026-05-26+27+28):** docs/wiki sync cycle (v80 bu closure) tamamlanmadan sonraki safe candidate'a (PR-T8-10 vb.) geçilmez. **Sıradaki:** PR-T8-10 next safe candidate seçimi (öneri: `agenda` model — Wave D yeni `modules/agenda/` scaffold gerekiyor; AgendaCard ORM tek class; 3 caller pre-PR audit zorunlu) veya alternatif Wave D vector hardening adayları. Paralelde T7 cost_tracker initiative kullanıcı yetki + scope onayı bekliyor. -->
<!-- next: v80 merge → PR-T8-10 (öneri: agenda model NEW modules/agenda scaffold); T7 cost_tracker user yetki bekliyor. -->

<!-- v79 (önceki — context için): ✅ **T8-8 ✅ TAMAMLANDI — Wave C ısınma (event → modules/clusters/models.py)** — v78 Option B FAIL sonrası T8 risk-classified mode altında **ilk safe candidate**. PR [#1320](https://github.com/selmanays/nodrat/pull/1320) `d331865` 2026-05-27 22:11 merged → main CI **11/11 GREEN** (run `26541773543` ≈3 dk) + Deploy.yml **FULL success** (run `26541919349` ≈2 dk: Detect=success + Deploy_to_VPS=success) + `/health=200` + production stable. **`EventCluster` + `EventArticle` ORM modelleri artık `app/modules/clusters/models.py`'de** (önceden `app/models/event.py`); 100% rename, 156 satır, history preserved. `app/models/__init__.py` facade `from app.modules.clusters.models import EventArticle, EventCluster` formuyla re-export ediyor. **Wave C ısınma ✅** — Option B FAIL sonrası ilk başarılı T8 PR (safe candidate). **4 dosya değişiklik** (caller bütçesi ≤ 8): (a) rename `models/event.py → modules/clusters/models.py`, (b) facade `app/models/__init__.py` re-export, (c) `apps/api/app/modules/generations/tasks/agenda.py:32` caller flip (eager `from app.modules.clusters.models import EventCluster`; `EventArticle` agenda.py'da kullanılmıyor), (d) `modules/clusters/README.md` Layout + Migration history T8-8 entry. **Pre-PR audit (kullanıcı v78 hard kuralı §1):** ORM caller scan 1 production + 1 facade self-ref + 0 core/ + 0 api/ (raw SQL `FROM event_clusters/event_articles` var ama tablo adı; class taşınmadı → unaffected); modules/clusters/__init__.py LAZY (docstring + `__all__: list[str] = []`); B-group → `_purge_cached_modules` listesinde DEĞİL → T8-6 dersi tetiklenmez; SAFE. **Local pre-flight (8/8 PASS — kalıplaşmış matris, 7. iterasyon):** ruff ✅ / 5-form grep `app.models.event` 0 stale ✅ / facade identity `EventCluster`+`EventArticle` ≡ modules path ✅ / lint-imports **16/16 kept, 0 broken** ✅ / mapper_resolution 3/3 ✅ / module_init_lazy 9/9 ✅ / test_admin_rag --collect-only 10 NO ImportError ✅ / **TAM `pytest tests/unit/` 1186 passed 46.60s** ✅. **Behavior-preserving:** no migration write, no DB schema change, veri güvenliği invariant korunur (no rechunk/reembed/backfill; `event_clusters`+`event_articles` tablolarına dokunulmadı; `relationship()` string-form korunur — `EventCluster.articles` ↔ `EventArticle.event` back_populates AYNEN). Raw SQL caller'lar (`core/data_sufficiency`, `core/retrieval`, `api/admin_rag`) etkilenmedi → tablo adı sabit. **Pattern T8-1 v2 + T8-2 + T8-3 + T8-4 + T8-5 + T8-6 + T8-8 = 7 iterasyonda kalıcı.** **T8 cycle status (20 PR + 2 revert + 8 başarılı + 1 deferred + 1 local-proof FAIL):** T8-1 v1 #1298 reverted (v68) → T8-PRE-1 v1 #1301 reverted (v69) → T8-PRE-1 v2 #1304 ✅ (v70) → T8-1 v2 #1306 ✅ (v71) → T8-2 #1308 ✅ (v72) → T8-3 #1310 ✅ (v73) → T8-4 #1312 ✅ (v74) → T8-5 #1314 ✅ (v75) → T8-6 #1316 ✅ (v76) → T8-7 🛑 DEFERRED (v77) → Option B local proof FAIL (v78) → **T8-8 #1320 ✅ DONE (v79 bu closure)**. **Hard kural takibi (kullanıcı 2026-05-26+27+28):** docs/wiki sync cycle (v79 closure) tamamlanmadan sonraki safe candidate'a (PR-T8-9 vb.) geçilmez. **Caller analizi (post-PR):** `EventCluster`+`EventArticle` ORM direct importer = 1 production (agenda.py); 5-form grep post-PR 0 stale; facade re-export çalışıyor (production identity check OK). **Caller estimate adjustment (v78 audit → v79 reality):** v78 audit "event 1 caller; clusters/tasks/event.py target" demişti — gerçek caller `modules/generations/tasks/agenda.py:32` (cross-module read; clusters lower domain, generations upper layer); target seçimi değişmedi (`modules/clusters/` event-clustering domain'i olarak doğru). Ayrıca event.py 2 ORM class (EventCluster + EventArticle) içeriyordu — v78 audit "1 class" da yanlıştı; ama caller flip tek class `EventCluster` ile yetti (EventArticle agenda.py'da kullanılmıyor). **Sıradaki:** PR-T8-9 next safe candidate (öneri: `agenda` model — 2 caller, Wave D yeni `agenda` modülü scaffold, vector hardening; alternatif `cluster_assignment` raw SQL — caller analizi gerek). T8 risk-classified mode AKTİF; pre-PR audit her safe candidate için zorunlu. **Diğer açık öncelikler:** (a) **T7 cost_tracker initiative** [#1086](https://github.com/selmanays/nodrat/issues/1086) — 7 BLOCKED model unblock yolu (kullanıcı yetki + scope onayı bekliyor); (b) **Phase 8.1+** core/api code migration; (c) **PR-8c-2/3/4** docs/engineering refresh (kullanıcı `docs/` yetki bekliyor); (d) **Phase 8.3** raw-SQL only tables → ORM stub. -->
<!-- next: v79 merge → next safe T8 candidate (öneri: agenda model 2 caller; alternatif cluster_assignment raw SQL veya event done bekleniyor). -->

<!-- v78 (önceki — context için): 🔬 Option B local proof RESULT: **FAIL** — facade path import-linter contract'ı tatmin ETMEZ; transitive analiz `app.models` üzerinden `app.modules.*` zincirini yakaladı. Kullanıcı kararı (Option E + B-local-proof) tamamlandı; **T8 risk-classified mode KALICI** (consumer-layer policy v77'de locked). Bu PR docs-only — Option B FAIL kayda geçer + comprehensive audit (7 BLOCKED + 7 SAFE) + sıradaki safe T8 candidate seçimi. **Option B test setup (main worktree'de, no commit/push):** (1) `git mv apps/api/app/models/provider_log.py apps/api/app/modules/ops/models.py` (T8-7 simulation), (2) `app.models.__init__.py` facade re-export: `from app.modules.ops.models import ProviderCallLog`, (3) `apps/api/app/core/cost_tracker.py:35` facade path: `from app.models import ProviderCallLog` (direct submodule yerine). **Sonuç (lint-imports output):** `app.core is not allowed to import app.modules: app.core.cost_tracker -> app.models (l.35), app.models -> app.modules.style_profiles.models (l.32)` + 3 transitive violations (`media/rag/style_profiles tasks → cost_tracker → app.models → app.modules.ops.models`). **Critical insight (v78 capstone):** `app.models.__init__.py` artık T8-1..T8-6 facade re-exports ile **POISONED**. Lines 25-32 6 farklı modülden import ediyor (`legal`, `ops`-future-T8-7, `prompts_admin`, `rag`, `settings_admin`, `sft`, `style_profiles`). Core/ herhangi bir `from app.models import X` kullanırsa, import-linter `cost_tracker → app.models → app.modules.X.models` transitive zincirini her T8-completed modül için detect ediyor. **Sonuç:** facade path **ÇÖZÜM DEĞİL** — T8 model relocation + core/ consumer + Phase 8 boundary contract birbirleriyle çatışır; her iki taraf da düzelmedikçe T8-7 ve diğer 6 BLOCKED model gerçekten DEFERRED. **🗺️ Comprehensive audit (kullanıcı instructed step 3):** `grep -rn "from app.models.<modul>" apps/api/app/core apps/api/app/api apps/api/app/modules` ile her remaining flat model taraması: **🛑 7 BLOCKED (core/ consumer):** provider_log (cost_tracker), billing (plan_features), conversation (conversation_context — **YENİ KEŞIF**), generation (quota), research_cache_telemetry, source (polling_tier — **YENİ KEŞIF**), user (deps + plan_features). v77'de 5 olarak tahmin edilmişti; audit ile 7 KONFIRME. **✅ 7 SAFE (T8 ilerleyebilir):** email (0 consumers — T8-9 scaffold candidate), job (5 api/ + 8 modules/; T8-7 partial — FailedJob+AdminAuditLog, provider_log left as deferred; 13 caller > 8 → sub-PR split), agenda (3 callers; Wave D YENİ), article (13 callers; Wave C, sub-PR split), event (1 caller; Wave C, en küçük), research_cluster (3 callers; Wave C), agency (0 consumers — orphan? read-only audit gerek). **Triage decision (per kullanıcı kuralı step 3+5):** T8 risk-classified mode KALICI; yeni "T7/core-consumer cleanup prerequisite" mini-plan önerisi v78 docs'da; sonraki T8 PR safe-only candidate'den seçilecek (en küçük scope = event.py 1 caller). **Yeni hard kural takibi:** pre-PR audit her T8 PR için zorunlu (refactor-pr-checklist §"Pre-PR core/ consumer audit (T8-7 v77 dersi)"). **Behavior-preserving:** Option B test yalnız worktree'de yapıldı; commit/push YOK; throwaway worktree force-removed. Production state pre-T8-7 path'lerinde KALMAYA devam. **T8 cycle status (18 PR + 2 revert + 7 başarılı + 1 deferred + Option B FAIL kayda):** T8-1..T8-6 ✅; T8-7 🛑 DEFERRED (v77); Option B FAIL (v78 kayda). **Sıradaki:** v78 closure merge + watcher → **PR-T8-X** seçimi (event.py önerilir — en küçük scope, 1 caller, Wave C başlangıç; eski Wave B sequence partial-complete kabul edilir; alternatif: T8-9 email scaffold YENİ ama scope analizi gerek). **Hard kural takibi (kullanıcı 2026-05-26):** docs/wiki sync cycle (bu PR — v78 closure) tamamlanmadan sonraki implementation'a (T8-X safe candidate) geçilmez. -->
...
## [2026-05-28] t8-11a-deferred-hard-stop-v82 | 🛑 T8-11a HARD-STOP — DEFERRED (NOT FAILED) — `domain modules must not import ops/` contract analog of v77 core/ pattern

- **Bağlam:** v81 closure (T8-10 agenda) merge sonrası PR-T8-11 candidate seçimi için triage. failed_job + admin_audit_log T8-7 partial split olarak değerlendirildi (ProviderCallLog DEFERRED kalır v77; FailedJob+AdminAuditLog SAFE olabilir hipotezi).
- **🛑 HARD-STOP DİSCOVERY (v82 capstone):** FailedJob → `modules/ops/models.py` taşıma `domain modules must not import ops/` import-linter contract'ını **ihlal eder**. v77/v78'in `core/* must not import modules/*` paterninin **analog'u** — bu sefer domain modules consumer'lar sorun.
- **FailedJob caller breakdown (5 production + 2 test):**
  - `apps/api/app/modules/articles/tasks/articles.py:53` (module-level eager) — **articles modülü domain modules contract forbidden source list'inde** 🛑
  - `apps/api/app/modules/sources/tasks/sources.py:195` (lazy in-function) — **sources modülü contract forbidden source list'inde** 🛑
  - `apps/api/app/api/admin_queue.py:47` (api/ → ops izinli, contract scope dışında) ✅
  - `apps/api/tests/integration/test_record_failure_539.py:15` (test)
  - `apps/api/tests/unit/test_admin_queue.py:237` (test, inside function)
- **Import-linter contract reference:**
  ```toml
  [[tool.importlinter.contracts]]
  name = "domain modules must not import ops/"
  type = "forbidden"
  source_modules = [
      "app.modules.sources",       # 🛑 FailedJob importer
      "app.modules.articles",      # 🛑 FailedJob importer
      "app.modules.crawler",
      "app.modules.rag",
  ...
  ```
- **Conclusion:** FailedJob → modules/ops/models.py taşıması iki domain modules import-linter edge'i yaratır:
  - `app.modules.articles → app.modules.ops` (NEW forbidden edge)
  - `app.modules.sources → app.modules.ops` (NEW forbidden edge)

  Bu hard-stop — Option C/`ignore_imports` yasak; hard kural §1 + import-linter contract break.

- **🚨 AdminAuditLog daha kötü durumda — DOUBLE BLOCK:**
  - **Caller > 8:** 11 production caller (admin_audit + admin_queue + admin_system lazy + admin_users + app_me + 5 modül admin routes) → caller budget aşar (kullanıcı hard kuralı: > 8 = SPLIT)
  - **Domain modules ops/ ihlali:** articles/admin, legal/routes, prompts_admin/routes, settings_admin/routes, sft/admin/routes, sources/admin/routes — hepsi domain modules contract forbidden source list'inde 🛑
- **3-aday triage sonucu:**
  | Aday | Sonuç | Sebep |
  |---|---|---|
  | FailedJob → ops | 🛑 **BLOCKED** | domain modules ops/ contract ihlali (articles + sources) |
  | AdminAuditLog → ops | 🛑 **DOUBLE BLOCK** | caller > 8 + domain modules ops/ contract (6 modül) |
  | ProviderCallLog → ops | 🛑 **BLOCKED (v77)** | core/* must not import modules/* (cost_tracker:35) |
- **Other remaining models audit (12 → blocked/deferred):**
  - **`article`** — Wave C 13 caller sub-PR split gerekiyor (büyük scope; ayrı planning)
  - **`billing` (Plan, Subscription, etc.)** — core/ consumer BLOCKED (plan_features)
  - **`conversation`** — core/ consumer BLOCKED (conversation_context — v78 keşfi)
  - **`email`** — shared/Base prerequisite BLOCKED (v80 keşfi: `shared/* must not import legacy core/api/models`)
  - **`generation` (UsageEvent)** — core/ consumer BLOCKED (quota)
  - **`provider_log`** — core/ consumer BLOCKED v77 (cost_tracker)
  - **`research_cache_telemetry`** — core/ consumer BLOCKED (lazy import)
  - **`source` (Source, SourceConfig, SourceHealth)** — core/ consumer BLOCKED (polling_tier — v78 keşfi)
  - **`user`/Session** — core/ consumer BLOCKED (deps); 28 caller; alt-PR sequence
  - **`failed_job` + `admin_audit_log`** — domain modules ops/ BLOCKED v82 (bu)
- **Wave D vector hardening durumu:**
  - **T8-19 (article.summary_embedding)** — BLOCKED, T8-12 (article move) prereq, article 13 caller
  - **T8-20 (EventCluster.embedding re-verify)** — class T8-8'de modules/clusters/models.py'ye taşındı + Phase 8.2-11 ORM hardened → re-verify pratik olarak NOP PR (her PR'ın mapper_resolution + TAM unit suite zaten doğruluyor); minimal value
- **🔑 v82 capstone insight (refactor-pr-checklist'e eklenecek — NEW lesson):**
  - **5 hard-stop tetiği listesi (kümülatif v77+v78+v80+v82):**
    1. Target module core/ tarafından import ediliyorsa → `core/* must not import modules/*` ihlali (v77)
    2. Facade path da `core/* → app.models → app.modules.*` transitive ihlali (v78 — POISONED facade insight)
    3. Target module domain modules tarafından import ediliyorsa ve target import-linter `domain modules must not import <target>/` contract'ında ise → **v82 yeni (bu)**
    4. Target shared/* paketinde ve model `from app.models.base import Base` import ediyorsa → `shared/* must not import legacy core/api/models` ihlali (v80 email keşfi)
    5. Caller bütçesi > 8 dosya → sub-PR split zorunlu (kullanıcı v78 hard kuralı)
  - **Pre-PR audit yeni adım (v82):** target module bir import-linter forbidden contract'ında (e.g. `domain modules must not import ops/`) ise, her caller modülünü contract `source_modules` listesi ile cross-reference yap. Spesifik komut:
    ```bash
    git grep -lE "from app.models.X|from app.models import .*\bClassName\b" apps/api/app/modules \
      | xargs -I {} bash -c 'echo "Check: {}"; grep -F "app.modules.$(echo {} | cut -d/ -f5)" apps/api/pyproject.toml'
    ```
- **T8 SAFE pool DEPLETED at 10/22 (~45%):** Her remaining candidate hard-stop'ta. T8 implementation cycle SOFT CEILING. İlerleme için:
  - (a) **T7 cost_tracker initiative başlat** — 7 BLOCKED model unblock yolu (cost_tracker + plan_features + quota + deps + research_cache_telemetry + conversation_context + polling_tier core/ → modules/services/ taşınma); kullanıcı yetki + scope onayı bekliyor
  - (b) **article sub-PR split mini-plan** — 13 caller'ı 2-3 alt-PR'a böl (her biri ≤ 8); ayrı planning gerek
  - (c) **T8 implementation pause** — diğer initiative'lere geç (Phase 8.1+ core/api code migration veya 8.3 raw-SQL ORM stub)
  - (d) **T8-20 NOP "re-verify" PR** — düşük değerli ama bir cycle ilerletir; opsiyonel
- **Behavior-preserving:**
  - PR-T8-11a worktree pre-flight'a girmedi (audit aşamasında hard-stop tetiklendi)
  - kod main'e gönderilmedi
  - throwaway worktree force-removed (cleanup yapıldı)
  - production state pre-T8-10 path'lerinde (T8-10 deploy'lu)
  - DB/migration/runtime hiç değişmedi
  - veri güvenliği invariant KORUNDU (hard-stop kod-zamanı; runtime etki yok)
- **T8 cycle status (24 PR + 2 revert + 10 başarılı + 2 deferred + 1 local-proof FAIL):**
  - T8-1 v1 #1298 reverted (v68) → T8-PRE-1 v1 #1301 reverted (v69) → T8-PRE-1 v2 ✅ (v70)
  - → T8-1 v2 ✅ (v71) → T8-2 ✅ (v72) → T8-3 ✅ (v73)
  - → T8-4 ✅ (v74) → T8-5 ✅ (v75) → T8-6 ✅ (v76)
  - → T8-7 🛑 DEFERRED (v77 — core/ cost_tracker)
  - → Option B local proof FAIL (v78 — POISONED facade insight)
  - → T8-8 ✅ (v79 — Wave C ısınma event)
  - → T8-9 ✅ (v80 — Wave C continued research_cluster; email BLOCKED keşfi)
  - → T8-10 ✅ (v81 — Wave D ısınma agenda)
  - → **T8-11a 🛑 DEFERRED (v82 bu closure — domain modules ops/ contract analog; v82 yeni hard-stop pattern)**.

### Master Plan Progress Snapshot (v82 itibarıyla — T8 SOFT CEILING)

| Alan | Durum | Not |
|---|---|---|
| T6 | ✅ CLOSED | God-file cycle TAM (2026-05-23) |
| Phase 7b | ✅ CLOSED | Frontend god-pages mini-cycle TAM |
| Phase 8 | ✅ CLOSED | 16 import-linter contract (T8-11a hard-stop tetiği — `domain modules must not import ops/`) |
| Phase 8.2 | ✅ CLOSED | ORM completion + alembic check strict gate |
| **T8** | 🔄 **SOFT CEILING ~45% (10/22 PR ✅)** | Wave A 3/3 + Wave B 3/6 + Wave C 2/7 + Wave D 1/6; 2 deferred (T8-7 + T8-11a); SAFE pool DEPLETED |
| **T7** ([#1086](https://github.com/selmanays/nodrat/issues/1086)) | 🔓 **OPEN — KRİTİK prerequisite** | Artık T8 ilerlemesi için tek unblock yolu; 7 BLOCKED model + 2 ops-blocked model; kullanıcı yetki + scope onayı bekliyor |
| Phase 8.1+ | ⏸️ DEFERRED | core/api code migration; T8 SOFT CEILING sonrası alternative initiative |
| Phase 8.3 | ⏸️ DEFERRED | Raw-SQL → ORM stub; alternative initiative |
| **Genel** | **İleri aşama; T8 SOFT CEILING** | T8 ilerleme T7 başlatılmadan veya article sub-PR planlanmadan duracak |

- **Sıradaki kullanıcı kararı bekleniyor** (autonomous mod hard-stop ile dur):
  1. **T7 cost_tracker initiative başlat** (yetki + scope onayı gerek)
  2. **article sub-PR split mini-plan** (planning)
  3. **T8 pause + Phase 8.1+ veya 8.3** (alternative initiative)
  4. **T8-20 NOP "re-verify" PR** (minimal value, opsiyonel)
- **Hard kural takibi (kullanıcı 2026-05-26+27+28):** docs/wiki sync cycle (v82 — bu PR) tamamlandıktan sonra T8 implementation DUR — kullanıcı kararı bekleniyor. T8-7 + T8-11a DEFERRED kalır. Option C/`ignore_imports` yasak. Option D/T7 core-consumer cleanup otomatik başlamayacak (kullanıcı yetki gerek).
- **Etkilenen sayfalar:** [[t8-model-relocation-mini-plan]] (Wave B T8-7 satırı v82 update + double-block notu; v82 frontmatter update; TL;DR), [[refactor-pr-checklist]] (NEW lesson "Pre-PR domain-modules ops/ consumer audit (T8-11a v82 dersi)"; 5 hard-stop tetiği listesi), index.md stats line.

## [2026-05-28] t8-10-done-v81 | ✅ T8-10 ✅ TAMAMLANDI — Wave D ısınma (AgendaCard → modules/agenda/models.py, NEW module scaffold)

- **Bağlam:** v78 Option B FAIL sonrası T8 risk-classified mode aktif. v80 closure (T8-9 research_cluster) merge sonrası **3. başarılı safe candidate** olarak `agenda` model seçildi. Önerilen Wave D ısınma; kullanıcı locked decision v67 + master plan §2.4 doğrultusunda agenda AYRI modül scaffold edildi.
- **PR:** [#1324](https://github.com/selmanays/nodrat/pull/1324) `3b1f17f` 2026-05-27 23:08 UTC merged.
- **Main CI:** **11/11 GREEN** + read-only smoke OK.
- **Deploy.yml:** **FULL success** (Detect=success + Deploy_to_VPS=success); code change → production deploy gerçekleşti.
- **Smoke (read-only):** `curl https://nodrat.com/health` → `HTTP 200` (kullanıcı v78 hard kuralı §6 — no DB/RAG/embedding/manual trigger).
- **`AgendaCard` ORM modeli artık `app/modules/agenda/models.py`'de** (önceden `app/models/agenda.py`); 100% rename, 173 satır, history preserved.
- **NEW module package scaffold:**
  - `apps/api/app/modules/agenda/__init__.py` — lazy: docstring + `__all__: list[str] = []`
  - `apps/api/app/modules/agenda/README.md` — Layout + Migration history
- **Wave D ısınma ✅** — T8-8/T8-9 Wave C sonrası Wave D'nin ilk başarılı PR'ı.
- **7 dosya değişiklik** (+119 / -4; caller bütçesi 7/8):
  - (a) **`git mv apps/api/app/models/agenda.py → apps/api/app/modules/agenda/models.py`** (100% rename)
  - (b) **`apps/api/app/models/__init__.py`** — facade re-export: `from app.modules.agenda.models import AgendaCard  # T8-10: moved 2026-05-28`
  - (c) **`apps/api/app/api/admin_queue.py:46`** — caller flip (eager); admin queue observation
  - (d) **`apps/api/app/modules/generations/tasks/agenda.py:31`** — caller flip (eager); Celery task agenda card generator
  - (e) **`apps/api/app/modules/rag/tasks/raptor.py:29`** — caller flip (eager); Celery task weekly summary (RAPTOR clustering)
  - (f) **`apps/api/app/modules/agenda/__init__.py`** — NEW (lazy)
  - (g) **`apps/api/app/modules/agenda/README.md`** — NEW
- **Pre-PR audit raporu (kullanıcı v78 hard kuralı §1 — tüm checks geçti):**
  | Kontrol | Sonuç | Detay |
  |---|---|---|
  | ORM caller scan (5-form) | ✅ | 3 production (eager) + 1 facade self-ref |
  | core/ ORM consumer | ✅ **0** | (raw SQL refs sadece alembic migration — taşınmıyor) |
  | api/ ORM consumer | 1 | admin_queue.py:46 |
  | modules/ ORM consumer | 2 | generations/agenda + rag/raptor |
  | shared/ ORM consumer | ✅ 0 | — |
  | Target `modules/agenda/__init__.py` lazy | ✅ NEW LAZY | docstring + `__all__: list[str] = []` |
  | Facade poisoning (T8-6 ders) | ✅ N/A | `_MODULES_REQUIRING_LAZY_INIT` A-group fixed 8 modül; agenda DEĞİL → ders tetiklemez |
  | FK | event_clusters.id (CASCADE) + agenda_cards.id (self-ref SET NULL) | Table-name based; UNAFFECTED |
  | `relationship()` | YOK | yalnız FK column'ları; T8 string-form hard kural N/A |
  | Vector(1024) embedding | Phase 8.2 PR-8.2-11 hardened | class taşıma data-safety etkilemez |
- **Local pre-flight (8/8 PASS — kalıplaşmış matris, 9. iterasyon):**
  - ruff ✅ (1 isort auto-fix sonrası clean)
  - 5-form grep `app.models.agenda` 0 stale ref (a/b/c/d/e tüm formlar) ✅
  - facade identity OK — `AgendaCard` facade ≡ `app.modules.agenda.models` (Python `is` test); `__tablename__=agenda_cards`
  - lint-imports **16/16 kept, 0 broken** ✅ (file count 202 → 203)
  - mapper_resolution 3/3 PASS ✅
  - module_init_lazy 9/9 PASS ✅
  - test_admin_rag --collect-only 10 tests NO ImportError ✅
  - **TAM `pytest tests/unit/` 1186 passed 47.16s** ✅
- **Behavior-preserving (kullanıcı kuralları §4-6):**
  - no migration write, no DB schema change, no migration edit
  - veri güvenliği invariant korunur: no rechunk/reembed/backfill; `agenda_cards` tablosuna dokunulmadı
  - `Vector(1024)` embedding kolonu Phase 8.2-11 hardened — re-verify Wave D'de (T8-18 deferred, ayrı PR)
  - FK references (table-name based) unchanged
  - `INSERT/UPDATE/UPSERT agenda_cards` SQL string'leri caller'larda AYNEN
  - Celery task names korunur (`tasks.agenda.*` → `event_queue`/`embedding_queue`)
  - Production smoke read-only (no DB/RAG/embedding/manual trigger)
- **T8 cycle status (23 PR + 2 revert + 10 başarılı + 1 deferred + 1 local-proof FAIL):**
  - T8-1 v1 #1298 reverted (v68) → T8-PRE-1 v1 #1301 reverted (v69) → T8-PRE-1 v2 #1304 ✅ (v70)
  - → T8-1 v2 #1306 ✅ (v71) → T8-2 #1308 ✅ (v72) → T8-3 #1310 ✅ (v73)
  - → T8-4 #1312 ✅ (v74) → T8-5 #1314 ✅ (v75) → T8-6 #1316 ✅ (v76)
  - → T8-7 🛑 DEFERRED (v77 — `cost_tracker:35` core/ consumer)
  - → Option B local proof FAIL (v78 — POISONED facade insight)
  - → T8-8 #1320 ✅ DONE (v79 — Wave C ısınma — event family)
  - → T8-9 #1322 ✅ DONE (v80 — Wave C continued — research/message clusters)
  - → **T8-10 #1324 ✅ DONE (v81 bu closure — Wave D ısınma — agenda)**.
- **Pattern T8-1 v2 + T8-2..T8-6 + T8-8 + T8-9 + T8-10 = 9 iterasyonda kalıcı.**

### Master Plan Progress Snapshot (v81 itibarıyla)

| Alan | Durum | Not |
|---|---|---|
| T6 | ✅ CLOSED | God-file cycle TAM (2026-05-23); 251 char test |
| Phase 7b | ✅ CLOSED | Frontend god-pages mini-cycle TAM (2026-05-23) |
| Phase 8 | ✅ CLOSED | 16 import-linter contract aktif (`core/* must not import modules/*` T8-7 hard-stop tetiği) |
| Phase 8.2 | ✅ CLOSED | ORM completion 15 PR; alembic check strict gate ACTIVE; T8 5/5 ön-şart GREEN |
| **T8** | 🔄 **~45% (10/22 PR ✅)** | Wave A 3/3 + Wave B 3/6 + Wave C 2/7 + Wave D 1/6; pattern 9 iter kalıcı |
| T7 ([#1086](https://github.com/selmanays/nodrat/issues/1086)) | 🔓 OPEN — prerequisite | 7 BLOCKED model unblocker; kullanıcı yetki bekliyor |
| Phase 8.1+ | ⏸️ DEFERRED | core/api code migration |
| Phase 8.3 | ⏸️ DEFERRED | Raw-SQL → ORM stub |
| **Genel** | **İleri aşama; ana kalan iş T8 + T7** | T8 ~55% (12 PR) + T7 prerequisite + opsiyonel 8.1/8.3 |

- **Hard kural takibi (kullanıcı 2026-05-26+27+28):** docs/wiki sync cycle (v81 — bu PR) tamamlanmadan sonraki safe candidate'a geçilmez. **T8-7 (provider_log/job/cost_tracker) DEFERRED kalıyor.** Option C/`ignore_imports` yasak. Option D/T7 core-consumer cleanup otomatik başlamayacak.
- **Sıradaki:** PR-T8-11 next safe candidate (öneriler: (1) `failed_job` + `admin_audit_log` — T8-7 partial split; ProviderCallLog DEFERRED kalır ama job models SAFE olabilir (caller bütçesi audit gerek — 10+ caller olabilir → sub-PR split); (2) `article` Wave C 4/7 — 13 caller sub-PR split; (3) Wave D vector hardening T8-19/T8-20 idempotent re-verify (HIGH risk ama Phase 8.2-11/12 zaten ORM hardened); (4) cluster_assignment N/A model dosyası yok). Paralelde T7 cost_tracker initiative kullanıcı yetki + scope onayı bekliyor.
- **Etkilenen sayfalar:** [[t8-model-relocation-mini-plan]] (Wave D T8-18 → T8-10 ✅; frontmatter v81 update; TL;DR; Wave D tablo), [[refactor-pr-checklist]] (potansiyel: NEW module scaffold + B-group lazy discipline pattern), index.md stats line.

## [2026-05-28] t8-9-done-v80 | ✅ T8-9 ✅ TAMAMLANDI — Wave C continued (ResearchCluster + MessageCluster → modules/generations/models.py)

- **Bağlam:** v78 Option B local proof FAIL sonrası T8 risk-classified mode aktif. v79 closure (T8-8 event) merge sonrası **2. başarılı safe candidate** seçimi için 3-aday pre-PR audit yapıldı (agenda / cluster_assignment / research_cluster + bonus email). research_cluster SAFE bucket konfirme; email BLOCKED (yeni keşif); agenda DEFER (Wave D HIGH risk + scaffold).
- **PR:** [#1322](https://github.com/selmanays/nodrat/pull/1322) `54083e9` 2026-05-27 22:45 UTC merged.
- **Main CI:** **11/11 GREEN** + read-only smoke OK.
- **Deploy.yml:** **FULL success** (Detect=success + Deploy_to_VPS=success); code change → production deploy gerçekleşti.
- **Smoke (read-only):** `curl https://nodrat.com/health` → `HTTP 200` (kullanıcı v78 hard kuralı §6 — no DB/RAG/embedding/manual trigger).
- **`ResearchCluster` + `MessageCluster` ORM modelleri artık `app/modules/generations/models.py`'de** (önceden `app/models/research_cluster.py`); 100% rename, 149 satır, history preserved.
- **Master plan §2.4 revize (2026-05-20) ownership = generations** kararı uygulandı (clusters yerine generations; cluster_assigner.py burada zaten yaşıyor — same-module residence).
- **Wave C continued ✅** — T8-8 event sonrası Wave C 2. PR.
- **6 dosya değişiklik** (+20 / -5; caller bütçesi 6/8):
  - (a) **`git mv apps/api/app/models/research_cluster.py → apps/api/app/modules/generations/models.py`** (100% rename)
  - (b) **`apps/api/app/models/__init__.py`** — facade re-export: `from app.modules.generations.models import MessageCluster, ResearchCluster  # T8-9: moved 2026-05-28`
  - (c) **`apps/api/app/api/admin_clusters.py:32`** — caller flip (eager); admin observation route (Faz 3c salt-okuma, require_admin)
  - (d) **`apps/api/app/api/app_me.py:50`** — caller flip (eager); #1016 Pivot Faz 3b kullanıcı araştırma ilgi alanları (KVKK md.11 export)
  - (e) **`apps/api/app/modules/generations/tasks/cluster_assigner.py:52`** — caller flip (eager); **same-module flip** (en temiz import)
  - (f) **`apps/api/app/modules/generations/README.md`** — Layout (`models.py` eklendi) + Migration history T8-9 entry
- **3-aday audit triage (read-only):**
  | Aday | Sonuç | Sebep |
  |---|---|---|
  | **research_cluster** | ✅ **SEÇİLDİ** | 3 caller, 0 core/, target lazy + B-group, same-module easy flip, §2.4 ownership clear |
  | **email** | 🛑 **BLOCKED — YENİ KEŞİF** | `shared/* must not import legacy core/api/models` import-linter ihlali — `from app.models.base import Base` engelli; Base relocation prerequisite. T8-9 plan satırı (shared/email/models.py) BLOCKED kalır. |
  | **agenda** | ⏸️ **DEFER** | Wave D HIGH risk (vector(1024)); NEW `modules/agenda/` scaffold gerekiyor; daha sonra |
- **Local pre-flight (8/8 PASS — kalıplaşmış matris, 8. iterasyon):**
  - ruff ✅ (2 isort auto-fix)
  - 5-form grep `app.models.research_cluster` 0 stale ref (a/b/c/d/e tüm formlar) ✅
  - facade identity OK ✅ — `ResearchCluster`/`MessageCluster` facade ≡ `app.modules.generations.models` (Python `is` test); `__tablename__=research_clusters`+`message_clusters`
  - lint-imports **16/16 kept, 0 broken** ✅
  - mapper_resolution 3/3 PASS ✅
  - module_init_lazy 9/9 PASS ✅
  - test_admin_rag --collect-only 10 tests NO ImportError ✅
  - **TAM `pytest tests/unit/` 1186 passed 46.34s** ✅
- **Pre-PR audit raporu (kullanıcı v78 hard kuralı §1 — 4 check tamamı geçti):**
  | Kontrol | Sonuç | Detay |
  |---|---|---|
  | ORM caller scan (5-form) | ✅ | 3 production caller (eager) + 1 facade self-ref |
  | core/ ORM consumer | ✅ **0** | raw SQL ref yok |
  | Target `modules/generations/__init__.py` lazy | ✅ | docstring + future surface (upper layer) |
  | Facade poisoning (T8-6 ders) | ✅ N/A | generations B-group; `_purge_cached_modules` listesinde DEĞİL |
- **Behavior-preserving (kullanıcı kuralları §4-6):**
  - no migration write, no DB schema change, no migration edit
  - veri güvenliği invariant korunur: no rechunk/reembed/backfill; `research_clusters` + `message_clusters` tablolarına dokunulmadı
  - UPSERT pipeline + parent edges hierarchy AYNEN
  - `relationship()` string-form korunur (T8 hard kural)
  - Celery task names korunur (`tasks.research_clustering.assign|refine_hierarchy` → `embedding_queue`)
  - Production smoke read-only (no DB/RAG/embedding/manual trigger)
- **T8 cycle status (22 PR + 2 revert + 9 başarılı + 1 deferred + 1 local-proof FAIL):**
  - T8-1 v1 #1298 reverted (v68) → T8-PRE-1 v1 #1301 reverted (v69) → T8-PRE-1 v2 #1304 ✅ (v70)
  - → T8-1 v2 #1306 ✅ (v71) → T8-2 #1308 ✅ (v72) → T8-3 #1310 ✅ (v73)
  - → T8-4 #1312 ✅ (v74) → T8-5 #1314 ✅ (v75) → T8-6 #1316 ✅ (v76)
  - → T8-7 🛑 DEFERRED (v77 — `cost_tracker:35` core/ consumer)
  - → Option B local proof FAIL (v78 — POISONED facade insight)
  - → T8-8 #1320 ✅ DONE (v79 — Wave C ısınma — event family)
  - → **T8-9 #1322 ✅ DONE (v80 bu closure — Wave C continued — research/message clusters)**.
- **Pattern T8-1 v2 + T8-2..T8-6 + T8-8 + T8-9 = 8 iterasyonda kalıcı.**

### Master Plan Progress Snapshot (v80 itibarıyla)

| Alan | Durum | Not |
|---|---|---|
| T6 | ✅ CLOSED | God-file char/split/facade cycle TAM (2026-05-23); backend 141 + frontend 110 = 251 char test |
| Phase 7b | ✅ CLOSED | Frontend god-pages mini-cycle TAM (2026-05-23) |
| Phase 8 | ✅ CLOSED | 16 import-linter contract kalıcı; `core/* must not import modules/*` aktif (T8-7 hard-stop tetiği) |
| Phase 8.2 | ✅ CLOSED | ORM completion 15 PR; alembic check strict gate ACTIVE; T8 5/5 ön-şart GREEN |
| **T8** | 🔄 **IN PROGRESS ~41%** (9/22 PR ✅) | 8 başarılı (Wave A 3/3 + Wave B 3/6 + Wave C 2/7) + 1 deferred (T8-7) + 1 local-proof FAIL (Option B); pattern 8 iterasyonda kalıcı |
| T7 ([#1086](https://github.com/selmanays/nodrat/issues/1086)) | 🔓 OPEN — prerequisite | cost_tracker + 6 core/ consumer → modules/services taşıması; 7 BLOCKED T8 model unblocker; kullanıcı yetki bekliyor |
| Phase 8.1+ | ⏸️ DEFERRED | core/api code migration (148+15 import sitesi); başlatılmadı |
| Phase 8.3 | ⏸️ DEFERRED (öneri) | Raw-SQL only tables → ORM stub; kullanıcı yetki bekliyor |
| **Genel** | **İleri aşama; ana kalan iş T8 + T7** | Modular monolith mimari fazları TAM; T8 ~59% kalıyor (13 PR + T7 prerequisite). Son düzlüğe yakın DEĞİL |

- **En kritik blokajlar:**
  1. **T7 cost_tracker** — 7 BLOCKED model unblocker (provider_log, research_cache_telemetry, plan/subscription, usage_event, user, conversation, source)
  2. **POISONED facade** — her T8 PR yeni transitive risk; core/ consumer eklenmesi blokajı artırır
  3. **`relationship()` string-form discipline** — T8-21 (User 28-caller alt-PR a/b/c) için kritik
- **Hard kural takibi (kullanıcı 2026-05-26+27+28):** docs/wiki sync cycle (v80 — bu PR) tamamlanmadan sonraki safe candidate'a geçilmez. **T8-7 (provider_log/job/cost_tracker) DEFERRED kalıyor.** Option C/`ignore_imports` yasak. Option D/T7 core-consumer cleanup otomatik başlamayacak.
- **Sıradaki:** PR-T8-10 next safe candidate (öneri: `agenda` model — Wave D yeni `modules/agenda/` scaffold; AgendaCard ORM; 3 caller pre-PR audit zorunlu) veya alternatif Wave D vector hardening. Paralelde T7 cost_tracker initiative kullanıcı yetki + scope onayı bekliyor.
- **v78 audit corrections (T8-8 + T8-9 cumulative):**
  - v78 audit "event 1 caller; clusters/tasks/event.py target" demişti — gerçek caller `modules/generations/tasks/agenda.py:32`; target seçimi yine clusters
  - v78 audit "event.py 1 class" demişti — actual 2 class (EventCluster + EventArticle)
  - v78 audit "email 0 callers" demişti — actual 1 caller (`app/email/service.py:24`); ayrıca shared/email/models.py taşımanın Base import-linter prerequisite'i var (yeni keşif v80)
  - v78 audit "research_cluster 3 callers; Wave C clusters target" demişti — actual 3 callers konfirme; target generations (§2.4 revize) doğru
- **Etkilenen sayfalar:** [[t8-model-relocation-mini-plan]] (Wave C T8-14 satırı → T8-9 ✅; v80 update yorumu; TL;DR güncellemesi), [[refactor-pr-checklist]] (potansiyel yeni ders: shared/email Base prerequisite), index.md stats line.

## [2026-05-28] t8-8-done-v79 | ✅ T8-8 ✅ TAMAMLANDI — Wave C ısınma (EventCluster + EventArticle → modules/clusters/models.py)

- **Bağlam:** v78 Option B local proof FAIL kaydedildi → T8 risk-classified mode altında **ilk safe candidate** PR olarak event modeli seçildi. Pre-PR audit (kullanıcı v78 hard kuralı §1) SAFE bucket'ı konfirme etti: 1 production caller (`modules/generations/tasks/agenda.py:32`, eager) + 0 core/ ORM consumer + 0 api/ ORM consumer; target `modules/clusters` B-group → `_purge_cached_modules` listesinde DEĞİL → T8-6 LAZY+purge dersi tetiklenmez.
- **PR:** [#1320](https://github.com/selmanays/nodrat/pull/1320) `d331865` 2026-05-27 22:11 UTC merged.
- **Main CI:** run `26541773543` **11/11 GREEN** ≈3 dk (`api-unit-tests`, `lint-imports`, `ruff`, `alembic check`, `api-migration-tests`, vd. tüm jobs success).
- **Deploy.yml:** run `26541919349` **FULL success** ≈2 dk — Detect=success + **Deploy_to_VPS=success** (code change → docs-only SKIP DEĞİL); production deploy gerçekleşti.
- **Smoke (read-only):** `curl https://nodrat.com/health` → `HTTP 200` + `{"status":"ok","version":"0.1.0","service":"nodrat-api"}`. DB/RAG/embedding/manual trigger YOK (kullanıcı v78 hard kuralı §6).
- **`EventCluster` + `EventArticle` ORM modelleri artık `app/modules/clusters/models.py`'de** (önceden `app/models/event.py`); 100% rename, 156 satır, git history preserved.
- **Wave C ısınma ✅** — v78 Option B FAIL sonrası T8 risk-classified mode'da ilk başarılı safe candidate PR'ı.
- **4 dosya değişiklik** (+14 / -4; caller bütçesi ≤ 8 sınırı içinde):
  - (a) **`git mv apps/api/app/models/event.py → apps/api/app/modules/clusters/models.py`** (100% rename)
  - (b) **`apps/api/app/models/__init__.py`** — facade re-export: `from app.modules.clusters.models import EventArticle, EventCluster  # T8-8: moved 2026-05-28`
  - (c) **`apps/api/app/modules/generations/tasks/agenda.py:32`** — caller flip: eager `from app.modules.clusters.models import EventCluster` (yalnız `EventCluster`; `EventArticle` agenda.py'da kullanılmıyor — minimal-change discipline)
  - (d) **`apps/api/app/modules/clusters/README.md`** — Layout (`models.py` eklendi) + Migration history T8-8 entry
- **Local pre-flight (8/8 PASS — kalıplaşmış matris, 7. iterasyon):**
  - ruff ✅ (no fixes needed)
  - 5-form grep `app.models.event` 0 stale ref (a/b/c/d/e tüm formlar) ✅
  - facade identity check ✅ — `EventCluster`/`EventArticle` facade ≡ `app.modules.clusters.models` (Python `is` test); `__tablename__=event_clusters`/`event_articles`
  - lint-imports **16/16 kept, 0 broken** ✅ (`core/* must not import modules/*` KEPT — bu PR core/ touch etmedi)
  - mapper_resolution 3/3 PASS ✅
  - module_init_lazy 9/9 PASS ✅
  - test_admin_rag --collect-only 10 tests NO ImportError ✅
  - **TAM `pytest tests/unit/` 1186 passed 46.60s** ✅
- **Pre-PR audit raporu (kullanıcı v78 hard kuralı §1 — tüm 4 check geçti):**
  | Kontrol | Sonuç | Detay |
  |---|---|---|
  | ORM caller scan (5-form) | ✅ | 1 production caller (agenda.py:32 eager) + 1 facade self-ref |
  | core/ ORM consumer | ✅ **0** | Raw SQL `FROM event_clusters/event_articles` var ama tablo adı → import-linter scope dışı; tablo taşınmadı |
  | api/ ORM consumer | ✅ **0** | — |
  | Target `modules/clusters/__init__.py` lazy | ✅ | docstring + `__all__: list[str] = []` (B-group; T8-PRE-1 disciplinine doğal uyum) |
  | Facade poisoning T8-6 dersi (LAZY + purge) | ✅ N/A | `clusters` B-group; `_MODULES_REQUIRING_LAZY_INIT` (test_module_init_lazy.py:45-54) listesinde DEĞİL → ders tetiklemez |
- **Behavior-preserving (kullanıcı kuralları §4-6):**
  - no migration write, no DB schema change, no migration edit
  - veri güvenliği invariant korunur: no rechunk/reembed/backfill; `event_clusters` + `event_articles` tablolarına dokunulmadı (yalnız Python class taşındı)
  - `relationship()` string-form korunur; `EventCluster.articles` ↔ `EventArticle.event` back_populates AYNEN
  - Raw SQL caller'lar (`core/data_sufficiency.py`, `core/retrieval.py`, `api/admin_rag.py`) etkilenmedi → tablo adı sabit, sadece Python class taşındı
  - smoke read-only (no DB/RAG/embedding/manual trigger) — kullanıcı v78 hard kuralı §6
- **T8 cycle status (20 PR + 2 revert + 8 başarılı + 1 deferred + 1 local-proof FAIL):**
  - T8-1 v1 #1298 reverted (v68) → T8-PRE-1 v1 #1301 reverted (v69) → T8-PRE-1 v2 #1304 ✅ (v70)
  - → T8-1 v2 #1306 ✅ (v71) → T8-2 #1308 ✅ (v72) → T8-3 #1310 ✅ (v73)
  - → T8-4 #1312 ✅ (v74) → T8-5 #1314 ✅ (v75) → T8-6 #1316 ✅ (v76)
  - → T8-7 🛑 DEFERRED (v77 — `cost_tracker:35` core/ consumer)
  - → Option B local proof FAIL (v78 — POISONED facade insight)
  - → **T8-8 #1320 ✅ DONE (v79 bu closure — Wave C ısınma; safe candidate)**.
- **Pattern T8-1 v2 + T8-2 + T8-3 + T8-4 + T8-5 + T8-6 + T8-8 = 7 iterasyonda kalıcı.**
- **v78 audit corrections sürpriz keşifleri (transparency):**
  1. v78 audit "event 1 caller; target `modules/clusters/tasks/event.py`" demişti — gerçek caller `modules/generations/tasks/agenda.py:32` (cross-module read; generations upper layer clusters'ı import edebilir — boundary contract'a uygun)
  2. v78 audit "1 class" demişti — event.py 2 ORM class (EventCluster + EventArticle) içeriyordu
  3. Sonuç değişmedi: target `modules/clusters/` event-clustering domain'i olarak doğru; caller flip 1 file kaldı; SAFE bucket konfirme
- **Hard kural takibi (kullanıcı 2026-05-26+27+28):** docs/wiki sync cycle (v79 — bu PR) tamamlanmadan sonraki safe candidate'a (PR-T8-9 vb.) geçilmez. **T8-7 (provider_log/job/cost_tracker) DEFERRED kalıyor.** Option C/`ignore_imports` yasak. Option D/T7 core-consumer cleanup otomatik başlamayacak.
- **Sıradaki:** PR-T8-9 next safe candidate (öneri: `agenda` model — Wave D yeni `agenda` modülü scaffold; AgendaCard ORM; 2 caller `modules/generations/tasks/agenda.py:31` + `api/admin_rag.py` (ORM?); pre-PR audit yeniden zorunlu). Alternatif: `cluster_assignment` raw SQL only veya `event` modelinden sonra bekleyen kapsam analizi.
- **Etkilenen sayfalar:** [[t8-model-relocation-mini-plan]] (Wave C başlangıç + T8-8 entry), [[refactor-pr-checklist]] (deltası yok — 7. iterasyon pattern stable), index.md stats line.
