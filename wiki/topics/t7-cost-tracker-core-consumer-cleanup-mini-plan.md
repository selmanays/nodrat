...
| **T7-6** ✅ **DONE v105 — RECLASSIFIED (REDESIGN değil, basit decouple)** | `cost_tracker.py` — ProviderCallLog import KALDIR (raw SQL INSERT); core/'da KALIR | LOW-MED | 1 dosya | ~~model-import-in-shared~~ → **çözüm: raw INSERT (tablo-adı; import yok)** | T8-7 ProviderCallLog unblock — PR [#1367](https://github.com/selmanays/nodrat/pull/1367) `aa36fb7`; **REDESIGN→decouple reclassify** (usage pure INSERT write-only): Option B raw `db.execute(text)` + no_autoflush; Option A (DI)❌ domain→ops, Option C (ops)❌ core→ops. ZERO prod caller değişikliği. lint 16/16 (**core/ artık model-free**) + TAM 1186 + FULL deploy GREEN + **production 50 INSERT canlı kanıt** (deepseek/chat success). 🏁 **T7 7/7 TAM.** |
| **T7-7** ✅ **DONE v100-v104 → [[t7-7-deps-split-mini-plan]]** | `deps.py` → `modules/accounts/deps.py` | **HIGH** | 24 import-site / 29 dosya — **5 sub-PR (a/b/c/d/e)** | P1 atomik→shim; **P2 sources→accounts R2 contract-refine** | **TAMAMLANDI** (core/deps.py sil + R2 [[modular-monolith-boundary]]); T8-21 (User+Session) UNBLOCK |
