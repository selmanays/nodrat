...
- **[#619 Query Decomposition](https://github.com/selmanays/nodrat/issues/619) (aktif feature, mini-plan archived 2026-06-05)** — modular-monolith sonrası **ilk feature**. Retrieval-time query-side; no data/schema/embedding/RAG-index mutation; saf transform `app/prompts/query_decomposition.py` (planner/rewrite komşusu) + orchestration `app/api` (`_research_stream_body`); flag default OFF (byte-identical). §5 feature-dev kuralları + facade-first characterization disiplini uygulanıyor. Plan: [[query-decomposition-mini-plan]]. Implementation PR-1 characterization baseline ile başlar.
