...
progress: "C+0 mini-plan DONE (#1212). C+1/PR-A9 DONE (#1213, first-yield branch-matrix char, test-only, +5 test; mock=3). C+2 DONE (#1215, context/condense extraction → YENİ _research_stream_context.py 234 LoC, behavior-preserving PROD refactor, +5 helper test, 17 async-helper korundu; app_research_stream.py 1416→1274 LoC). C+3 DONE (#1217, 2nd-yield positive-path char, test-only, +1 test, mock=4 — PR-C+2 mock düşüşü testle kanıtlandı). C+4 DONE (#1219, RC3-B reframe-decision extraction → _maybe_reframe_for_faithfulness saf helper + _FAITHFULNESS_REFRAME_TEXT, behavior-preserving, +6 pure test mock=0 byte-lock + #1058 dışlama; yield + _log_coverage_gap + final_text ataması orchestrator'da). research-stream SSE char 89→101 (orchestrator 8, helpers 33→39; önceki '96' generate_sse mis-split düzeltildi). Sıradaki: Phase 6 PR-C+ kapanış değerlendirmesi (read-only; full TestClient / tool-loop timeout / persist / RC3-B orchestrator-coupling / negative-path bilinçli deferred). Full TestClient endpoint integration DEFERRED."
---

## TL;DR

T6 #1085'in Phase 6 alt-kalemi (`app_research_stream.py` SSE god-file) için kalan **derin orchestration characterization** borcunu küçük, test-first, düşük-riskli PR'lara böler. SSE çıktı kontratı (yield şekli/sırası) zaten kilitli (15 P6 PR, 101 research-stream test); açık olan **yield-arası 3.+ orchestrator path** + **full endpoint integration**. C+1 first-yield ✅ + C+2 context/condense extraction ✅ + C+3 2nd-yield positive-path ✅ + C+4 RC3-B reframe-decision extraction ✅ (behavior-preserving pure-helper, mock=0); sıradaki **Phase 6 PR-C+ kapanış değerlendirmesi** (full TestClient / tool-loop timeout / persist / RC3-B orchestrator-coupling / negative-path bilinçli deferred); full TestClient SSE integration **şimdilik deferred**.
