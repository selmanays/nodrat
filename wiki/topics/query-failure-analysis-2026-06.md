...
2. **Öncelik 2 — search-arg observability micro-PR için reality-analysis:** tool-call arama metninin `thinking_steps` metadata'sına PII-bilinçli persist'i (PR-F deseninin devamı). Hem sınıf-3 retrieval-miss (§4) hem sınıf-A gündem-dump (§8) teşhisinin ortak ön-koşulu. **✅ Güncelleme (2026-06-12): #1483 implemented + micro-canary validated** ([#1486](https://github.com/selmanays/nodrat/pull/1486); canary: 5/5 searches[], 11/11 key-set, PII/log sızıntısı 0, behavior-neutral, rollback tamam; detay [[search-arg-observability-analysis-2026-06]]). Search-arg observability **canlıda çalışıyor ama flag OFF** (kalıcı ON ayrı prod-config kararı). **✅ #1484 citation-gap guard da implemented (2026-06-12, [#1489](https://github.com/selmanays/nodrat/pull/1489), default OFF).** **S-2 micro-canary yapıldı (2026-06-12): güvenlik temiz (retry 0/7, yanlış-pozitif 0, zarar 0) ama etkinlik kanıtı yok; flag'ler reset/OFF.** Q5'in ilk "forced-final faz-2 kanıtı" yorumu geri çekildi (L1 cross-conv enrichment kök nedeni — [[citation-gap-guard-analysis-2026-06]] Q5 düzeltmesi). **Yeni öncelik: "new-search context isolation / L1 retrieval_forced classification" reality-analysis** (ayrı onay); kalıcı activation YOK.
