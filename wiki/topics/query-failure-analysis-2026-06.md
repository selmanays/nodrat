...
2. **Öncelik 2 — search-arg observability micro-PR için reality-analysis:** tool-call arama metninin `thinking_steps` metadata'sına PII-bilinçli persist'i (PR-F deseninin devamı). Hem sınıf-3 retrieval-miss (§4) hem sınıf-A gündem-dump (§8) teşhisinin ortak ön-koşulu. **✅ Güncelleme (2026-06-12): #1483 implemented** ([#1486](https://github.com/selmanays/nodrat/pull/1486), flag `research.search_arg_telemetry_enabled` **default OFF**, prod no-op; detay [[search-arg-observability-analysis-2026-06]]). Sıradaki doğal adım: S-1 flag-enable/canary **veya** #1484 (S-2) reality-analysis/implementation — **her ikisi ayrı karar/onay**.
