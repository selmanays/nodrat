...
|---|---|---|---|
| **T8-12a** (decouple) | sources/tasks Article ORM query → raw SQL; sources→articles edge KALDIR | 1 dosya (sources/tasks/sources.py) + targeted test | LOW — davranış-koruyan; Article hâlâ flat; pre-flight lint-imports 16/16 (sources→articles zaten yokken de PASS, ama bu PR onu kalıcı decouple eder) |
