...
| **T8-12b** ✅ **DONE v98** | `article.py` → `modules/articles/models.py` + facade + **11 caller DIRECT flip** | 18 dosya (model + facade + 11 caller + 3 docstring + test path fix + README) | **TAMAMLANDI** PR [#1354](https://github.com/selmanays/nodrat/pull/1354) `db78a34`; atomik 11 caller DIRECT; relationship() internal → mapper 3/3; facade identity Article+ArticleImage; Vector korundu; lint-imports 16/16; TAM 1186; FULL deploy GREEN + SSH 13/13. 1 source-text path fix (test_articles_cleaned_at, T7-2 dersi). **🏁 article split TAM.** |
