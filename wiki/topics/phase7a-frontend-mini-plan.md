...
created: "2026-05-21"
updated: "2026-05-22"
progress: "24 PR DONE — PHASE 7a TEKNİK SPLIT TAMAMLANDI (PR-7a-0..19b; Research #793 TAM = 19a non-SSE + 19b SSE client); 110 char test cumulative; api.ts -1461 LoC (2041 → 580, ~%72); artık yalnız Core + facade re-export, çıkarılacak domain KALMADI; 21 facade doğrulama; api/research.ts tam Research domain modülü (264 LoC; 6 if + 8 fn; API_BASE export value-unchanged); api/admin/rag.ts tek kohezyon (421 LoC); kalan (Phase 7a'yı bloklamaz): renameResearchConversation + createConfig 0-caller cleanup; SIRADA: Phase 7a kapanış değerlendirmesi (issue #1095 + T6 P7a; kullanıcı onayı)"
sources:
...
| 18a | PR-7a-18a | **Admin RAG read-only observability extract** (#189 Part 1/2) → YENİ `api/admin/rag.ts` | ~250 LoC | 1 caller (`/admin/rag`) | Düşük | ✅ **DONE** ([#1204](https://github.com/selmanays/nodrat/pull/1204), YENİ `api/admin/rag.ts` 281 LoC; 19 read-only interface + 9 read-only GET fn (ragHealth/ragBenchmarkHistory/ragBenchmarkStatus/ragCitationStats/ragRerankStats/ragCacheTelemetry/ragRaptorClusters/ragNerStats/ragPipelineComparison); **interleaved read-only/trigger split** tsc-doğrulandı; 3 trigger fn inline kaldı (18b); cumulative 94 test) |
| 18b | PR-7a-18b | **Admin RAG trigger/pipeline extract** (#189 Part 2/2) → mevcut `api/admin/rag.ts` | ~130 LoC | 1 caller | Düşük (3 trigger smoke-skip) | ✅ **DONE** ([#1206](https://github.com/selmanays/nodrat/pull/1206), 3 trigger fn (`ragBenchmarkRun`/`ragRaptorTrigger`/`ragInspectQuery`) + 9 trigger interface → mevcut `api/admin/rag.ts` (281 → 421 LoC); **contiguous blok** tsc-doğrulandı; api.ts 900 → 787 LoC; **Admin RAG #189 TAM ayrıldı**; benchmark/RAPTOR/inspect prod'da TETİKLENMEDİ; cumulative 98 test) |
| 19a | PR-7a-19a | **Research non-SSE extract** (#793 Part 1/2) → YENİ `api/research.ts` | ~146 LoC | 5 caller (research/* + feedback) | Orta | ✅ **DONE** ([#1208](https://github.com/selmanays/nodrat/pull/1208), YENİ `api/research.ts` 180 LoC; 6 interface + 7 apiFetch fn (list/get read-only + create/rename/archive conversation mutation + flagHalu/recordAction feedback mutation); `streamResearchMessage` SSE inline kaldı (19b); **`API_BASE` export YOK**; `renameResearchConversation` 0-caller korundu; `buildQuery` dead import temizlendi (ESLint yakaladı); tsc inline SSE tip-ref'lerini doğruladı; cumulative 106 test) |
| 19b | PR-7a-19b | **`streamResearchMessage` SSE client extract** (#793 Part 2/2) → mevcut `api/research.ts` | ~70 LoC | 1 caller (research/[id]) | Yüksek — SSE/ReadableStream | ✅ **DONE** ([#1210](https://github.com/selmanays/nodrat/pull/1210), `streamResearchMessage` raw-fetch SSE/ReadableStream client → api/research.ts (180 → 264 LoC); api.ts 657 → 580 LoC; **`API_BASE` export edildi value-unchanged**; apiFetch/ApiException/token/refresh TAŞINMADI; SSE parse byte-for-byte korundu; +4 char test (fetch+ReadableStream mock); prod'da TETİKLENMEDİ; **Research #793 TAM ayrıldı → Phase 7a teknik split DONE**; cumulative 110 test) |
