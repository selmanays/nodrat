...
created: "2026-05-21"
updated: "2026-05-22"
progress: "21 PR DONE (PR-7a-0..17 + PR-7a-18a admin-rag-readonly); 94 char test cumulative; api.ts -1141 LoC (2041 → 900, ~%56; ilk kez <1000); 18 facade doğrulama; api/admin/rag.ts read-only (19 if + 9 GET fn; interleaved split tsc-doğrulandı; 3 trigger inline → 18b); kalan: Admin RAG 18b trigger (Admin RAG'i tamamlar) + Research+feedback+SSE (EN SONA, tek kalan büyük) + createConfig cleanup; PR-7a-18b RAG trigger sırada; Research deferred"
sources:
...
| 18b | PR-7a-18b | **Admin RAG trigger/pipeline extract** (#189 Part 2/2) → mevcut `api/admin/rag.ts` | ~130 LoC | 1 caller | Düşük (3 trigger smoke-skip) | 🔄 **SIRADA** (closure docs v24 sonrası; `ragBenchmarkRun`/`ragRaptorTrigger`/`ragInspectQuery` + trigger/inspect interface'ler; benchmark/RAPTOR/pipeline prod'da TETİKLENMEZ) |
