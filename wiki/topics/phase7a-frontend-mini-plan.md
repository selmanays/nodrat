...
created: "2026-05-21"
updated: "2026-05-22"
progress: "23 PR DONE (PR-7a-0..19a; Research non-SSE = 6 if + 7 apiFetch fn → api/research.ts); 106 char test cumulative; api.ts -1384 LoC (2041 → 657, ~%68); 20 facade doğrulama; api/research.ts (180 LoC; streamResearchMessage SSE inline → 19b; API_BASE export YOK; renameResearchConversation 0-caller korundu; buildQuery dead import temizlendi-ESLint); api/admin/rag.ts tek kohezyon (421 LoC); kalan: streamResearchMessage SSE client (19b, Phase 7a SON teknik hamle) + renameResearchConversation/createConfig cleanup; PR-7a-19b SSE sırada (closure v26 sonrası implementation)"
sources:
...
| 19b | PR-7a-19b | **`streamResearchMessage` SSE client extract** (#793 Part 2/2) → mevcut `api/research.ts` | ~70 LoC | 1 caller (research/[id]) | Yüksek — SSE/ReadableStream | 🔄 **SIRADA** (closure docs v26 sonrası; `API_BASE` minimal export; raw fetch + ReadableStream client; SSE event parse davranışı korunur; prod'da TETİKLENMEZ — fetch+ReadableStream mock; **Research'ü tamamlar = Phase 7a SON teknik hamle**) |
