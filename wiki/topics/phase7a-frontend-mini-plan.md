...
created: "2026-05-21"
updated: "2026-05-22"
progress: "22 PR DONE (PR-7a-0..18b; Admin RAG #189 TAM ayrıldı = 18a read-only + 18b trigger); 98 char test cumulative; api.ts -1254 LoC (2041 → 787, ~%61); 19 facade doğrulama; api/admin/rag.ts tek kohezyon modül (421 LoC; 28 if + 12 fn; read-only observability + trigger/pipeline; 18b contiguous blok tsc-doğrulandı); kalan: Research+feedback+streamResearchMessage SSE (SON büyük domain) + createConfig cleanup; PR-7a-19 Research+SSE scope analizi sırada (kullanıcı onayı sonrası implementation)"
sources:
...
| 19 | PR-7a-19 | **Research + message-feedback + `streamResearchMessage` SSE cluster** (~225 LoC) → muhtemel `api/research.ts` | ~225 LoC | 6 caller (research/* + feedback) | Yüksek — SON büyük domain | 🔄 **SIRADA** (closure docs v25 sonrası **scope analizi**; SSE client coupling / ReadableStream; backend P6 PR-A8 ile bağ; conversation CRUD + feedback + stream; kullanıcı onayı sonrası implementation) |
