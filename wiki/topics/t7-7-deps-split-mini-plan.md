...
| **T7-7c** ✅ **DONE v102** | api/ app+auth caller flip (app_consent/app_me/app_research/app_research_stream/auth/auth_2fa/billing/public_search) | 8 | **TAMAMLANDI** PR [#1361](https://github.com/selmanays/nodrat/pull/1361) `32a0c11`; mekanik flip (auth.py fonksiyon-içi dahil; ruff --fix 6 I001); lint-imports 16/16; 8 modül import OK; module_init+research_stream 65 passed; TAM 1186; FULL deploy GREEN + SSH 13/13. |
| **T7-7d** ✅ **DONE v103** | non-sources modül caller flip (articles/legal/media/prompts_admin/settings_admin/sft/style_profiles) | 7 | **TAMAMLANDI** PR [#1363](https://github.com/selmanays/nodrat/pull/1363) `dc2a20c`; 7 modül flip (hepsi →accounts LEGAL); ruff --fix 7 I001; lint-imports 16/16; **module_init 9/9 (bu 7 canary subject — lazy etkilenmez)**; TAM 1186; FULL deploy GREEN + SSH 13/13. **23/24 caller flip TAM; kalan yalnız sources/admin (T7-7e).** |
| **T7-7e** ✅ **DONE v104** | **FINAL:** gerçek kod core→accounts taşı (shim→real) + **R2 contract refine** + sources/admin flip + core/deps.py SİL + **canary test güncelle** (`app.core.deps`→`app.modules.accounts.deps`) + research_stream yorum güncelle | 9 dosya | **TAMAMLANDI** PR [#1365](https://github.com/selmanays/nodrat/pull/1365) `bc2e357`; git mv core/deps.py→accounts/deps.py + R2 (pyproject:224 accounts sources-forbidden'dan çıktı + [[modular-monolith-boundary]] decision note) + sources/admin flip (24/24 TAM) + canary 8-ref + 4 research_stream yorum + core/deps.py SİL. Pre-flight: 5-form grep `app.core.deps`=0 (import) / lint-imports **16/16** (core→modules KEPT, User edge kalktı; sources→accounts LEGAL) / canary 9/9 / mapper 3/3 / TAM 1186. **⚠️ Deploy CI "cancelled" = public smoke 502 false-fail (T8-15 v90 emsali); container swap TAMAMLANDI → production T7-7e + 13/13 healthy + ~5h stabil (SSH doğrulandı).** **🏁 T8-21 (User+Session) UNBLOCK.** |

> Toplam 5 sub-PR ✅ TAM (v100-v104). T7-7a..d (23/24 caller) otonom; T7-7e P2/contract/canary'yi taşıdı. Her PR sonrası FULL deploy + SSH smoke + lint-imports.
>
> 🏁 **T7-7 deps split TAMAMLANDI (5/5; v100-v104).** core/deps.py silindi → `core/*` artık `app.models.user` import etmez → **T8-21 (User+Session → accounts/models.py) UNBLOCK**. core/ kalan tek `app.models` importu: cost_tracker→ProviderCallLog (T7-6). **R2 mimari karar** (sources→accounts auth cross-cutting) [[modular-monolith-boundary]]'de kayıtlı. **Deploy notu:** T7-7e public-smoke 502 false-fail (api cold-start; T8-15 v90 paterni; v91 fix 120s'ye çıkardı ama yine aştı) → CI "cancelled"; production container swap tamamlandı + 5h stabil (re-deploy YAPILMADI; functional success). **Sıradaki track:** T7-6 cost_tracker redesign (kullanıcı öncelik #3) → email/Base → T8 milestone closure (T8-21/T8-7/T8-9 harvest).
