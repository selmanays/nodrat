...
_NER_PROMPT_SYSTEM = """Sen Türkçe haber metinlerinden özel ad çıkarıcı bir
asistansın. Verilen başlık + içerik metninden:
  - person: kişi adı (Emine Aydınbelge, Fatih Tutak, Cumhurbaşkanı Erdoğan)
  - place: yer adı (Rodos, Salt Galata, İstanbul, Karşıyaka)
  - org: kurum/şirket/takım adı (Bursaspor, Cengiz Holding, MKE)
  - event: etkinlik/olay (15 Temmuz, SAHA 2026, Şehit Anneler Programı)
  - money: para miktarı (488 milyon dolar, 11 milyar TL)
  - number: niş sayısal değer (yüzde 42, 21 ülke, 5 kent)

Sadece JSON array döndür, başka metin YOK:
[
  {"text": "Emine Aydınbelge", "type": "person"},
  {"text": "Şehit Anneler Programı", "type": "event"},
  ...
]
...
- Max 20 entity döndür
- Tarihler (5 Mayıs 2026) atla — bunlar planner timeframe'inde
"""
...
async def _extract_article_entities_async(article_id: UUID) -> dict:
    ...
    async with factory() as db:
        ...
        for ent in entities_raw[:30]:  # cap 30 entity/article
