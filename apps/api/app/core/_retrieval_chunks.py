"""retrieval chunks hybrid search — RRF + NER + parent-doc + rerank (P5 B7, v3).

app/core/retrieval.py'den ÇIKARILAN (behavior-preserving pure move). Article-chunks
için sparse+dense RRF fusion + NER IDF boost (STAGE 1 RESCUE / STAGE 2 FILTER) +
phrase/gram boost + parent-document expansion + semantic rerank. retrieval god-file'ın
en büyük public fonksiyonu (801 satır). Mantık BİREBİR korundu → recall/ranking
by-construction sabit (logic dokunulmadı; corpus benchmark manuel/staging gate).

PUBLIC: research_tools + admin_rag + retrieval_benchmark + niche_chunks_benchmark
`from app.core.retrieval import hybrid_search_chunks` ile erişir → retrieval.py
