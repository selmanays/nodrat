...
def upgrade() -> None:
    # Halu flag
    op.add_column("messages", sa.Column("halu_flagged_at", sa.DateTime(timezone=True), nullable=True))
    ...
    op.add_column("messages", sa.Column("user_action_at", sa.DateTime(timezone=True), nullable=True))
    ...
    op.add_column("messages", sa.Column("sft_recomputed_at", sa.DateTime(timezone=True), nullable=True))
...
def downgrade() -> None:
    ...
    for col in (
        "dpo_chosen_content", "dpo_rejected",
        "sft_recomputed_at", "sft_excluded_reason", "sft_eligible",
        "edited_content", "edit_distance", "user_action_at", "user_action",
        "halu_flagged_reason", "halu_flagged_by", "halu_flagged_at",
    ):
